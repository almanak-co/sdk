...
------
    ...
    python -m src.cli.run --strategy demo_uniswap_lp --once
...
class UniswapLPStrategy(IntentStrategy[UniswapLPConfig]):
    ...
    def decide(self, market: MarketSnapshot) -> Intent | None:
        ...
        if self._current_position_id:
            ...
            # rebalance blindly.
            return Intent.hold(reason=f"Position {self._current_position_id} exists - range unknown")
        ...
        logger.info("No position - opening LP position with balanced inventory")
        add_event(
            TimelineEvent(
                timestamp=datetime.now(UTC),
                event_type=TimelineEventType.STATE_CHANGE,
                description="Opening LP position with balanced inventory",
