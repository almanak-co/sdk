...
on Ethereum mainnet — the first Euler V2 kitchenloop test on Ethereum.

Lifecycle steps (one per iteration, sequential):
1. SUPPLY: Deposit USDC into the eUSDC-2 vault
2. WITHDRAW: Withdraw USDC from the vault
3. HOLD: Lifecycle complete

Uses the eUSDC-2 vault (0x797DD80692c3b2dAdabCe8e30C07fDE5307D48a9)
on Ethereum mainnet.
...
@almanak_strategy(
    name="euler_v2_supply_ethereum",
    description="Euler V2 supply lifecycle on Ethereum: deposit USDC -> withdraw USDC",
    version="1.0.0",
    author="Kitchen Loop",
...
class EulerV2SupplyEthereumStrategy(IntentStrategy):
    """Euler V2 supply lifecycle on Ethereum.

    Deposit USDC into the eUSDC-2 ERC-4626 vault, then withdraw.
    Tests the Euler V2 deposit/withdraw flow on Ethereum mainnet.
    """
    ...
    def get_status(self) -> dict[str, Any]:
        return {
            "strategy": "euler_v2_supply_ethereum",
            "chain": self.chain,
            "state": self._loop_state,
        ...
        if self._supplied_amount > 0:
            positions.append(
                PositionInfo(
                    ...
                    position_id=f"euler_v2-supply-{self.supply_token}-ethereum",
                    chain=self.chain,
                    protocol="euler_v2",
