"""PancakeSwap V3 RSI Swap Strategy — PnL Backtesting on BSC.

This strategy exercises the PnL backtester with PancakeSwap V3 swap intents
on BSC. PancakeSwap V3 swap was validated on BSC in iter 120 (PR #972) but
has never been exercised through the PnL backtesting pipeline.

PURPOSE:
--------
1. First PnL backtest on BSC (all prior PnL backtests are on other chains).
2. First PnL backtest for PancakeSwap V3 (only Uniswap V3, Aerodrome,
   TraderJoe, Curve, Aave, Morpho, Compound covered so far).
3. Validates historical price data for BNB/USDC on BSC.

USAGE:
------
    almanak strat backtest pnl -s demo_pancakeswap_pnl_swap_bsc \\
        --start 2024-01-01 --end 2024-06-01 --interval 1h

STRATEGY LOGIC:
---------------
Each tick:
  1. Read RSI(BNB, 14-period)
  2. If RSI < 30 (oversold) and have USDC -> buy WBNB
...
@almanak_strategy(
    name="demo_pancakeswap_pnl_swap_bsc",
    description="PancakeSwap V3 RSI swap for PnL backtesting on BSC — buy/sell WBNB based on RSI",
    version="1.0.0",
    author="Almanak",
    tags=["demo", "backtesting", "swap", "pancakeswap-v3", "bsc", "pnl"],
    supported_chains=["bsc"],
    default_chain="bsc",
    supported_protocols=["pancakeswap_v3"],
    intent_types=["SWAP", "HOLD"],
)
class PancakeSwapPnLSwapBSCStrategy(IntentStrategy):
    """RSI-based swap strategy using PancakeSwap V3 on BSC.
    ...
    def decide(self, market: MarketSnapshot) -> Intent | None:
        ...
        if rsi_value > self.rsi_overbought:
            ...
            base_amount = (self.trade_size_usd / base_price).quantize(Decimal("0.0001"))

            try:
                base_balance = market.balance(self.base_token)
                # Compare token amounts, not USD — avoids blocking sells when
                # position value dipped below trade_size_usd due to fees/slippage
    ...
    def on_intent_executed(self, intent: Intent, success: bool, result: Any) -> None:
        ...
        if intent_type and intent_type.value == "SWAP":
            ...
            if from_token == self.quote_token:
                self._total_buys += 1
                # Track base token acquired — handle both live (swap_amounts)
                # and PnL backtest (TradeRecord with actual_amount_out) results
    ...
    def get_status(self) -> dict[str, Any]:
        return {
            "strategy": "demo_pancakeswap_pnl_swap_bsc",
            "chain": self.chain,
        ...
        if self._base_held > 0:
            positions.append(
                PositionInfo(
                    ...
                    position_id=f"pancakeswap-{self.base_token}",
                    chain=self.chain,
                    protocol="pancakeswap_v3",
                    value_usd=Decimal("0"),
                    details={
