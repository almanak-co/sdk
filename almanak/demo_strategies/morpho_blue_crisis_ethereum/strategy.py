"""
===============================================================================
Morpho Blue Crisis Scenario Backtest — Lending Under Stress on Ethereum
===============================================================================

Stress-tests Morpho Blue wstETH supply under historical crisis conditions on
Ethereum. First crisis scenario backtest for Morpho Blue and first crisis
scenario on Ethereum mainnet.

CRISIS PARAMETERS:
- Higher withdrawal sensitivity (15% drawdown threshold) to protect capital
- Automatic re-supply when drawdown recovers below 8%
- Longer max hold (50 ticks) since lending is more passive than trading
- Uses wstETH/USDC market (market_id in config.json)

USAGE:
------
    # Run against predefined crisis scenario
    almanak strat backtest scenario \
        -s demo_morpho_blue_crisis_ethereum \
        --scenario black_thursday \
        --chain ethereum \
        --tokens wstETH,USDC,WETH \
        --initial-capital 10000

    # Run against all 3 predefined scenarios
    almanak strat backtest scenario \
        -s demo_morpho_blue_crisis_ethereum \
        --scenario terra_collapse \
        --chain ethereum --tokens wstETH,USDC,WETH

    almanak strat backtest scenario \
        -s demo_morpho_blue_crisis_ethereum \
        --scenario ftx_collapse \
        --chain ethereum --tokens wstETH,USDC,WETH

KEY METRICS TO WATCH:
- Max drawdown during crisis vs hold-only baseline
- Number of withdraw/re-supply cycles during crisis
- Capital preservation (lending has lower IL but protocol/liquidation risk)

===============================================================================
"""

import logging
from datetime import UTC, datetime
from decimal import ROUND_DOWN, Decimal
from typing import TYPE_CHECKING, Any

from almanak.framework.data import PriceUnavailableError
from almanak.framework.intents import Intent
from almanak.framework.market import MarketSnapshot
from almanak.framework.strategies import IntentStrategy, almanak_strategy

logger = logging.getLogger(__name__)

if TYPE_CHECKING:
    from almanak.framework.teardown import TeardownMode, TeardownPositionSummary


@almanak_strategy(
    name="demo_morpho_blue_crisis_ethereum",
    description="Crisis scenario stress test -- Morpho Blue wstETH lending on Ethereum",
    version="1.0.0",
    author="Almanak",
    tags=["demo", "crisis", "scenario-backtest", "lending", "morpho-blue", "ethereum", "backtesting"],
    supported_chains=["ethereum"],
    supported_protocols=["morpho_blue"],
    intent_types=["SUPPLY", "WITHDRAW", "HOLD"],
    default_chain="ethereum",
)
class MorphoBlueCrisisEthereumStrategy(IntentStrategy):
    """Morpho Blue lending strategy tuned for crisis scenario backtesting on Ethereum.

    Supplies wstETH to a Morpho Blue market and monitors ETH price drawdown
    during crisis periods. Withdraws when drawdown exceeds threshold, re-supplies
    when conditions stabilize. Uses price-based drawdown detection since
    lending rate data is typically unavailable in the backtester context.

    Configuration (config.json):
        market_id: Morpho Blue market identifier (bytes32 hex)
        supply_token: Token to supply (default: wstETH)
        supply_amount: Amount to supply (default: 2)
        withdraw_drawdown_threshold: Portfolio drawdown to trigger withdrawal (default: 0.15 = 15%)
        resupply_recovery_threshold: Drawdown recovery to trigger re-supply (default: 0.08 = 8%)
        max_hold_ticks: Max ticks to hold before forced withdrawal (default: 50)
        force_entry_if_no_rate: Supply on first tick without rate data (default: True for backtest)
    """

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)

        self.market_id = self.get_config(
            "market_id", "0xb323495f7e4148be5643a4ea4a8221eef163e4bccfdedc2a6f4696baacbc86cc"
        )
        self.supply_token = self.get_config("supply_token", "wstETH")
        self.supply_amount = Decimal(str(self.get_config("supply_amount", "2")))
        self.withdraw_drawdown_threshold = Decimal(
            str(self.get_config("withdraw_drawdown_threshold", "0.15"))
        )
        self.resupply_recovery_threshold = Decimal(
            str(self.get_config("resupply_recovery_threshold", "0.08"))
        )
        self.max_hold_ticks = int(self.get_config("max_hold_ticks", 50))

        raw_force = self.get_config("force_entry_if_no_rate", True)
        if isinstance(raw_force, bool):
            self.force_entry_if_no_rate = raw_force
        elif isinstance(raw_force, str):
            self.force_entry_if_no_rate = raw_force.strip().lower() in {"1", "true", "yes", "on"}
        else:
            self.force_entry_if_no_rate = bool(raw_force)

        # State machine: idle -> supplying -> supplied -> withdrawing -> idle (cycle)
        self._state = "idle"
        self._previous_stable_state = "idle"
        self._supplied_amount = Decimal("0")
        self._ticks_held = 0
        self._peak_value = Decimal("0")
        self._peak_needs_init = False  # True when supplied without price; init on first available price
        self._cycle_count = 0
        self._last_market_timestamp: datetime | None = None
        self._last_eth_price: Decimal | None = None
        self._last_supply_token_price: Decimal | None = None

        logger.info(
            "MorphoBlueCrisisEthereumStrategy initialized: "
            "supply=%s %s, market_id=%s..., withdraw_threshold=%.0f%%, resupply_threshold=%.0f%%",
            self.supply_amount,
            self.supply_token,
            self.market_id[:18],
            float(self.withdraw_drawdown_threshold) * 100,
            float(self.resupply_recovery_threshold) * 100,
        )

    def decide(self, market: MarketSnapshot) -> Intent | None:
        """Make supply/withdraw decisions based on ETH price drawdown."""
        self._last_market_timestamp = getattr(market, "timestamp", None)

        # Track ETH price for drawdown monitoring
        eth_price = self._get_eth_price(market)
        # Track supply token price separately for accurate position valuation
        self._last_supply_token_price = self._get_supply_token_price(market)

        if self._state == "idle":
            if self._cycle_count == 0:
                # First cycle: supply if we have price data or force_entry_if_no_rate is set
                if eth_price is None and not self.force_entry_if_no_rate:
                    return Intent.hold(reason="Idle -- waiting for price data before first supply")
            elif eth_price is not None and self._peak_value > 0:
                # After a withdrawal: only re-supply once drawdown recovers
                drawdown_from_peak = (self._peak_value - eth_price) / self._peak_value
                if drawdown_from_peak >= self.resupply_recovery_threshold:
                    return Intent.hold(
                        reason=f"Idle -- drawdown {float(drawdown_from_peak) * 100:.1f}% "
                        f"still above {float(self.resupply_recovery_threshold) * 100:.0f}% recovery threshold"
                    )
            else:
                # No price data after a withdrawal -- hold until we can verify recovery
                return Intent.hold(reason="Idle -- waiting for price data to check recovery")

            logger.info(
                "SUPPLY %s %s to Morpho Blue market %s... (cycle #%d)",
                self.supply_amount,
                self.supply_token,
                self.market_id[:18],
                self._cycle_count + 1,
            )
            self._previous_stable_state = self._state
            self._state = "supplying"
            return Intent.supply(
                protocol="morpho_blue",
                token=self.supply_token,
                amount=self._round_amount(self.supply_amount),
                market_id=self.market_id,
                chain=self.chain,
            )

        if self._state == "supplied":
            self._ticks_held += 1

            # Track peak and compute drawdown
            if eth_price is not None:
                if self._peak_needs_init:
                    # First price after a no-price auto-advance -- initialize peak now
                    self._peak_value = eth_price
                    self._peak_needs_init = False
                if eth_price > self._peak_value:
                    self._peak_value = eth_price
                drawdown = (
                    (self._peak_value - eth_price) / self._peak_value
                    if self._peak_value > 0
                    else Decimal("0")
                )
            else:
                drawdown = Decimal("0")

            # Withdraw on crisis drawdown or max hold
            should_withdraw = (
                drawdown >= self.withdraw_drawdown_threshold
                or self._ticks_held >= self.max_hold_ticks
            )

            if should_withdraw:
                reason = (
                    f"Drawdown {float(drawdown) * 100:.1f}% >= "
                    f"{float(self.withdraw_drawdown_threshold) * 100:.0f}% threshold"
                    if drawdown >= self.withdraw_drawdown_threshold
                    else f"Max hold ticks ({self.max_hold_ticks}) reached"
                )
                logger.info(
                    "WITHDRAW %s %s from Morpho Blue market %s... (%s, cycle #%d)",
                    self._supplied_amount,
                    self.supply_token,
                    self.market_id[:18],
                    reason,
                    self._cycle_count,
                )
                self._previous_stable_state = self._state
                self._state = "withdrawing"
                return Intent.withdraw(
                    protocol="morpho_blue",
                    token=self.supply_token,
                    amount=self._round_amount(self._supplied_amount),
                    withdraw_all=True,
                    market_id=self.market_id,
                    chain=self.chain,
                )

            drawdown_str = f"{float(drawdown) * 100:.1f}%" if eth_price else "N/A"
            return Intent.hold(
                reason=f"Holding supply (ticks={self._ticks_held}/{self.max_hold_ticks}, drawdown={drawdown_str})"
            )

        if self._state in ("supplying", "withdrawing"):
            # Auto-advance stuck transitional states (PnL backtester doesn't call on_intent_executed)
            previous = self._state
            if self._state == "supplying":
                self._state = "supplied"
                self._supplied_amount = self.supply_amount
                self._ticks_held = 0
                if eth_price is not None:
                    self._peak_value = eth_price
                    self._peak_needs_init = False
                else:
                    # No price available yet; mark for init on first available price
                    # so crisis detection is not silently disabled.
                    self._peak_needs_init = True
                    logger.warning("No ETH price on supply auto-advance; peak will init on next available price")
            else:
                self._state = "idle"
                self._supplied_amount = Decimal("0")
                self._cycle_count += 1
            logger.warning("Stuck in '%s', auto-advancing to '%s'", previous, self._state)
            return Intent.hold(reason=f"Auto-advanced from {previous} to {self._state}")

        return Intent.hold(reason=f"Unexpected state: {self._state}")

    def _round_amount(self, amount: Decimal, decimals: int = 18) -> Decimal:
        """Round down to token precision to avoid overspend/revert at execution boundaries."""
        quantum = Decimal(1).scaleb(-decimals)
        return amount.quantize(quantum, rounding=ROUND_DOWN)

    def _get_eth_price(self, market: MarketSnapshot) -> Decimal | None:
        """Get ETH price for drawdown tracking. wstETH is excluded -- it trades at a
        premium over ETH (~1.17x) and would corrupt the drawdown baseline."""
        for token in ("ETH", "WETH"):
            try:
                price = market.price(token)
                self._last_eth_price = price
                return price
            except (PriceUnavailableError, ValueError, KeyError, AttributeError):
                continue
        return None

    def _get_supply_token_price(self, market: MarketSnapshot) -> Decimal | None:
        """Get supply token price for position valuation (separate from ETH drawdown proxy)."""
        try:
            price = market.price(self.supply_token)
            self._last_supply_token_price = price
            return price
        except (PriceUnavailableError, ValueError, KeyError, AttributeError):
            return None

    def on_intent_executed(self, intent: Intent, success: bool, result: Any = None) -> None:
        """Track execution results and advance state machine."""
        intent_type = intent.intent_type.value

        if success:
            if intent_type == "SUPPLY":
                self._state = "supplied"
                self._supplied_amount = self.supply_amount
                self._ticks_held = 0
                # Reset peak to current price on every supply so drawdown detection
                # starts from the new entry point, not from a previous cycle's ATH.
                if self._last_eth_price is not None:
                    self._peak_value = self._last_eth_price
                logger.info("SUPPLY confirmed: %s %s -> state=supplied", self.supply_amount, self.supply_token)

            elif intent_type == "WITHDRAW":
                self._state = "idle"
                self._supplied_amount = Decimal("0")
                self._cycle_count += 1
                # Preserve _peak_value so re-entry can check recovery threshold
                logger.info("WITHDRAW confirmed: cycle #%d complete -> state=idle", self._cycle_count)
        else:
            revert_to = self._previous_stable_state
            logger.warning("%s failed, reverting to '%s'", intent_type, revert_to)
            self._state = revert_to

    def get_status(self) -> dict[str, Any]:
        return {
            "strategy": "demo_morpho_blue_crisis_ethereum",
            "chain": self.chain,
            "state": self._state,
            "supplied_amount": str(self._supplied_amount),
            "ticks_held": self._ticks_held,
            "peak_value": str(self._peak_value),
            "cycle_count": self._cycle_count,
        }

    def get_persistent_state(self) -> dict[str, Any]:
        return {
            "state": self._state,
            "previous_stable_state": self._previous_stable_state,
            "supplied_amount": str(self._supplied_amount),
            "ticks_held": self._ticks_held,
            "peak_value": str(self._peak_value),
            "peak_needs_init": self._peak_needs_init,
            "cycle_count": self._cycle_count,
        }

    def load_persistent_state(self, state: dict[str, Any]) -> None:
        if "state" in state:
            self._state = state["state"]
        if "previous_stable_state" in state:
            self._previous_stable_state = state["previous_stable_state"]
        if "supplied_amount" in state:
            self._supplied_amount = Decimal(str(state["supplied_amount"]))
        if "ticks_held" in state:
            self._ticks_held = int(state["ticks_held"])
        if "peak_value" in state:
            self._peak_value = Decimal(str(state["peak_value"]))
        if "peak_needs_init" in state:
            self._peak_needs_init = bool(state["peak_needs_init"])
        if "cycle_count" in state:
            self._cycle_count = int(state["cycle_count"])

    # =========================================================================
    # TEARDOWN
    # =========================================================================

    def get_open_positions(self) -> "TeardownPositionSummary":
        from almanak.framework.teardown import PositionInfo, PositionType, TeardownPositionSummary

        positions: list[PositionInfo] = []

        if self._supplied_amount > 0:
            # Use last known price as a best-effort USD estimate; this strategy is
            # backtest-only so on-chain balance queries are not available.
            supply_price = self._last_supply_token_price
            value_usd = (
                self._supplied_amount * supply_price
                if supply_price is not None and supply_price > 0
                else Decimal("0")
            )
            positions.append(
                PositionInfo(
                    position_type=PositionType.SUPPLY,
                    position_id=f"morpho-blue-supply-{self.supply_token}-{self.chain}",
                    chain=self.chain,
                    protocol="morpho_blue",
                    value_usd=value_usd,
                    details={
                        "asset": self.supply_token,
                        "amount": str(self._supplied_amount),
                        "market_id": self.market_id,
                    },
                )
            )

        return TeardownPositionSummary(
            deployment_id=getattr(self, "deployment_id", None) or "demo_morpho_blue_crisis_ethereum",
            timestamp=self._last_market_timestamp or datetime.now(UTC),
            positions=positions,
        )

    def generate_teardown_intents(self, mode: "TeardownMode", market=None) -> list[Intent]:
        """Withdraw supplied wstETH from Morpho Blue."""
        if self._supplied_amount <= 0:
            return []

        return [
            Intent.withdraw(
                protocol="morpho_blue",
                token=self.supply_token,
                amount=self._round_amount(self._supplied_amount),
                withdraw_all=True,
                market_id=self.market_id,
                chain=self.chain,
            )
        ]
