...
class PendleOnChainReader:
    ...
    def __init__(
        ...
        if gateway_client is not None:
            ...
            # Direct/web3 mode (HTTPProvider). After VIB-5348 ONE caller reaches this
            # branch (verifiable against source — references are file:symbol, grep them):
            #
            #   Strategy-container readers, ONLY when ``gateway_client is None``:
            ...
            #   This is the local-dev-without-gateway compile/health path.
            #
            # This consumer is UNREACHABLE from a hosted strategy container: the runner
            ...
            #
            # The GATEWAY itself NO LONGER reaches this branch (VIB-5348): its
            # ``GetPtPrice`` → ``_read_pt_market`` → ``_build_pt_reader`` now injects a
            # gateway-native eth_call transport
            # (``almanak/gateway/services/pt_rpc_adapter.GatewayPtRpcClient``) into the
            # reader's GATEWAY-mode seam, so the gateway constructs this reader with
            # ``gateway_client=...`` and ``web3 is None`` — no raw HTTPProvider on the
            # hosted perimeter. The direct branch below is therefore local-dev only.
            #
            # Final removal of this local-dev direct path is tracked by VIB-5348
            # (debt origin: VIB-2986).
            from web3 import Web3

            self.web3 = Web3(
                Web3.HTTPProvider(rpc_url, request_kwargs={"timeout": request_timeout_seconds})
            )  # vib-2986-exempt: local-dev fallback (gateway uses gateway-native transport), removal tracked by VIB-5348
