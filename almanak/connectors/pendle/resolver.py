"""Pendle market resolver: dynamic market discovery plus connector metadata.

Wraps PendleAPIClient with a clean query interface for agents and strategies.
Falls back to connector-owned static metadata when API is unavailable.
...
class PendleMarketResolver:
    ...
    Uses live API lookups first and connector-owned static metadata when the
    API is unavailable or does not expose a lookup shape the framework needs.
    ...
    def __init__(
        ...
        # Pre-process connector metadata for O(1) lookups.
    ...
    def resolve_by_pt_symbol(self, pt_symbol: str) -> PendleMarketData | None:
        ...
        to connector-owned market metadata.
        ...
        # Try matching via connector-owned PT metadata (address cross-reference).
        ...
        # Fall back to connector metadata -> API single-market fetch.
    ...
    def resolve_market_address_from_pt_symbol(self, pt_symbol: str) -> str | None:
        ...
        # Fall back to connector metadata.
    ...
    def resolve_mint_sy_token(self, market_address: str) -> str | None:
        ...
        """
        # Connector metadata takes priority: it captures yield-bearing token
        # quirks that the API's underlying_address field may not reflect.
    ...
    def _get_markets(self) -> list[PendleMarketData]:
        ...
        try:
            ...
            logger.warning(f"PendleMarketResolver: API unavailable ({e}), using cached/connector data")
    ...
    def _fallback_resolve_pt(self, pt_symbol: str) -> PendleMarketData | None:
        """Fall back to connector metadata for PT resolution."""
        ...
        try:
            ...
            logger.info(f"PendleMarketResolver: resolved {pt_symbol} via connector metadata + API fetch")
            return market
        except PendleAPIError:
            # Build minimal PendleMarketData from connector metadata.
            pt_info = self._static_pt_lookup(pt_symbol)
            logger.warning(f"PendleMarketResolver: falling back to connector metadata for {pt_symbol} on {self._chain}")
    ...
    def _preprocess_static_yt_info(self) -> dict[str, tuple[str, int]]:
        return {
            token.symbol.lower(): (token.address, token.decimals)
            for token in _pendle_metadata().synthetic_tokens()
            if token.chain == self._chain and token.family == "YT"
        }

    def _preprocess_static_market_by_pt(self) -> dict[str, str]:
        return {
            market.token_symbol.lower(): market.market_address
            for market in _pendle_metadata().market_tokens()
            if market.chain == self._chain and market.family == "PT"
        }

    def _preprocess_static_mint_sy(self) -> dict[str, str]:
        return {
            mint.market_address.lower(): mint.mint_token_address
            for mint in _pendle_metadata().market_mint_tokens()
            if mint.chain == self._chain
        }


def _pendle_metadata():
    """Return the connector-owned Pendle metadata capability."""
    from almanak.connectors._strategy_protocol_metadata_registry import (
        PROTOCOL_METADATA_REGISTRY,
    )

    metadata = PROTOCOL_METADATA_REGISTRY.lookup("pendle")
    if metadata is None:
        raise RuntimeError("Pendle protocol metadata provider is not registered")
    return metadata
