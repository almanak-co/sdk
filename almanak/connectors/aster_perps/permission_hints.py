"""Aster Perps permission hints for permission discovery.

Aster Diamond on BSC accepts native BNB as margin collateral in addition to
ERC-20 (USDT/USDC). Two distinct selectors:
...
open at ``execTransactionWithRole``. Aster owns its discovery vectors via
``build_discovery_vectors`` below so the BSC native-collateral synthetic is
declared next to the adapter that produces the selectors — when BSC adds
another native-collateral surface, or another chain is added with a different
gas token, the change lives here rather than in framework-side
``synthetic_intents`` chain branches.

PERP_CLOSE on Aster compiles ``closeTrade(bytes32)`` (selector ``0x5177fd3b``)
and requires ``intent.position_id`` to be a 0x-prefixed bytes32 tradeHash.
A placeholder hash satisfies the compiler's shape validation; the manifest
target is the Diamond address, not the trade-specific hash.

PancakeSwap Perps is broker_id=2 on the same Aster Diamond — its
``permission_hints.py`` carries an identical ``build_discovery_vectors`` so
each connector pins its own BSC invariant. Duplication is correct: if either
...
def build_discovery_vectors(
    ...
    Returns ``None`` for non-BSC chains (Aster Phase 1 is BSC-only — see
    ``adapter.py``) and for non-perp intent types so the framework default
    takes over.
    """
