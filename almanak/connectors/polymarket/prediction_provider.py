"""Polymarket prediction-market data provider (VIB-4989).

Relocated verbatim from ``almanak/framework/data/prediction_provider.py`` (the
``PredictionMarketDataProvider`` class + its ``CacheEntry`` helper) so the
Polymarket CLOB implementation lives in the connector folder, not the framework.
The venue-neutral result dataclasses (``PredictionMarket`` / ``PredictionPosition``
/ …) stay in the framework as the seam's public interface and are imported below
(connector→framework is allowed). The framework consumes this provider through the
duck-typed ``PredictionProvider`` Protocol via ``PredictionReadRegistry``
(VIB-4989, epic VIB-4851); the published spec lives in ``prediction_read.py``.

Behaviour is byte-identical to the framework original — pinned method-by-method by
