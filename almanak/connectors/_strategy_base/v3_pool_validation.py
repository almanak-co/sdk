"""Uniswap V3-family pool-existence validation and slot0() reads (foundation).

A single ``getPool(address,address,uint24)`` validator and ``slot0()`` reader
for the whole :class:`~almanak.connectors._strategy_base.address_registry.AbiFamily.V3_FACTORY`
family -- ``uniswap_v3`` and its forks (``sushiswap_v3``, ``pancakeswap_v3``,
``aerodrome`` slipstream, ``agni_finance``) all expose the same factory and
pool ABI. The per-fork factory address is resolved through
``AddressRegistry.resolve_contract_address`` rather than hardcoded here, so the
forks share this one validator instead of importing it from each other.

Lives in the connector foundation (not in any single connector) so that the V3
forks consume it from here without a cross-connector import -- deleting
``uniswap_v3`` must not strand the shared validator in ``aerodrome``. Imports
only ``_strategy_base`` primitives; the ``GatewayClient`` reference is
type-checking-only.
"""
