"""Connector-owned compiler for Ethena staking intents."""
...
from .adapter import EthenaAdapter, EthenaConfig

logger = logging.getLogger(__name__)


class EthenaCompiler(BaseStakingCompiler):
    """Compile Ethena STAKE and UNSTAKE intents."""

    protocols: ClassVar[frozenset[str]] = frozenset({"ethena"})
    ...
    def compile_stake(self, ctx: BaseCompilerContext, intent: StakeIntent) -> CompilationResult:
        try:
            action_bundle = EthenaAdapter(
                EthenaConfig(chain=ctx.chain, wallet_address=ctx.wallet_address),
            ...
            if result.status == CompilationStatus.SUCCESS:
                logger.info(
                    "Compiled STAKE intent: %s %s via ethena, %s txs, %s gas",
                    intent.amount,
                    intent.token_in,
            ...
            logger.exception("Failed to compile Ethena STAKE intent: %s", exc)
    ...
    def compile_unstake(self, ctx: BaseCompilerContext, intent: UnstakeIntent) -> CompilationResult:
        try:
            ...
            if result.status == CompilationStatus.SUCCESS:
                logger.info(
                    "Compiled UNSTAKE intent: %s %s via ethena, %s txs, %s gas",
                    intent.amount,
                    intent.token_in,
                    len(result.transactions),
                    result.total_gas_estimate,
            ...
            logger.exception("Failed to compile Ethena UNSTAKE intent: %s", exc)
            return CompilationResult(status=CompilationStatus.FAILED, error=str(exc), intent_id=intent.intent_id)


__all__ = ["EthenaCompiler"]
