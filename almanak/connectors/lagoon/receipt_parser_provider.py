"""Strategy-side receipt-parser connector for Lagoon (VIB-4854 / W2)."""

from __future__ import annotations

from typing import ClassVar

from almanak.connectors._base.types import ProtocolKind, ProtocolName
from almanak.connectors._strategy_base.receipt_parser_registry import (
    ReceiptParserCapability,
    ReceiptParserConnector,
)


class LagoonReceiptParserConnector(ReceiptParserConnector, ReceiptParserCapability):
    protocol: ClassVar[ProtocolName] = ProtocolName("lagoon")
    kind: ClassVar[ProtocolKind] = ProtocolKind.VAULT

    def receipt_parser_keys(self) -> frozenset[str]:
        return frozenset({"lagoon"})

    def receipt_parser_class(self, key: str) -> type:
        from almanak.connectors.lagoon.receipt_parser import (
            LagoonReceiptParser,
        )

        return LagoonReceiptParser


__all__ = ["LagoonReceiptParserConnector"]
