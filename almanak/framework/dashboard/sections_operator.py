...
``deployment_id`` so two operator tabs (different strategies) don't trample
...
# Session-state keys — namespaced by deployment_id so different tabs don't collide.
_PREVIEW_TOKEN_KEY = "phase1_reconcile_preview_token::{deployment_id}"
_PREVIEW_RESULT_KEY = "phase1_reconcile_preview_result::{deployment_id}"
_APPLY_OUTCOME_KEY = "phase1_reconcile_apply_outcome::{deployment_id}"
_REFRESH_OUTCOME_KEY = "phase1_reconcile_refresh_outcome::{deployment_id}"


def _key(template: str, deployment_id: str) -> str:
    return template.format(deployment_id=deployment_id)
...
def render_reconciliation_operator_panel(
    ...
    *,
    ...
    with col_preview:
        preview_clicked = st.button("Preview reconcile", key=f"phase1_preview_btn_{deployment_id}")
    with col_apply:
        # Apply button is enabled iff a preview token exists.
        has_token = bool(st.session_state.get(_key(_PREVIEW_TOKEN_KEY, deployment_id)))
        apply_clicked = st.button(
            "Apply reconcile",
            key=f"phase1_apply_btn_{deployment_id}",
    ...
    with col_refresh:
        refresh_clicked = st.button(
            ...
            key=f"phase1_refresh_btn_{deployment_id}",
            help="Force a fresh on-chain read pass for this strategy.",
        )

    if preview_clicked:
        _do_preview(deployment_id, client)
    if apply_clicked:
        _do_apply(deployment_id, client)
    if refresh_clicked:
        _do_refresh(deployment_id, client)

    _render_preview_result(deployment_id)
    _render_apply_outcome(deployment_id)
    _render_refresh_outcome(deployment_id)
...
def _do_preview(deployment_id: str, client: OperatorDashboardServiceClient) -> None:
    try:
        result = client.preview_reconcile(deployment_id)
    except DashboardClientError as exc:
        _surface_reconcile_error("PreviewReconcile", exc)
        return
    st.session_state[_key(_PREVIEW_TOKEN_KEY, deployment_id)] = result.preview_token
    st.session_state[_key(_PREVIEW_RESULT_KEY, deployment_id)] = result
    # Clear any stale apply outcome from a previous preview.
    st.session_state.pop(_key(_APPLY_OUTCOME_KEY, deployment_id), None)


def _do_apply(deployment_id: str, client: OperatorDashboardServiceClient) -> None:
    token = st.session_state.get(_key(_PREVIEW_TOKEN_KEY, deployment_id))
    if not token:
        st.error("No preview token in session — click Preview first.")
        return
    try:
        outcome = client.apply_reconcile(deployment_id, token)
    except DashboardClientError as exc:
        _surface_reconcile_error("ApplyReconcile", exc)
        return
    st.session_state[_key(_APPLY_OUTCOME_KEY, deployment_id)] = outcome
    ...
    st.session_state.pop(_key(_PREVIEW_TOKEN_KEY, deployment_id), None)


def _do_refresh(deployment_id: str, client: OperatorDashboardServiceClient) -> None:
    try:
        outcome = client.refresh_registry_from_chain(deployment_id)
    except DashboardClientError as exc:
        _surface_reconcile_error("RefreshRegistryFromChain", exc)
        return
    st.session_state[_key(_REFRESH_OUTCOME_KEY, deployment_id)] = outcome
...
def _render_preview_result(deployment_id: str) -> None:
    result = st.session_state.get(_key(_PREVIEW_RESULT_KEY, deployment_id))
    if result is None:
        return
...
def _render_apply_outcome(deployment_id: str) -> None:
    outcome = st.session_state.get(_key(_APPLY_OUTCOME_KEY, deployment_id))
    if outcome is None:
        return
...
def _render_refresh_outcome(deployment_id: str) -> None:
    outcome = st.session_state.get(_key(_REFRESH_OUTCOME_KEY, deployment_id))
    if outcome is None:
        return
