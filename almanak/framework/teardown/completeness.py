...
def _covers_token(intent: Any, position: PositionInfo, itype: str) -> bool:
    # Known limitation (VIB-5469 follow-up): ``full_close`` emits NO intent for a
    # held TOKEN/STAKE position whose token already equals the consolidation
    # target (nothing to swap) — that position would be flagged uncovered here.
    # The skip is not modelled because the consolidation target is resolved in a
    # later phase (after this check), so this check has no target context. Effect
    # is fail-SAFE (a FAILED teardown, never a fund-loss strand) and latent (no
    # shipped strategy surfaces the target token as a TOKEN position). Fixing it
    # cleanly needs the target threaded in — tracked as VIB-5494.
    #
...
def check_intent_coverage(
    ...
    uncovered = [p for p in enforceable if not any(_covers(i, p) for i in intent_list)]
...
__all__ = ["CompletenessReport", "check_intent_coverage"]
