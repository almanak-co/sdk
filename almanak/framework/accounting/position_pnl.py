...
def compute_position_pnl(events: list[dict]) -> PositionPnLSummary | None:
    ...
    Cost basis rules:
      ...
      WITHDRAW / REPAY → -= principal_delta_usd  (capital returned or repaid)
      None values are skipped — UNAVAILABLE amounts are never treated as zero.

    Realized PnL rules:
      REPAY    → -= interest_delta_usd  (interest paid is a realized cost to the borrower)
    ...
    for ev in sorted_events:
        ...
        if principal_raw is not None:
            try:
                ...
                elif event_type in ("WITHDRAW", "REPAY"):
                    cost_basis -= principal
            except InvalidOperation:
                pass

        if interest_raw is not None and event_type in ("REPAY", "WITHDRAW"):
            try:
                interest = Decimal(str(interest_raw))
                if event_type == "REPAY":
                    realized_pnl -= interest  # interest paid is a cost to the borrower
                else:
