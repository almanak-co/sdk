...
class AccountingProcessor:
    ...
    async def drain_one_direct(
        ...
        *,
        ...
        """Process one accounting event directly without writing to the outbox DB.

        Fallback path used when the gateway outbox is not yet available (VIB-3482).
        Builds a synthetic in-memory outbox row, reads the ledger row, dispatches
        to the appropriate category handler, and writes the event via
        save_accounting_event(). Mirrors drain_one's logic minus the DB-side
        outbox bookkeeping.

        Returns True if an event was written (or there was nothing to write),
        ...
        try:
            ...
            if ledger_row is None:
                logger.warning(
                    "drain_one_direct: ledger row not found for %s — accounting event dropped "
                    "(VIB-3482: gateway get_ledger_entry_by_id not yet implemented)",
                    ledger_entry_id,
                )
                return False

            synthetic_outbox_row: dict[str, Any] = {
                "id": str(uuid.uuid4()),
            ...
            event = self._dispatch(synthetic_outbox_row, ledger_row)
            if event is not None:
                ok = await self._writer.write(event)
                if not ok:
                    logger.warning(
                        "drain_one_direct: writer.write returned False for %s — accounting event may be lost",
                        ledger_entry_id,
                    )
                    return False
            return True
        except Exception:
            logger.warning("drain_one_direct failed for %s", ledger_entry_id, exc_info=True)
            return False
