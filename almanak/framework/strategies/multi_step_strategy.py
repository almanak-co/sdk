...
class MultiStepStrategy(IntentStrategy):
    ...
    def decide(self, market: MarketSnapshot) -> DecideResult:
        ...
        try:
            ...
            logger.error(f"Unknown state '{self._ms_state}', reverting to '{self._ms_previous_stable}'")
            self._ms_state = self._ms_previous_stable
            return Intent.hold(reason=f"Recovered from unknown state '{self._ms_state}'")
    ...
    def _start_step(self, step_name: str, market: MarketSnapshot) -> AnyIntent:
        ...
        if intent is None:
            logger.warning(f"intent_for_step('{step_name}') returned None, reverting to '{self._ms_previous_stable}'")
            self._ms_state = self._ms_previous_stable
