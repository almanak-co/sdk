...
This module provides a historical volume data provider for SushiSwap V3 pools
across supported chains. It implements the HistoricalVolumeProvider interface
and fetches data from The Graph's SushiSwap V3 subgraphs.

Key Features:
    - Supports Ethereum chain (Arbitrum pending subgraph deployment)
...
# =============================================================================
# SushiSwap V3 Subgraph IDs (from The Graph Explorer)
# =============================================================================

# Subgraph deployment IDs for SushiSwap V3 on various chains
# These are from The Graph's decentralized network
# Note: Arbitrum subgraph not yet available on decentralized network
SUSHISWAP_V3_SUBGRAPH_IDS: dict[Chain, str] = {
    Chain.ETHEREUM: "2tGWMrDha4164KkFAfkU3rDCtuxGb4q1emXmFdLLzJ8x",
    # Arbitrum subgraph pending deployment to The Graph decentralized network
    # Chain.ARBITRUM: "TBD",
}
...
# SushiSwap V3 uses the same schema as Uniswap V3 for poolDayDatas
POOL_DAY_DATA_QUERY = """
...
class SushiSwapV3VolumeProvider(HistoricalVolumeProvider):
    """Historical volume provider for SushiSwap V3 pools.

    Fetches daily volume data from The Graph's SushiSwap V3 subgraphs.
    Currently supports Ethereum chain, with Arbitrum support pending
    ...
    def __init__(
        ...
        logger.debug(
            "Initialized SushiSwapV3VolumeProvider: supported_chains=%s, fallback_volume=%s",
    ...
    def _get_subgraph_id(self, chain: Chain) -> str | None:
        ...
        """
        return SUSHISWAP_V3_SUBGRAPH_IDS.get(chain)
    ...
    async def get_volume(
        ...
        """Fetch historical volume data for a SushiSwap V3 pool.

        Queries The Graph's SushiSwap V3 subgraph for daily volume data
        ...
        Args:
            ...
            chain: The blockchain the pool is on. Must be one of:
                   ETHEREUM (Arbitrum support pending).
        ...
        logger.info(
            "Fetching SushiSwap V3 volume: chain=%s, pool=%s..., start=%s, end=%s",
            chain.value,
        ...
        try:
            ...
            logger.info(
                "Fetched %d days of SushiSwap V3 volume: chain=%s, pool=%s...",
                len(results),
                chain.value,
                pool_address_lower[:10],
            )
