...
client) has moved into the gateway sidecar — the TraderJoe V2 connector's
:class:`GatewayDexVolumeCapability` owns the Liquidity-Book
``lbPairDayDatas`` subgraph query (``volumeUSD`` keyed by ``lbPair``). The
strategy container holds no subgraph URLs, no API key, and opens no
socket.

The :class:`TraderJoeV2VolumeProvider` public API, the
``TRADERJOE_V2_SUBGRAPH_IDS`` / ``SUPPORTED_CHAINS`` tables and the
...
# =============================================================================
# TraderJoe V2 Subgraph IDs (preserved for back-compat)
# =============================================================================
#
# The authoritative copy now lives on the TraderJoe V2 connector's
# ``GatewayDexVolumeCapability``. Arbitrum support is on the hosted
# service but not the decentralized network (pending), so Avalanche is the
# only chain here — preserved verbatim from pre-W7.
...
_GATEWAY_DEX = "traderjoe_v2"


# =============================================================================
# TraderJoeV2VolumeProvider (thin gRPC client — VIB-4859 / W7)
# =============================================================================


class TraderJoeV2VolumeProvider(HistoricalVolumeProvider):
    """Historical volume provider for TraderJoe V2 LB pools — gateway-backed.

    Issues a ``GetDexVolumeHistory`` RPC for daily volume on Avalanche.
    ...
    def __init__(
        ...
        logger.debug(
            "Initialized TraderJoeV2VolumeProvider (gateway-backed): supported_chains=%s",
    ...
    async def get_volume(
        ...
        """Fetch historical daily volume for a TraderJoe V2 LB pool via the gateway.
        ...
        """
        if chain not in TRADERJOE_V2_SUBGRAPH_IDS:
