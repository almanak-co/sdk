"""Paper-local token override registry (VIB-2378).

Loads custom token metadata from ``~/.config/almanak/paper_trading_tokens.json``
for use during paper trading wallet funding. This avoids contaminating the
    ...
    for symbol, value in chain_data.items():
        if isinstance(value, str):
            ...
            if decimals is not None and (not isinstance(decimals, int) or isinstance(decimals, bool)):
                logger.warning("[paper-trading] Token override '%s' has non-int decimals, ignoring", symbol)
                decimals = None
