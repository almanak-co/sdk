...
def generate_manifest(
    ...
    3. Infrastructure - MultiSend (always), Enso delegates (if enso protocol)
...
def _build_infrastructure_permissions(
    ...
    - Enso delegates: included only when "enso" is in protocols
    """
    ...
    # Enso delegates (DELEGATECALL) - only when enso protocol is used
    if any(p.lower() == "enso" for p in protocols):
        for delegate_addr in sorted(ENSO_DELEGATE_ADDRESSES):
            permissions.append(
                ContractPermission(
                    target=delegate_addr.lower(),
                    label="Enso Delegate",
                    operation=SafeOperation.DELEGATE_CALL,
                    send_allowed=False,
                    function_selectors=[],  # Enso delegates use arbitrary calldata
                )
            )
