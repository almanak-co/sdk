"""Aster Perps receipt parser.

Decodes the events emitted by the Aster Diamond on BSC along the open/close
lifecycle:
...
class AsterPerpsReceiptParser:
    ...
    # ``leverage`` is listed so ResultEnricher does not flag every successful OPEN with an
    # "unsupported field" warning (ResultEnricher.EXTRACTION_SPECS["PERP_OPEN"] requests it).
    # There is no ``extract_leverage`` method: the v1 MarketPendingTrade event does not carry
    # a leverage_bps field, so the enricher silently skips it (hasattr check).
    ...
    def __init__(self, chain: str = "bsc", **_: Any) -> None:
        ...
        Aster Perps is BSC-only in Phase 1; the chain argument is accepted for
    ...
    def parse_receipt(self, receipt: dict[str, Any]) -> ParsedReceipt:
        ...
        Safe to call on receipts with no Aster-Perps events (returns an empty
        ...
        for log in receipt.get("logs", []) or []:
            ...
            try:
                ...
                logger.warning(
                    "AsterPerpsReceiptParser: failed to decode log topic0=%s: %s",
                    topic0,
                    e,
                )
        return parsed
    ...
    def _normalize_log(log: dict[str, Any]) -> tuple[list[str], str]:
        topics = log.get("topics") or []
        topics_norm = [AsterPerpsReceiptParser._to_hex_str(t) for t in topics]
        data = AsterPerpsReceiptParser._to_hex_str(log.get("data") or "0x")
        return topics_norm, data

    @staticmethod
    def _decode_log_index(value: Any) -> int:
        """Decode a logIndex value that may be a decimal int, a hex string, or None.

        Web3.py returns ``logIndex`` as int; raw JSON-RPC responses (and many test
        fixtures) return it hex-encoded (e.g. ``"0x0"``). ``int("0x0")`` raises,
        so the naive cast would get silently swallowed by the parse_receipt
        exception handler, making the whole event vanish.
        """
        if value is None:
            return 0
        if isinstance(value, str):
            return int(value, 0) if value else 0  # base=0 auto-detects 0x prefix
        return int(value)
    ...
    def _decode_market_pending_trade(log: dict[str, Any]) -> MarketPendingTradeEvent | None:
        ...
        """
        topics, data = AsterPerpsReceiptParser._normalize_log(log)
        if len(topics) < 3:
            return None
        ...
        return MarketPendingTradeEvent(
            ...
            log_index=AsterPerpsReceiptParser._decode_log_index(log.get("logIndex")),
        )
