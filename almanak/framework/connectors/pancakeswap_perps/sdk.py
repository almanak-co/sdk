...
# Broker id constant (re-exported from core.contracts).
PCS_BROKER_ID: int = PANCAKESWAP_PERPS_BROKER_ID
...
class OpenTradeStruct:
    """Python mirror of the ApolloX openMarketTrade input struct.
    ...
    broker: int = PCS_BROKER_ID  # uint24
...
def get_router_address(chain: str = "bsc") -> str:
    """Return the PancakeSwap Perps router address for the given chain."""
    entry = PANCAKESWAP_PERPS.get(chain)
    if not entry:
        raise ValueError(f"PancakeSwap Perps not configured for chain '{chain}'")
    return entry["router"]
...
def get_pair_base(market: str, chain: str = "bsc") -> str:
    ...
    markets = PANCAKESWAP_PERPS_MARKETS.get(chain, {})
    if market not in markets:
        raise ValueError(
            f"Market '{market}' not registered for PancakeSwap Perps on '{chain}'. "
            f"Supported (v1): {sorted(markets.keys())}"
        )
...
def get_margin_token_address(
    ...
    :data:`PANCAKESWAP_PERPS_TOKENS` allowlist is only consulted to reject
    tokens that the ApolloX router does not accept as margin; the *address*
    ...
    Raises:
        ValueError: If the symbol is not one of PCS Perps' supported margin tokens.
    ...
    tokens = PANCAKESWAP_PERPS_TOKENS.get(chain, {})
    if up not in tokens:
        raise ValueError(
            f"Margin token '{symbol}' not registered for PancakeSwap Perps on '{chain}'. "
