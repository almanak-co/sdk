...
class PancakeSwapPerpsConfig:
    """Minimal connector config.

    Attributes:
        chain: chain key (v1 = 'bsc').
        ...
        broker_id: broker attribution (default = PCS = 2).
    """

    chain: str = "bsc"
    wallet_address: str | None = None
    broker_id: int = PCS_BROKER_ID


@dataclass
class PancakeSwapPerpsTx:
...
class PerpOpenOrderResult:
    ...
    success: bool
    error: str | None = None
    tx: PancakeSwapPerpsTx | None = None
...
class PancakeSwapPerpsAdapter:
    """Translate PERP_OPEN / PERP_CLOSE intents into transaction data."""

    def __init__(self, config: PancakeSwapPerpsConfig) -> None:
        if config.chain != "bsc":
            raise ValueError(f"PancakeSwap Perps v1 supports chain='bsc' only, got '{config.chain}'")
        self.config = config
        ...
        *,
        ...
        Args:
            market: market symbol (e.g. 'BTC/USD'). Must be registered in
                almanak.core.contracts.PANCAKESWAP_PERPS_MARKETS[chain].
        ...
        tx = PancakeSwapPerpsTx(
            to=self.router,
            value=tx_value,
            data=calldata,
            gas_estimate=gas,
            description=(
                f"PancakeSwapPerps {'LONG' if is_long else 'SHORT'} {market} "
    ...
    def build_close(self, *, trade_hash: str | bytes) -> PancakeSwapPerpsTx:
        ...
        on (market, is_long, collateral_token) which is insufficient for ApolloX
        ...
        return PancakeSwapPerpsTx(
            to=self.router,
            value=0,
            data=calldata,
            gas_estimate=GAS_CLOSE_TRADE,
            description=f"PancakeSwapPerps closeTrade({th_hex})",
        )
...
def build_open_transaction(
    ...
    **open_kwargs: Any,
    ...
    adapter = PancakeSwapPerpsAdapter(
        PancakeSwapPerpsConfig(chain=chain, wallet_address=wallet_address, broker_id=broker_id)
    )
