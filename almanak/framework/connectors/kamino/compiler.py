"""Connector-owned compiler for kamino (Solana lending).
...
class KaminoCompiler(BaseLendingCompiler):
    """Compile kamino lending intents (Solana)."""
    ...
    def compile_supply(self, ctx: BaseCompilerContext, intent: SupplyIntent) -> CompilationResult:
        from almanak.framework.intents.compiler_solana import compile_kamino_supply

        return compile_kamino_supply(_SolanaCompilerAdapter(ctx), intent)

    def compile_withdraw(self, ctx: BaseCompilerContext, intent: WithdrawIntent) -> CompilationResult:
        from almanak.framework.intents.compiler_solana import compile_kamino_withdraw

        return compile_kamino_withdraw(_SolanaCompilerAdapter(ctx), intent)

    def compile_borrow(self, ctx: BaseCompilerContext, intent: BorrowIntent) -> CompilationResult:
        from almanak.framework.intents.compiler_solana import compile_kamino_borrow

        return compile_kamino_borrow(_SolanaCompilerAdapter(ctx), intent)

    def compile_repay(self, ctx: BaseCompilerContext, intent: RepayIntent) -> CompilationResult:
        from almanak.framework.intents.compiler_solana import compile_kamino_repay

        return compile_kamino_repay(_SolanaCompilerAdapter(ctx), intent)
