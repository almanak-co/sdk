"""Joe Lend (Banker Joe) Receipt Parser.

Parses transaction receipts for Joe Lend lending operations (Compound V2 architecture).
Uses base infrastructure utilities for hex decoding and event mapping.

Key events:
- Mint: User supplies underlying, receives jTokens
- Redeem: User redeems jTokens for underlying
...
EVENT_TOPICS: dict[str, str] = {
    # Core lending events (identical to all Compound V2 forks)
...
EVENT_NAME_TO_TYPE: dict[str, JoeLendEventType] = {
    "Mint": JoeLendEventType.MINT,
    "Redeem": JoeLendEventType.REDEEM,
    "Borrow": JoeLendEventType.BORROW,
    "RepayBorrow": JoeLendEventType.REPAY_BORROW,
    "LiquidateBorrow": JoeLendEventType.LIQUIDATE_BORROW,
    "Transfer": JoeLendEventType.TRANSFER,
    "Approval": JoeLendEventType.APPROVAL,
}
...
class JoeLendEvent:
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> "JoeLendEvent":
        """Create from dictionary."""
        return cls(
            event_type=JoeLendEventType(data["event_type"]),
...
class ParseResult:
    ...
    success: bool
    events: list[JoeLendEvent] = field(default_factory=list)
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "j_tokens_minted": str(self.j_tokens_minted),
            "j_tokens_redeemed": str(self.j_tokens_redeemed),
            "error": self.error,
        }
...
class JoeLendReceiptParser:
    ...
    def parse_receipt(
        ...
        Args:
            ...
            j_token_address: Optional filter for specific jToken
        ...
        # Filter to specific jToken if provided
        if j_token_address:
            j_lower = j_token_address.lower()
            events = [e for e in events if e.contract_address.lower() == j_lower]

        # Aggregate amounts
        result = ParseResult(success=True, events=events)
        for event in events:
            if event.event_type == JoeLendEventType.MINT:
                result.supply_amount += Decimal(str(event.data.get("mint_amount", 0)))
                result.j_tokens_minted += Decimal(str(event.data.get("mint_tokens", 0)))
            elif event.event_type == JoeLendEventType.REDEEM:
                result.withdraw_amount += Decimal(str(event.data.get("redeem_amount", 0)))
                result.j_tokens_redeemed += Decimal(str(event.data.get("redeem_tokens", 0)))
            elif event.event_type == JoeLendEventType.BORROW:
                result.borrow_amount += Decimal(str(event.data.get("borrow_amount", 0)))
            elif event.event_type == JoeLendEventType.REPAY_BORROW:
    ...
    def parse_logs(
        ...
        for log in logs:
            ...
            event_type = EVENT_NAME_TO_TYPE.get(event_name, JoeLendEventType.UNKNOWN)
