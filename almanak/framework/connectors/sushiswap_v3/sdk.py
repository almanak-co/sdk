"""SushiSwap V3 SDK Core Functions.

This module provides essential SDK functions for SushiSwap V3 operations:
...
SushiSwap V3 is a fork of Uniswap V3, using the same concentrated liquidity model
with identical function signatures and event formats.
...
Example:
    from almanak.framework.connectors.sushiswap_v3.sdk import SushiSwapV3SDK
...
# SushiSwap V3 constants (same as Uniswap V3)
Q96 = 2**96
Q128 = 2**128
MAX_UINT128 = 2**128 - 1
MAX_UINT256 = 2**256 - 1
...
FACTORY_ADDRESSES: dict[str, str] = {chain: addrs["factory"] for chain, addrs in SUSHISWAP_V3.items()}

# SwapRouter addresses per chain (Uniswap V3 style SwapRouter)
# Note: SushiSwap also has RouteProcessor contracts for more complex routing
ROUTER_ADDRESSES: dict[str, str] = {chain: addrs["swap_router"] for chain, addrs in SUSHISWAP_V3.items()}

# QuoterV2 addresses per chain
QUOTER_ADDRESSES: dict[str, str] = {chain: addrs["quoter_v2"] for chain, addrs in SUSHISWAP_V3.items()}

# NonfungiblePositionManager addresses per chain
POSITION_MANAGER_ADDRESSES: dict[str, str] = {chain: addrs["position_manager"] for chain, addrs in SUSHISWAP_V3.items()}
...
# Position Manager function selectors
MINT_SELECTOR = (
    "0x88316456"  # mint((address,address,uint24,int24,int24,uint256,uint256,uint256,uint256,address,uint256))
)
INCREASE_LIQUIDITY_SELECTOR = "0x219f5d17"
DECREASE_LIQUIDITY_SELECTOR = "0x0c49ccbe"
...
class SushiSwapV3SDKError(Exception):
    """Base exception for SushiSwap V3 SDK errors."""

    pass


class InvalidFeeError(SushiSwapV3SDKError):
...
class InvalidTickError(SushiSwapV3SDKError):
...
class PoolNotFoundError(SushiSwapV3SDKError):
    """Pool does not exist."""
...
class QuoteError(SushiSwapV3SDKError):
...
class MintParams:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "amount0_desired": str(self.amount0_desired),
            "amount1_desired": str(self.amount1_desired),
            "amount0_min": str(self.amount0_min),
            "amount1_min": str(self.amount1_min),
...
class LPTransaction:
    ...
    Attributes:
        to: Position manager address
        value: ETH value to send
        data: Encoded calldata
        gas_estimate: Estimated gas
        description: Human-readable description
        operation: LP operation type (mint, increase, decrease, collect)
    """
...
def compute_pool_address(
    ...
    Args:
        ...
        token1: Second token address
        fee: Fee tier
        init_code_hash: Pool init code hash (default for SushiSwap V3)
    ...
    # Sort tokens (V3 pools always order token0 < token1)
...
# =============================================================================
# SushiSwap V3 SDK Class
# =============================================================================


class SushiSwapV3SDK:
    ...
    def __init__(
        ...
        logger.info(f"SushiSwapV3SDK initialized for chain={chain}")
    ...
    # =========================================================================
    # LP Position Transaction Building
    # =========================================================================
    ...
    def build_mint_tx(
        ...
        Args:
            ...
            value: ETH value to send (if one token is WETH and user wants to use ETH)

        Returns:
            LPTransaction with encoded calldata
        """
        calldata = self._encode_mint(params)

        description = (
            f"Mint LP position in {params.token0[:10]}.../{params.token1[:10]}... pool "
        ...
        return LPTransaction(
            ...
            value=value,
            data=calldata,
            gas_estimate=500000,  # LP operations are gas-intensive
            description=description,
            operation="mint",
        )

    def build_increase_liquidity_tx(
        self,
        token_id: int,
        amount0_desired: int,
        amount1_desired: int,
        ...
        Returns:
            LPTransaction with encoded calldata
        """
        calldata = self._encode_increase_liquidity(
            token_id=token_id,
        ...
        description = f"Increase liquidity for position #{token_id}"
    ...
    def build_decrease_liquidity_tx(
        ...
        Returns:
            LPTransaction with encoded calldata
        """
        calldata = self._encode_decrease_liquidity(
            token_id=token_id,
            liquidity=liquidity,
            amount0_min=amount0_min,
            amount1_min=amount1_min,
        ...
        description = f"Decrease liquidity for position #{token_id} by {liquidity}"
    ...
    def build_collect_tx(
        ...
        Args:
            token_id: NFT token ID of the position
            recipient: Address to receive collected tokens
            amount0_max: Maximum amount of token0 to collect (default: all)
            amount1_max: Maximum amount of token1 to collect (default: all)

        Returns:
            LPTransaction with encoded calldata
        """
        calldata = self._encode_collect(
            token_id=token_id,
            recipient=recipient,
        ...
        description = f"Collect fees/tokens from position #{token_id}"
    ...
    def _encode_mint(self, params: MintParams) -> str:
        """Encode mint calldata for NonfungiblePositionManager."""
        # Sort tokens for the pool
        token0, token1 = sort_tokens(params.token0, params.token1)
        amount0, amount1 = params.amount0_desired, params.amount1_desired
        amount0_min, amount1_min = params.amount0_min, params.amount1_min

        # If tokens were swapped, swap amounts too
        if token0.lower() != params.token0.lower():
        ...
        return (
            ...
            + self._pad_uint(params.fee)
            + self._pad_int(params.tick_lower)
            + self._pad_int(params.tick_upper)
            + self._pad_uint(amount0)
            + self._pad_uint(amount1)
            + self._pad_uint(amount0_min)
            + self._pad_uint(amount1_min)
            + self._pad_address(params.recipient)
            + self._pad_uint(params.deadline)
        )
    ...
    def _encode_increase_liquidity(
        ...
        return (
            INCREASE_LIQUIDITY_SELECTOR
            + self._pad_uint(token_id)
            + self._pad_uint(amount0_desired)
            + self._pad_uint(amount1_desired)
            + self._pad_uint(amount0_min)
            + self._pad_uint(amount1_min)
            + self._pad_uint(deadline)
        )
    ...
    def _encode_decrease_liquidity(
        ...
        """Encode decreaseLiquidity calldata."""
        return (
            DECREASE_LIQUIDITY_SELECTOR
            + self._pad_uint(token_id)
            + self._pad_uint(liquidity)
            + self._pad_uint(amount0_min)
            + self._pad_uint(amount1_min)
            + self._pad_uint(deadline)
        )
    ...
    def _encode_collect(
        ...
        """Encode collect calldata."""
        return (
            COLLECT_SELECTOR
            + self._pad_uint(token_id)
            + self._pad_address(recipient)
            + self._pad_uint(amount0_max)
            + self._pad_uint(amount1_max)
        )
    ...
    @staticmethod
    def _pad_int(value: int) -> str:
        """Pad signed int to 32 bytes (two's complement)."""
        if value >= 0:
            return hex(value)[2:].zfill(64)
    ...
    async def _get_web3(self) -> Any:
        ...
        try:
            ...
            raise SushiSwapV3SDKError("web3 package required for RPC operations") from e
        ...
        if self.rpc_url is None:
            raise SushiSwapV3SDKError("No gateway_client, RPC URL, or Web3 instance provided")
