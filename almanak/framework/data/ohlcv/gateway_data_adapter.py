...
class CoinGeckoGatewayDataProvider:
    """Adapts GatewayCoinGeckoOHLCVProvider to the DataProvider protocol.

    Properties:
        name: Returns ``"coingecko"`` to match the router's provider chain key.
        data_class: INFORMATIONAL -- OHLCV is never execution-grade.
    """

    def __init__(self, gateway_provider: GatewayCoinGeckoOHLCVProvider) -> None:
        self._provider = gateway_provider
    ...
    def fetch(self, **kwargs: object) -> DataEnvelope:
        """Synchronous DataProvider entry point for CoinGecko OHLCV.
