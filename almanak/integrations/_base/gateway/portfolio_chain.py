...
from almanak.integrations._base.gateway.circuit_breaker import CircuitBreaker
...
def build_portfolio_chain(
    ...
    for cfg in configs:
        try:
            factory = INTEGRATION_REGISTRY.gateway_portfolio_provider_factory(cfg.name)
        except KeyError:
            logger.warning("Skipping unknown portfolio integration: %s", cfg.name)
            continue
        if factory.requires_api_key and not cfg.api_key:
        ...
        # Some providers (notably OKX) require a credential set richer than a
        # single API key. The provider owns that validation; never admit a
        # constructed-but-unconfigured client into the runtime failover chain.
