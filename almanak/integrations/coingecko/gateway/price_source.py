"""Gateway-only CoinGecko price source.
...
# Projected from typed chain-descriptor metadata (canonical chain names only).
COINGECKO_PLATFORM_IDS: Mapping[str, str] = MappingProxyType(integration_chain_map("coingecko"))
...
_CHAIN_TABLE_TOKEN_IDS: dict[str, str] = {
    # Native + wrapped-native coin ids are implemented by chain descriptors.
    # Keys are uppercased to
    # match get_price()'s symbol normalization. Placed first so any
    # chain-variant static row below still wins on overlap.
...
class CoinGeckoPriceSource(BasePriceSource):
    ...
    def _resolve_token_id(self, token: str) -> str | None:
        ...
        Resolution order:
        1. Chain-owned native/wrapped IDs and curated symbol mappings
        2. DEFAULT_TOKENS by symbol (uses Token.coingecko_id)
        ...
        """
        # Chain-owned native/wrapped IDs are authoritative for those assets;
        # curated provider rows likewise must not be overwritten by the broader
        # token registry (for example WMATIC -> the exact wrapped listing).
        explicit_id = GLOBAL_TOKEN_IDS.get(token)
        if explicit_id:
            return explicit_id

        # Fill genuine misses from DEFAULT_TOKENS (uses Token.coingecko_id).
        try:
