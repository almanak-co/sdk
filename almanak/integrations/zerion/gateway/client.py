"""Gateway-only Zerion wallet portfolio client.
...
class ZerionIntegration(BaseIntegration):
    ...
    async def get_wallet_positions(self, wallet_address: str, chain: str) -> WalletPortfolioSnapshot:
        ...
        if cached is not None:
            return dataclasses.replace(cached, positions=list(cached.positions), cache_hit=True)

        chain_id = integration_chain_id(chain, "zerion")
        if chain_id is None:
            chain_id = chain.lower()
            logger.warning("Zerion has no chain-id mapping for chain %r; falling back to %r", chain, chain_id)
