"""Gateway-only OKX OnchainOS portfolio client.
...
from almanak.integrations._base.gateway.base import BaseIntegration, IntegrationError, IntegrationRateLimitError
...
class OkxIntegration(BaseIntegration):
    ...
    # OKX-specific synthetic IDs (e.g. Solana = "501"). These live on each
    # chain's typed external-id declaration and resolve through ``integration_chain_id``.
    ...
    def _cache_address(wallet_address: str, chain: str) -> str:
        ...
        """
        if is_solana_chain(chain) or chain == integration_chain_id("solana", "okx"):
            return wallet_address
        return wallet_address.lower()

    @staticmethod
    def _resolve_chain_id(chain: str) -> str:
        """Resolve a canonical chain or preserve an explicit numeric OKX id."""
        chain_id = integration_chain_id(chain, "okx")
        if chain_id is not None:
            return chain_id
        if chain.isdecimal():
            return chain
        raise ValueError(f"OKX does not support chain {chain!r}")
