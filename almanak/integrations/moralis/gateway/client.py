"""Gateway-only Moralis wallet portfolio client.
...
class MoralisIntegration(BaseIntegration):
    ...
    _SOLANA_API_BASE = "https://solana-gateway.moralis.io"
    ...
    def _url_for_path(self, path: str) -> str:
        if path.startswith("/account/"):
            return f"{self._SOLANA_API_BASE}{path}"
        return super()._url_for_path(path)
    ...
    def _get_chain_slug(self, chain: str) -> str | None:
        ...
        (https://docs.moralis.io/supported-chains). Resolved from typed metadata
        implemented by each chain descriptor; alias-
        normalised so ``bnb`` -> ``bsc``, and Solana (no Moralis slug) -> None.
        """
        return integration_chain_id(chain, "moralis")
    ...
    async def _fetch_solana_portfolio(self, wallet_address: str, chain: str) -> WalletPortfolioSnapshot:
        """Fetch Solana token balances from Moralis's Solana API origin."""
        data = await self._fetch(
    ...
    def _normalize_evm_response(self, wallet_address: str, chain: str, data: Any) -> WalletPortfolioSnapshot:
        ...
        for item in items:
            ...
            decimals = self._parse_decimals(raw_decimals, symbol, token_address)
    ...
    def _normalize_solana_response(self, wallet_address: str, chain: str, data: Any) -> WalletPortfolioSnapshot:
        ...
        for item in items:
            ...
            mint = item.get("mint", "")
            raw_decimals = item.get("decimals")
            decimals = self._parse_decimals(raw_decimals, symbol, mint)
    ...
    @staticmethod
    def _parse_decimals(raw_decimals: Any, symbol: str, identifier: str) -> int | None:
        if raw_decimals is None:
            logger.warning("Moralis: missing decimals for %s (%s), skipping valuation", symbol, identifier)
            return None
        if isinstance(raw_decimals, bool):
            logger.warning(
                "Moralis: invalid decimals %r for %s (%s), skipping valuation", raw_decimals, symbol, identifier
            )
            return None
        try:
