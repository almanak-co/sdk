...
- **Connector portability is strong**: Well-established connectors (Uniswap V3, Aave V3) work on new chains with zero code changes. Confirmed: UniV3 on 7 chains (arbitrum, base, optimism, ethereum, avalanche, polygon, mantle), Aave V3 on 8 chains (+ Sonic in iter 100). The blockers are always config entries (PROTOCOL_ROUTERS, SWAP_QUOTER_ADDRESSES, token registry addresses), not connector logic. Confirmed iters 49, 51, 90, 94, 100.
