...
- **多链支持** - Ethereum、Arbitrum、Optimism、Base、Avalanche、Polygon、BSC、Sonic、Plasma、Blast、Linea、Mantle、Berachain、Solana、Monad、X-Layer、0G Chain、HyperEVM、Robinhood。
- **协议集成** - Uniswap V3/V4、PancakeSwap V3、SushiSwap V3、TraderJoe V2、Aerodrome、Curve、Aave V3、Morpho Blue、GMX V2、Compound V3、Pendle、Spark、Lido、Ethena、Enso、LiFi、Kraken 以及[另外 20+ 个](api/connectors/index.md)。_（预测市场（Polymarket）与闪电贷（Balancer）为实验性功能，在完成测试前暂不列入支持矩阵。）_
- **非托管设计** - 通过 Safe 智能账户完全控制您的资金，自动生成 Zodiac Roles 权限清单。
...
Anvil 分叉测试（见下文）需要 [Foundry](https://book.getfoundry.sh/getting-started/installation)：
...
```bash
# 创建新策略（创建一个包含 pyproject.toml、.venv/、uv.lock 的独立 Python 项目）
almanak strat new
...
每个创建的策略都是一个独立的 Python 项目，拥有自己的 `pyproject.toml`、`.venv/` 和 `uv.lock`。相同的文件同时驱动本地开发和平台的云端 Docker 构建。
