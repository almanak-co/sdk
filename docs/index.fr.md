...
- **Support multichaînes** - Ethereum, Arbitrum, Optimism, Base, Avalanche, Polygon, BSC, Sonic, Plasma, Blast, Linea, Mantle, Berachain, Solana, Monad, X-Layer, 0G Chain, HyperEVM, Robinhood.
- **Intégration de protocoles** - Uniswap V3/V4, PancakeSwap V3, SushiSwap V3, TraderJoe V2, Aerodrome, Curve, Aave V3, Morpho Blue, GMX V2, Compound V3, Pendle, Spark, Lido, Ethena, Enso, LiFi, Kraken, et [20+ autres](api/connectors/index.md). _(Les marchés prédictifs (Polymarket) et les prêts flash (Balancer) sont expérimentaux et temporairement retirés de la matrice de support en attendant les tests.)_
...
almanak/
  framework/           # Framework de stratégie V2
    ...
    execution/         # Orchestration des transactions (incl. signers, simulateurs)
    backtesting/       # PnL, paper trading, balayage de paramètres
    data/              # Oracles de prix, indicateurs, résolution de tokens
    market/            # MarketSnapshot canonique
    alerting/          # Notifications Slack/Telegram
    services/          # Détection de blocage, gestion d'urgence
  connectors/          # Adaptateurs de protocoles (SDK, adapter, receipt parser)
