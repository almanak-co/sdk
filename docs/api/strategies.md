...
| **Each forced branch requires execution by default.** An unexpected `Intent.hold(...)` fails lifecycle assertions. Guard tests may declare `test_action_expectations = {"open": {"expected_to_hold": True, "reason": "Entry guard must reject this scenario."}}` before the run. | A declared HOLD must hold and provides no execution coverage. Repeated action names share the same expectation. Deployment gating must consume `summary.deployment_ready`, not assertion-only `all_passed`. |
