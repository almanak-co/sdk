...
| `ALMANAK_GATEWAY_NO_SPAWN` | Garde pour les environnements gérés de `almanak ax`. Lorsque `true`, un échec de connexion à la passerelle est signalé comme une défaillance de l'environnement au lieu de démarrer silencieusement une passerelle locale sans identifiants. Par défaut `false` ; à utiliser uniquement lorsqu'une passerelle provisionnée par la plateforme est attendue. |
