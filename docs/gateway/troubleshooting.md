...
Allowed chains: `ethereum`, `arbitrum`, `base`, `optimism`, `polygon`, `avalanche`, `bsc`, `sonic`, `plasma`, `linea`, `blast`, `mantle`, `berachain`, `monad`, `xlayer`, `zerog`, `hyperevm`, `robinhood`, `solana`
...
1. **Wrong deployment_id**
   ```python
   # Ensure consistent deployment_id
   ...
   # Load with same ID
   data = await state.load_state(deployment_id="my-strategy")  # Same ID!
   ```
...
3. **Database not configured**
   ```bash
   # Verify ALMANAK_GATEWAY_DATABASE_URL is set in gateway
