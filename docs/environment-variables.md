...
| `ALMANAK_ALLOW_PLACEHOLDER_PRICES` | **Temporary escape hatch — do not set in production.** When truthy, restores the pre-ALM-3183 behaviour in which an empty price oracle silently fell back to a table of hardcoded placeholder prices (ETH=$2000, WBTC=$45000, unknown symbol=$1 — 40-60% wrong) instead of failing that intent's compilation. It also re-disables the price-impact guard, which is skipped whenever placeholder mode is on, so slippage floors, position sizing and health-factor math are all computed from made-up numbers. Logs at ERROR on every read. **Scope — it does not relax everything:** it applies *only* to the in-process `StrategyRunner` compile path (`_init_single_chain_state`). The gateway's `ExecutionServiceServicer` compile path — which is what the gateway-backed runner and every hosted deployment actually use — and the config-mode `MultiChainOrchestrator` both stay **fail-closed regardless of this variable**, by design: the gateway is the security perimeter and already fails closed on mainnet via the VIB-523 price gate. Provided only so an operator who hits an unexpected fail-closed on the local path can keep running for one release; **slated for removal one release after ALM-3183**. The correct fix is to supply real prices — call `market.price(token)` in `decide()` for every token your intents touch. | `false` |
