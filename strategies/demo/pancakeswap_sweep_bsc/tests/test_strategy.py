"""Unit tests for the PancakeSwap V3 RSI Sweep Strategy on BSC.
...
from strategies.demo.pancakeswap_sweep_bsc.strategy import PancakeSwapSweepBSCStrategy
...
def _create_strategy(config_overrides=None):
    ...
    return PancakeSwapSweepBSCStrategy(
        config=config,
        chain="bsc",
...
def _make_market(
    ...
    quote_bal = MagicMock()
    quote_bal.balance = quote_balance_usd / Decimal("1")
...
class TestInitialization:
    ...
    def test_sweep_parameter_override(self):
        ...
        assert s.trade_size_usd == Decimal("200")
...
class TestDecisions:
    ...
    def test_buy_uses_pancakeswap_v3_protocol(self, strategy):
        """Buy intent specifies pancakeswap_v3 as protocol."""
        market = _make_market(Decimal("25"))
        intent = strategy.decide(market)
    ...
    def test_buy_at_exact_oversold_boundary(self, strategy):
    ...
    def test_sell_at_exact_overbought_boundary(self, strategy):
    ...
    def test_hold_when_insufficient_base_balance(self, strategy):
        """Sell signal but insufficient WBNB balance -> hold."""
    ...
    def test_hold_when_rsi_unavailable(self, strategy):
        ...
        market = MagicMock()
        market.price.return_value = Decimal("600")
        market.rsi.side_effect = ValueError("Not enough data")
        intent = strategy.decide(market)
        assert intent.intent_type.value == "HOLD"
        assert "unavailable" in intent.reason.lower()

    def test_hold_when_base_price_zero(self, strategy):
        """Sell signal but zero base price -> hold (avoid division by zero)."""
...
class TestSweepParameterVariation:
    ...
    def test_narrow_vs_wide_rsi_range(self):
        ...
        assert narrow_intent.from_token == "WBNB"  # selling
    ...
    def test_different_trade_sizes_in_intent(self):
        ...
        small = _create_strategy({"trade_size_usd": "50"})
        large = _create_strategy({"trade_size_usd": "200"})
        ...
        assert small_intent.amount_usd == Decimal("50")
        assert large_intent.amount_usd == Decimal("200")
    ...
    def test_rsi_period_passed_to_market(self):
        ...
        market10.rsi.assert_called_once_with("WBNB", period=10)
        market20.rsi.assert_called_once_with("WBNB", period=20)
...
class TestTeardown:
    ...
    def test_teardown_soft(self, strategy):
        ...
        assert intents[0].protocol == "pancakeswap_v3"
    ...
    def test_open_positions(self, strategy):
        ...
        balance_obj.balance_usd = Decimal("300")
        ...
        assert summary.positions[0].chain == "bsc"
