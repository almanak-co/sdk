"""Compound V3 PnL Backtest Strategy — Supply Rate Tracker on Polygon.

Kitchen Loop iteration 133 strategy (VIB-2034). Validates the PnL backtester
pipeline with Compound V3 on Polygon — the first PnL backtest of Compound V3
on Polygon. Exercises the PnL engine with Compound V3's comet-based lending
on a chain with different gas dynamics (MATIC gas token vs ETH).
...
- First Compound V3 PnL backtest on Polygon (protocol + chain coverage gap)
- Validates SupplyIntent + WithdrawIntent handling on Polygon chain
- Tests historical price data for Polygon chain tokens in backtesting context

Run PnL backtest:
    almanak strat backtest pnl -s demo_compound_v3_pnl_polygon \\
...
@almanak_strategy(
    name="demo_compound_v3_pnl_polygon",
    description="Compound V3 USDC supply rate tracker on Polygon for PnL backtesting",
    version="1.0.0",
    author="Almanak",
    tags=["demo", "backtesting", "lending", "compound-v3", "pnl", "polygon"],
    supported_chains=["polygon"],
    supported_protocols=["compound_v3"],
    intent_types=["SUPPLY", "WITHDRAW", "HOLD"],
    default_chain="polygon",
)
class CompoundV3PnLPolygonStrategy(IntentStrategy):
    """Compound V3 supply rate tracker strategy for PnL backtesting on Polygon.
    ...
    Configuration Parameters (from config.json):
        supply_token: Token to supply (default: "USDC.e" — bridged USDC on Polygon)
        ...
        force_entry_if_no_rate: Supply on every idle tick when lending_rate() is unavailable
            (default: False). When True, the strategy will emit SUPPLY whenever it is in idle
            state and rate data is missing — including after a failed supply reverts to idle.
            Set to True only in backtesting contexts where rate data is not wired up.
            NEVER set True in production — a data failure should hold, not transact.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        logger.info(
            "CompoundV3PnLPolygonStrategy initialized: "
    ...
    def decide(self, market: MarketSnapshot) -> Intent | None:
        ...
        if self._state == "idle":
            ...
            if not rate_available:
                return Intent.hold(reason="Supply rate unavailable; fallback entry disabled")
            return Intent.hold(
            ...
            if intent_type == "SUPPLY":
                ...
                add_event(
                    TimelineEvent(
                        timestamp=self._last_market_timestamp or datetime.now(UTC),
                        event_type=TimelineEventType.POSITION_MODIFIED,
                        description=f"Withdrew {self.supply_token} from Compound V3 on Polygon",
                        strategy_id=self.strategy_id,
    ...
    def get_status(self) -> dict[str, Any]:
        return {
            "strategy": "demo_compound_v3_pnl_polygon",
            "chain": self.chain,
            "state": self._state,
