#!/usr/bin/env python3
"""
Test Morpho-Aave Yield Rotation Strategy on Anvil Fork.
...
Usage:
    python strategies/reference/morpho_aave_arb/run_anvil.py morpho   # Test Morpho supply
    python strategies/reference/morpho_aave_arb/run_anvil.py aave     # Test Aave supply
    python strategies/reference/morpho_aave_arb/run_anvil.py both     # Test both (default)
"""
...
def start_anvil() -> subprocess.Popen | None:
    ...
    try:
        ...
        if proc.poll() is not None:
            ...
            print(f"ERROR: Anvil failed to start (check RPC URL config)")
            return None
...
def fund_erc20(token_address: str, token_symbol: str, amount: Decimal,
    ...
    try:
        ...
        if bal.returncode != 0:
            logger.warning(f"Balance check failed: {bal.stderr}")
            return False
...
def main():
    ...
    try:
        # Fund wallet
        if not fund_eth(Decimal("100")):
            print("ERROR: Failed to fund ETH")
            sys.exit(1)
        ...
        if args.action in ("aave", "both"):
            ...
            if args.action == "both":
                ...
                if not anvil_proc:
                    sys.exit(1)
                if not fund_eth(Decimal("100")):
                    print("ERROR: Failed to fund ETH for Aave test")
                    sys.exit(1)
                if not fund_wsteth(Decimal("2")):
