#!/usr/bin/env python3
"""
Run OBV Divergence Strategy on Anvil Fork

Tests the OBV divergence strategy through the full execution stack:
...
Usage:
    python strategies/tests/obv_divergence/run_anvil.py
"""
...
def calculate_obv(
    ...
    """
    Calculate On-Balance Volume (OBV) and signal line.
    ...
    if len(obv_series) >= signal_period:
        signal = sum(obv_series[-signal_period:]) / signal_period
    else:
        signal = sum(obv_series) / len(obv_series)
...
def run_strategy_on_anvil(force_action: str = "buy") -> SwapResult | None:
    """Run OBV Divergence Strategy on Anvil fork."""
    print(f"\n{'=' * 60}")
    print(f"RUNNING OBV DIVERGENCE STRATEGY (force: {force_action})")
    print(f"{'=' * 60}")
    ...
    from almanak.framework.strategies import MarketSnapshot, OBVData
    from almanak.framework.strategies.intent_strategy import TokenBalance
    from strategies.tests.obv_divergence import OBVDivergenceStrategy
    ...
    # Calculate OBV from simulated price and volume history
    # Simulate 30 periods of price and volume data for OBV calculation
    import random
    ...
    for _ in range(30):
        ...
        # Simulate volume (higher volume on up days for bullish signal)
        if change > 0:
            volumes.append(random.uniform(1000000, 2000000))
        else:
            volumes.append(random.uniform(500000, 1000000))

    obv_data = calculate_obv(prices, volumes, signal_period=21)

    print(f"ETH Price: ${eth_price}")
    print(f"OBV Value: {obv_data['obv']:,.0f}")
    print(f"OBV Signal: {obv_data['signal']:,.0f}")

    # Store OBV data in market snapshot using the new API
    market.set_obv(
        "WETH",
        OBVData(
            obv=Decimal(str(obv_data["obv"])),
            signal_line=Decimal(str(obv_data["signal"])),
            signal_period=21,
        ),
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run OBV Divergence Strategy on Anvil")
    parser.add_argument(
        "--action",
        choices=["buy", "sell"],
        default="buy",
    ...
    print("\n" + "=" * 60)
    print("OBV DIVERGENCE STRATEGY - ANVIL TEST")
    print("=" * 60)
    ...
    try:
        ...
        if result:
            ...
            print(f"\n{'=' * 60}")
            print("OBV DIVERGENCE STRATEGY EXECUTED SUCCESSFULLY!")
            print(f"{'=' * 60}\n")
        else:
