#!/usr/bin/env python3
"""
Run CCI Reversion Strategy on Anvil Fork

Tests the CCI reversion strategy through the full execution stack:
...
Usage:
    python strategies/tests/cci_reversion/run_anvil.py
"""
...
def calculate_cci(
    ...
    for i in range(len(prices_close)):
        tp = (prices_high[i] + prices_low[i] + prices_close[i]) / 3
    ...
    deviations = []
    for tp in typical_prices[-period:]:
        deviations.append(abs(tp - tp_sma))
...
def run_strategy_on_anvil(force_action: str = "buy") -> SwapResult | None:
    """Run CCI Reversion Strategy on Anvil fork."""
    print(f"\n{'=' * 60}")
    print(f"RUNNING CCI REVERSION STRATEGY (force: {force_action})")
    print(f"{'=' * 60}")
    ...
    config.cci_period = 20
    config.cci_upper_level = 100
    config.cci_lower_level = -100
    ...
    # Calculate CCI from simulated price history
    # Simulate 30 periods of OHLC data for CCI calculation
    import random
    ...
    cci_data = calculate_cci(prices_high, prices_low, prices_close, period=20)

    print(f"ARB Price: ${arb_price}")
    print(f"CCI Value: {cci_data['cci']:.2f}")

    # Store CCI data in market snapshot using the new API
    market.set_cci(
        "ARB",
        CCIData(
            value=Decimal(str(cci_data["cci"])),
            period=20,
        ),
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run CCI Reversion Strategy on Anvil")
    parser.add_argument(
        "--action",
        choices=["buy", "sell"],
        default="buy",
    ...
    print("\n" + "=" * 60)
    print("CCI REVERSION STRATEGY - ANVIL TEST")
    print("=" * 60)
    ...
    try:
        ...
        if result:
            ...
            print(f"\n{'=' * 60}")
            print("CCI REVERSION STRATEGY EXECUTED SUCCESSFULLY!")
            print(f"{'=' * 60}\n")
        else:
