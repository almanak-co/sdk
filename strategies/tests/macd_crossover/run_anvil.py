#!/usr/bin/env python3
"""
Run MACD Crossover Strategy on Anvil Fork

Tests the MACD crossover strategy through the full execution stack:
...
Usage:
    python strategies/tests/macd_crossover/run_anvil.py
"""
...
def fund_wallet_with_arb(wallet: str, amount_arb: Decimal) -> bool:
    """Fund wallet with ARB by swapping USDC to ARB on Uniswap."""
    print(f"\n{'=' * 60}")
    print(f"FUNDING WALLET WITH {amount_arb} ARB")
    print(f"{'=' * 60}")

    # ARB is an ERC20, we need to find a whale or swap for it
    # Let's use a whale - the Arbitrum treasury or a major holder
...
def calculate_macd(prices: list[float], fast: int = 12, slow: int = 26, signal: int = 9) -> dict:
    ...
    signal_ema = ema(macd_line_series, signal)

    macd_line = macd_line_series[-1]
    signal_line = signal_ema[-1] if signal_ema else macd_line
...
def run_strategy_on_anvil(force_action: str = "buy") -> SwapResult | None:
    """Run MACD Crossover Strategy on Anvil fork."""
    print(f"\n{'=' * 60}")
    print(f"RUNNING MACD CROSSOVER STRATEGY (force: {force_action})")
    print(f"{'=' * 60}")
    ...
    # Calculate MACD from simulated price history
    # Simulate 50 periods of price data for MACD calculation
    import random
    ...
    print(f"MACD Line: {macd_data['macd_line']:.6f}")
    print(f"Signal Line: {macd_data['signal_line']:.6f}")
    print(f"Histogram: {macd_data['histogram']:.6f}")

    # Store MACD data in market snapshot using proper API
    market.set_macd(
        "ARB",
        MACDData(
            macd_line=Decimal(str(macd_data["macd_line"])),
            signal_line=Decimal(str(macd_data["signal_line"])),
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run MACD Crossover Strategy on Anvil")
    parser.add_argument(
        "--action",
        choices=["buy", "sell"],
        default="buy",
    ...
    print("\n" + "=" * 60)
    print("MACD CROSSOVER STRATEGY - ANVIL TEST")
    print("=" * 60)
    ...
    try:
        ...
        if result:
            ...
            print(f"\n{'=' * 60}")
            print("MACD CROSSOVER STRATEGY EXECUTED SUCCESSFULLY!")
            print(f"{'=' * 60}\n")
        else:
