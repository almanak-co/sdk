#!/usr/bin/env python3
"""
Run RSI Reversion Strategy on Anvil Fork

Tests the RSI reversion strategy through the full execution stack:
...
Usage:
    python strategies/tests/rsi_reversion/run_anvil.py
"""
...
def run_strategy_on_anvil(force_action: str = "buy") -> SwapResult | None:
    """Run RSI Reversion Strategy on Anvil fork."""
    print(f"\n{'=' * 60}")
    print(f"RUNNING RSI REVERSION STRATEGY (force: {force_action})")
    print(f"{'=' * 60}")
    ...
    from strategies.tests.rsi_reversion import RSIReversionStrategy
    ...
    # Set RSI (for logging purposes, force_action overrides)
    rsi_value = Decimal("25") if force_action == "buy" else Decimal("75")
    rsi_data = RSIData(value=rsi_value, period=14)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run RSI Reversion Strategy on Anvil")
    parser.add_argument(
        "--action",
        choices=["buy", "sell"],
        default="buy",
    ...
    print("\n" + "=" * 60)
    print("RSI REVERSION STRATEGY - ANVIL TEST")
    print("=" * 60)
    ...
    try:
        ...
        if result:
            ...
            print(f"\n{'=' * 60}")
            print("RSI REVERSION STRATEGY EXECUTED SUCCESSFULLY!")
            print(f"{'=' * 60}\n")
        else:
