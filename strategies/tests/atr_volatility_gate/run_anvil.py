#!/usr/bin/env python3
"""
Run ATR Volatility Gate Strategy on Anvil Fork

Tests the ATR volatility gate strategy through the full execution stack:
...
Usage:
    python strategies/tests/atr_volatility_gate/run_anvil.py
"""
...
def calculate_atr(
    ...
    Returns ATR value and ATR as percentage of current price.
    """
...
def run_strategy_on_anvil(force_action: str = "buy") -> SwapResult | None:
    """Run ATR Volatility Gate Strategy on Anvil fork."""
    print(f"\n{'=' * 60}")
    print(f"RUNNING ATR VOLATILITY GATE STRATEGY (force: {force_action})")
    print(f"{'=' * 60}")
    ...
    from almanak.framework.strategies import ATRData, MarketSnapshot
    from almanak.framework.strategies.intent_strategy import TokenBalance
    from strategies.tests.atr_volatility_gate import ATRVolatilityGateStrategy
    ...
    config.max_volatility_threshold = Decimal("5")
    ...
    strategy = ATRVolatilityGateStrategy(
        config=config,
        chain="arbitrum",
    ...
    # Generate OHLC data for ATR calculation
    ...
    for _ in range(20):
        # Simulate price movement with moderate volatility (~2-3% daily)
        change = random.uniform(-0.02, 0.02)
        close = base_price * (1 + change)
        # Daily range typically 1-2% around close
        high = close * (1 + random.uniform(0.005, 0.015))
        low = close * (1 - random.uniform(0.005, 0.015))
    ...
    atr_data = calculate_atr(highs, lows, closes, period=14)

    print(f"WETH Price: ${eth_price}")
    print(f"ATR (14): {atr_data['atr']:.2f}")
    print(f"ATR as % of Price: {atr_data['atr_percent']:.2f}%")

    # Store ATR data in market snapshot using the new API
    market.set_atr(
        "WETH",
        ATRData(
            value=Decimal(str(atr_data["atr"])),
            value_percent=Decimal(str(atr_data["atr_percent"])),
            period=14,
        ),
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run ATR Volatility Gate Strategy on Anvil")
    parser.add_argument(
        "--action",
        choices=["buy", "sell"],
        default="buy",
    ...
    print("\n" + "=" * 60)
    print("ATR VOLATILITY GATE STRATEGY - ANVIL TEST")
    print("=" * 60)
    ...
    try:
        ...
        if result:
            ...
            print(f"\n{'=' * 60}")
            print("ATR VOLATILITY GATE STRATEGY EXECUTED SUCCESSFULLY!")
            print(f"{'=' * 60}\n")
        else:
