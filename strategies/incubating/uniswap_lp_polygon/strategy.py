"""Uniswap V3 LP on Polygon -- YAInnick Loop Iteration 22.

First kitchenloop strategy on Polygon (chain_id=137). Opens a concentrated
Uniswap V3 LP position (WETH/USDC, 0.05% fee tier) around the current price.
Exercises chain support, token resolution, Anvil fork, gateway auto-start,
and wallet funding on an untested chain.
...
@dataclass
class PolygonLPConfig:
    """Configuration for Uniswap V3 LP strategy on Polygon."""
...
@almanak_strategy(
    name="uniswap_lp_polygon",
    description="Uniswap V3 LP on Polygon - first kitchenloop test on Polygon chain",
    version="1.0.0",
    author="YAInnick Loop",
    tags=["incubating", "lp", "uniswap-v3", "polygon"],
    supported_chains=["polygon"],
    supported_protocols=["uniswap_v3"],
    intent_types=["LP_OPEN", "LP_CLOSE", "HOLD"],
)
class UniswapLPPolygonStrategy(IntentStrategy[PolygonLPConfig]):
    """Uniswap V3 concentrated LP on Polygon.

    Opens a WETH/USDC LP position with a configurable price range width.
    Designed to stress-test the Polygon chain path end-to-end.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        logger.info(
            f"UniswapLPPolygonStrategy initialized: "
            f"pool={self.pool}, "
    ...
    def decide(self, market: MarketSnapshot) -> Intent | None:
        ...
        try:
            # Get current price (token1 per token0, i.e. USDC per WETH)
            try:
            ...
            if self.force_action == "close":
                ...
                if not position_id:
                    logger.warning("force_action=close but no position_id available")
        ...
        if self._current_position_id:
            positions.append(
                PositionInfo(
                    ...
                    chain=self.chain,
                    protocol="uniswap_v3",
                    value_usd=Decimal("0"),  # Unknown without on-chain query
                    details={
                        "pool": self.pool,
    ...
    def generate_teardown_intents(self, mode: TeardownMode, market: MarketSnapshot | None = None) -> list[Intent]:
        """Return intents to close all open positions."""
