...
class XLayerAaveOkbClmmLoopStrategy(IntentStrategy):
    ...
    def decide(self, market: MarketSnapshot) -> Intent | None:
        ...
        #    Checked BEFORE price gate so teardown can proceed even when oracles are down.
        if self.get_config("force_teardown", False) and self._state == "running":
            ...
            if self._teardown_queue:
                # Return the next step without popping — on_intent_executed pops on success
                intent = self._teardown_queue[0]
    ...
    def _compute_realized_vol(self) -> Decimal | None:
        ...
        # Scale to daily: derive samples/day from actual observation interval.
        interval_s = getattr(self, "_tick_interval_s", 30)  # runner tick interval
        if len(self._price_history) >= 2:
            ts_diff = self._price_history[-1][0] - self._price_history[-2][0]
            if ts_diff > 0:
                interval_s = ts_diff
        samples_per_day = 86400 / max(interval_s, 1)
        daily_vol = vol_per_sample * math.sqrt(samples_per_day)
    ...
    def _build_lp_open_intent(self, market: MarketSnapshot) -> Intent:
        ...
        if amount0 <= 0 or amount1 <= 0:
            # Don't advance to "running" — no LP was opened. Stay in current state
            # so the next decide() cycle retries.
            return Intent.hold(
    ...
    def on_intent_executed(self, intent: Intent, success: bool, result: Any) -> None:
        ...
        # Pop completed teardown step on success (decide() returns queue[0] without popping)
        if self._teardown_queue and self._teardown_queue[0] is intent:
            self._teardown_queue.pop(0)
    ...
    def load_persistent_state(self, state: dict[str, Any]) -> None:
        ...
        # Rollback transient states on recovery — if the process died mid-transition
        # these states have no pending callback to advance them, so revert to the
        # last stable state to allow the decide() loop to retry.
        transient_states = ("supplying", "borrowing", "converting", "splitting", "opening_lp", "closing_lp")
        if self._state in transient_states:
            revert_to = self._previous_stable_state or "idle"
            logger.warning(f"Recovery: rolling back transient state '{self._state}' -> '{revert_to}'")
            self._state = revert_to
