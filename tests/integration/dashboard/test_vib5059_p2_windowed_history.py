...
async def test_nav_window_bounds_and_budget(big_sm: StateManager) -> None:
    ...
    points = [NavPoint(ts, Decimal(v)) for ts, v, _c in rows if v]
    ...
    first_raw_ts, first_raw_val, _ = rows[0]
    last_raw_ts, last_raw_val, _ = rows[-1]
...
async def test_postgres_window_sql_shape_and_parity(big_sm: StateManager) -> None:
    ...
    # No JSON blob columns projected.
    ...
    pg_db_rows = [
        {"timestamp": ts, "total_value_text": val, "value_confidence": conf} for ts, val, conf in reversed(sqlite_rows)
    ]
    ...
    def _to_points(rows):
        return [NavPoint(ts, Decimal(v)) for ts, v, _c in rows if v]
    ...
    empty_rows = [
        {"timestamp": _snap_ts(1), "total_value_text": "1000.00", "value_confidence": "HIGH"},
        {"timestamp": _snap_ts(2), "total_value_text": "", "value_confidence": "HIGH"},  # unmeasured
        {"timestamp": _snap_ts(3), "total_value_text": "1001.00", "value_confidence": "HIGH"},
    ]
...
async def test_nonempty_window_never_empty(tmp_path) -> None:
    ...
    out1 = decimate_nav([NavPoint(ts, Decimal(v)) for ts, v, _c in rows1 if v], clamp_max_points(1500))
    ...
    out2 = decimate_nav([NavPoint(ts, Decimal(v)) for ts, v, _c in rows2 if v], clamp_max_points(1500))
    ...
    outN = decimate_nav([NavPoint(ts, Decimal(v)) for ts, v, _c in rowsN if v], budget)
