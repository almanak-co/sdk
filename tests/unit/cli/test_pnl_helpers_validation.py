...
class TestValidateAndBuildContextConfigGuard:
    ...
    def test_builds_fresh_config_when_not_loaded(self) -> None:
        ...
        with patch("almanak.framework.cli.backtest.pnl.validate_strategy_is_registered"):
    ...
    def test_threads_auto_timeframe_into_fresh_config(self) -> None:
        """The public CLI seam preserves auto until provider preflight."""
        with patch("almanak.framework.cli.backtest.pnl.validate_strategy_is_registered"):
            ctx = _validate_and_build_context(
                strategy="demo",
                start=datetime(2024, 1, 1, tzinfo=UTC),
        ...
        assert ctx.pnl_config.timeframe == "auto"
        assert ctx.pnl_config.resolved_timeframe is None

    def test_raises_click_abort_when_config_is_none_after_load_path(self, capsys: pytest.CaptureFixture[str]) -> None:
        """Regression for #1700.
