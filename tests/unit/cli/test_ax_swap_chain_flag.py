"""Tests for ``almanak ax swap --chain`` flag handling (VIB-3182).

Follow-on to VIB-3142 / PR #1583, which fixed the same bug on
``ax balance``. Before VIB-3182, ``ax swap USDC ETH 100 --chain base``
was rejected by Click because only the ``ax`` group declared
``--chain``. The swap help text advertised the subcommand-level
placement, so the rejection was a UX bug. This test pins:

1. ``ax swap USDC ETH 100 --chain base`` is accepted (regression test).
2. ``ax --chain base swap USDC ETH 100`` (group placement) still works.
...
class TestAxSwapChainFlag:
    """Verify --chain works on the ax swap subcommand in all placements."""

    @patch("almanak.framework.cli.ax._run_tool")
    def test_subcommand_chain_flag_is_accepted(self, mock_run_tool: MagicMock):
        """Regression: ``ax swap USDC ETH 100 --chain base`` must not be rejected."""
        ...
        result = runner.invoke(
            almanak,
            ["ax", "swap", "USDC", "ETH", "100", "--chain", "base", "--dry-run"],
        )
    ...
    def test_group_chain_flag_still_works(self, mock_run_tool: MagicMock):
        """Backcompat: ``ax --chain base swap USDC ETH 100`` still works."""
        mock_response = MagicMock()
        mock_response.status = "success"
        mock_run_tool.return_value = mock_response
        ...
        result = runner.invoke(
            almanak,
            ["ax", "--chain", "base", "swap", "USDC", "ETH", "100", "--dry-run"],
        )
    ...
    def test_short_flag_c_also_works(self, mock_run_tool: MagicMock):
        ...
        result = runner.invoke(
            almanak,
            ["ax", "swap", "USDC", "ETH", "100", "-c", "base", "--dry-run"],
        )
