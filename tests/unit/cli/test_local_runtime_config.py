...
class TestLocalRuntimeConfigInit:
    ...
    def test_init_with_valid_config(self):
        ...
        assert config.simulation_enabled is True  # default flipped post #2066-era; live prod ships dry-run-on
...
class TestChainValidation:
    ...
    def test_all_supported_chains(self, chain: str):
        ...
        Solana is excluded because it requires a base58 Ed25519 keypair, not
        the hex EVM `TEST_PRIVATE_KEY` used here. See
        TestSolanaWalletValidation for Solana-specific cases.
        """
        config = LocalRuntimeConfig(
            chain=chain,
...
class TestFromEnv:
    ...
    def test_from_env_missing_required_raises_error(self):
        ...
        `ALMANAK_RPC_URL` was demoted to optional (the loader falls back to
        public RPCs / ALCHEMY_API_KEY when it is unset), so the only env var
        that still triggers `MissingEnvironmentVariableError` is the private
        key. Test that path instead.
        """
        env_vars = {
            "ALMANAK_CHAIN": "arbitrum",
            "ALMANAK_RPC_URL": "https://arb1.arbitrum.io/rpc",
            # Missing ALMANAK_PRIVATE_KEY
        }
    ...
    def test_chain_id_derived_correctly(self):
        ...
        for chain_name, expected_id in CHAIN_IDS.items():
            if chain_name == "solana":
                continue  # Solana requires a base58 Ed25519 key, not a hex EVM key.
            config = LocalRuntimeConfig(
                chain=chain_name,
                rpc_url="https://rpc.example.com",
                private_key=TEST_PRIVATE_KEY,
