...
class TestSubmission:
    ...
    def test_submit_insufficient_funds_raises(
        ...
        mock_web3 = MagicMock()
        mock_web3.eth = AsyncMock()
        # Use a realistic geth-style message so VIB-3714 parser yields have/want;
        ...
        # Confirm the have/want parser populated the structured fields, not
        # just that the exception class matched.
        assert exc_info.value.available == 0
        assert exc_info.value.required == 10**15
...
# =============================================================================
# Revert Data Decoding Tests (VIB-513)
# =============================================================================


class TestDecodeRevertData:
    """Tests for _decode_revert_data, including OpenZeppelin error selectors."""

    @pytest.fixture
    def submitter(self) -> PublicMempoolSubmitter:
        return PublicMempoolSubmitter(rpc_url="https://example.com")

    def test_decode_failed_inner_call(self, submitter: PublicMempoolSubmitter) -> None:
        """FailedInnerCall() from OpenZeppelin Address library."""
        result = submitter._decode_revert_data("0x1425ea42")
        assert "FailedInnerCall" in result

    def test_decode_failed_call(self, submitter: PublicMempoolSubmitter) -> None:
        """FailedCall() from OpenZeppelin Address library."""
        result = submitter._decode_revert_data("0xd6bda275")
        assert "FailedCall" in result

    def test_decode_address_insufficient_balance(self, submitter: PublicMempoolSubmitter) -> None:
        """AddressInsufficientBalance from OpenZeppelin."""
        result = submitter._decode_revert_data("0xcd786059")
        assert "AddressInsufficientBalance" in result

    def test_decode_address_empty_code(self, submitter: PublicMempoolSubmitter) -> None:
        """AddressEmptyCode from OpenZeppelin."""
        result = submitter._decode_revert_data("0x9996b315")
        assert "AddressEmptyCode" in result

    def test_decode_erc20_insufficient_balance(self, submitter: PublicMempoolSubmitter) -> None:
        """ERC20InsufficientBalance from OpenZeppelin ERC-20."""
        result = submitter._decode_revert_data("0xe450d38c")
        assert "ERC20InsufficientBalance" in result

    def test_decode_standard_error_string(self, submitter: PublicMempoolSubmitter) -> None:
        """Standard Error(string) decoding still works."""
        # "Error(string)" with message "hello"
        # selector: 0x08c379a0
        # offset: 32 (0x20)
        # length: 5
        ...
        result = submitter._decode_revert_data(revert)
        assert "hello" in result

    def test_decode_unknown_selector(self, submitter: PublicMempoolSubmitter) -> None:
        """Unknown selector returns raw selector info."""
        result = submitter._decode_revert_data("0xdeadbeef")
        assert "Unknown revert" in result
        assert "0xdeadbeef" in result

    def test_decode_empty_data(self, submitter: PublicMempoolSubmitter) -> None:
        """Empty revert data returns descriptive message."""
        result = submitter._decode_revert_data("")
        assert "Empty" in result

    def test_decode_erc20_insufficient_allowance(self, submitter: PublicMempoolSubmitter) -> None:
        """ERC20InsufficientAllowance from OpenZeppelin ERC-20."""
        result = submitter._decode_revert_data("0xfb8f41b2")
        assert "ERC20InsufficientAllowance" in result

    def test_decode_erc20_invalid_sender(self, submitter: PublicMempoolSubmitter) -> None:
        """ERC20InvalidSender from OpenZeppelin ERC-20."""
        result = submitter._decode_revert_data("0x96c6fd1e")
        assert "ERC20InvalidSender" in result

    def test_decode_erc20_invalid_receiver(self, submitter: PublicMempoolSubmitter) -> None:
        """ERC20InvalidReceiver from OpenZeppelin ERC-20."""
        result = submitter._decode_revert_data("0xec442f05")
        assert "ERC20InvalidReceiver" in result

    def test_decode_ownable_unauthorized_account(self, submitter: PublicMempoolSubmitter) -> None:
        """OwnableUnauthorizedAccount from OpenZeppelin Ownable."""
        result = submitter._decode_revert_data("0x118cdaa7")
        assert "OwnableUnauthorizedAccount" in result

    def test_decode_ownable_invalid_owner(self, submitter: PublicMempoolSubmitter) -> None:
        """OwnableInvalidOwner from OpenZeppelin Ownable."""
        result = submitter._decode_revert_data("0x1e4fbdf7")
        assert "OwnableInvalidOwner" in result

    def test_decode_bytes_input(self, submitter: PublicMempoolSubmitter) -> None:
