...
def mock_clob_handler(mock_clob_client):
    ...
    Mirrors `ClobActionHandler.can_handle` so the executor's handler-registry
    routing distinguishes CLOB orders (off-chain, empty transactions) from
    on-chain bundles like redemptions.
    """
    handler = MagicMock(spec=ClobActionHandler)
    handler.supported_protocols = ["polymarket"]

    def _can_handle(bundle):
        return (
            bundle.metadata.get("protocol") == "polymarket"
            and len(bundle.transactions) == 0
            and "order_request" in bundle.metadata
        )

    handler.can_handle.side_effect = _can_handle
    return handler
