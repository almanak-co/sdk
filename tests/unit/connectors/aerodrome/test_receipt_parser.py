...
class TestParserInit:
    ...
    def test_parser_creation(self) -> None:
        ...
        # Default initialization leaves token info unresolved.
        assert parser.token0_address is None
        assert parser.token1_address is None
        assert parser.quoted_price is None
...
class TestParsedSwapResult:
    ...
    def test_parsed_swap_result_to_dict(self) -> None:
        ...
        assert dict_result["pool_address"] == "0xpool"
        assert dict_result["slippage_bps"] == 10
...
class TestParsedLiquidityResult:
    ...
    def test_add_liquidity_result(self) -> None:
        ...
        result = ParsedLiquidityResult(
            ...
            amount0=1000000,
            amount1=500,
            amount0_decimal=Decimal("1.0"),
            amount1_decimal=Decimal("0.0000000000000005"),
            pool_address="0xpool",
        )

        assert result.operation == "add"
        assert result.amount0 == 1000000
    ...
    def test_to_dict(self) -> None:
        ...
        assert dict_result["amount1"] == "500"
        assert dict_result["pool_address"] == "0xpool"
...
class TestTokenConstants:
    ...
    def test_symbol_resolution(self) -> None:
        """Test symbol resolution from address via _resolve_token_info (mocked resolver)."""
        from unittest.mock import Mock, patch

        parser = AerodromeReceiptParser(chain="base")

        usdc_addr = "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913"
        fake_resolved = Mock(symbol="USDC", decimals=6)
        fake_resolver = Mock()
        fake_resolver.resolve.return_value = fake_resolved
        with patch(
            "almanak.framework.data.tokens.resolver.get_token_resolver",
            return_value=fake_resolver,
        ):
            symbol, decimals = parser._resolve_token_info(usdc_addr)
