...
class TestCompileStakeIntent:
    ...
    def test_compile_stake_intent_basic(self, adapter: EthenaAdapter) -> None:
        ...
        The adapter now emits an ERC-20 approve before the deposit, so the
        bundle contains two transactions: ``approve`` then ``stake``.
        """
        ...
        # Bundle: approve(USDe → sUSDe vault), then stake.
        assert len(bundle.transactions) == 2
        assert bundle.intent_type == "STAKE"

        approve_tx, stake_tx = bundle.transactions
        assert approve_tx["action_type"] == "approve"
        # approve targets the input token (USDe), not the staking vault, with
        # the standard ERC20 approve(address,uint256) selector and zero value.
        assert approve_tx["to"] == ETHENA_ADDRESSES["ethereum"]["usde"]
        ...
        assert bundle.metadata["total_gas_estimate"] == approve_tx["gas_estimate"] + DEFAULT_GAS_ESTIMATES["stake"]
    ...
    def test_compile_stake_intent_small_amount(self, adapter: EthenaAdapter) -> None:
        ...
        # approve + stake (see test_compile_stake_intent_basic for rationale).
        assert len(bundle.transactions) == 2
        assert bundle.transactions[0]["action_type"] == "approve"
        assert bundle.transactions[-1]["value"] == 0
    ...
    def test_compile_stake_intent_large_amount(self, adapter: EthenaAdapter) -> None:
        ...
        # approve + stake (see test_compile_stake_intent_basic).
        assert len(bundle.transactions) == 2
    ...
    def test_compile_stake_intent_metadata_completeness(self, adapter: EthenaAdapter) -> None:
        ...
        # Check all expected metadata fields. Bundle is approve + stake.
    ...
    def test_compile_stake_intent_gas_estimate(self, adapter: EthenaAdapter) -> None:
        ...
        Total gas now includes the approve tx; the stake portion still matches
        ...
        approve_tx, stake_tx = bundle.transactions
        assert stake_tx["gas_estimate"] == DEFAULT_GAS_ESTIMATES["stake"] == 150000
        assert bundle.metadata["total_gas_estimate"] == approve_tx["gas_estimate"] + stake_tx["gas_estimate"]
    ...
    def test_compile_stake_intent_calldata_encoding(self, adapter: EthenaAdapter) -> None:
        ...
        # Bundle is approve + stake; the stake tx is last.
        stake_tx = bundle.transactions[-1]
        assert stake_tx["action_type"] == "stake"
