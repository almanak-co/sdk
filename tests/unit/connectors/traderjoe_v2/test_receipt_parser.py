...
class TestParserIntegration:
    ...
    def test_parser_handles_bytes_tx_hash(self, parser: TraderJoeV2ReceiptParser) -> None:
        ...
        The parser normalizes a ``bytes`` ``transactionHash`` to a ``0x``-prefixed
        lowercase hex string so downstream consumers can rely on the standard
        EVM string form.
        """
        receipt = {
            "status": 1,
        ...
        assert result.success is True
        assert result.transaction_hash == TEST_TX_HASH.lower()
