...
EXPECTED_RUNNER_HOOK_PROVIDER_MODULES = {
    ...
    # ALM-2972: observes keeper-settled GMX orders and progresses the measured
    # cancellation window in the current managed Anvil session.
    "gmx_v2": "almanak.connectors.gmx_v2.runner_hooks",
...
class TestDexVolumeDeclValidation:
    ...
    def test_accepts_lowercase_https_subgraph_urls(self):
        decl = self._decl(volume_subgraph_urls={"ethereum": "https://gateway.example/subgraph"})
        assert decl.volume_subgraph_urls == {"ethereum": "https://gateway.example/subgraph"}
    ...
    def test_rejects_malformed_subgraph_urls(self, bad_urls):
        with pytest.raises(
            ValueError,
            match="volume_subgraph_urls values must be https:// URLs keyed by lowercase chain name",
        ):
            self._decl(volume_subgraph_urls=bad_urls)

    def test_rejects_non_mapping_subgraph_urls(self):
        with pytest.raises(ValueError, match="volume_subgraph_urls must be None or a Mapping"):
            self._decl(volume_subgraph_urls=[("ethereum", "https://gateway.example/subgraph")])
