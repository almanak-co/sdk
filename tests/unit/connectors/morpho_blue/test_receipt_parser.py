...
class TestSupplyEvent:
    ...
    def test_parse_supply_event(self, parser: MorphoBlueReceiptParser) -> None:
        ...
        Event ABI: ``Supply(Id indexed id, address indexed caller,
        address indexed onBehalfOf, uint256 assets, uint256 shares)``.
        ``caller`` and ``onBehalfOf`` are indexed → topics[2] / topics[3];
        data carries only ``assets`` and ``shares``.
        """
...
class TestRepayEvent:
    ...
    def test_parse_repay_event(self, parser: MorphoBlueReceiptParser) -> None:
        ...
        Event ABI: ``Repay(Id indexed id, address indexed caller,
        address indexed onBehalfOf, uint256 assets, uint256 shares)``.
        """
        assets = 500 * 10**6
        shares = 500 * 10**18
...
class TestSupplyCollateralEvent:
    ...
    def test_parse_supply_collateral_event(self, parser: MorphoBlueReceiptParser) -> None:
        ...
        Event ABI: ``SupplyCollateral(Id indexed id, address indexed caller,
        address indexed onBehalfOf, uint256 assets)``.
        """
...
class TestLiquidateEvent:
    ...
    def test_parse_liquidate_event(self, parser: MorphoBlueReceiptParser) -> None:
        ...
        Event ABI: ``Liquidate(Id indexed id, address indexed caller,
        ...
        assert event.data["caller"].lower() == CALLER_ADDRESS.lower()
        assert event.data["borrower"].lower() == ON_BEHALF_OF_ADDRESS.lower()
        assert event.data["repaid_assets"] == str(repaid_assets)
        assert event.data["repaid_shares"] == str(repaid_shares)
        assert event.data["seized_assets"] == str(seized_assets)
        assert event.data["bad_debt_assets"] == str(bad_debt_assets)
        assert event.data["bad_debt_shares"] == str(bad_debt_shares)
