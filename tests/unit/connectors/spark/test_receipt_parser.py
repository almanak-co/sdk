...
class TestSparkReceiptParserInit:
    ...
    def test_known_topics_loaded(self, parser: SparkReceiptParser) -> None:
        ...
        Parser now delegates topic registration to the shared ``EventRegistry``.
        """
        assert len(parser.registry.known_topics) > 0
        assert EVENT_TOPICS["Supply"] in parser.registry.known_topics
        assert EVENT_TOPICS["Withdraw"] in parser.registry.known_topics
