...
class TestPoolFetching:
    ...
    @patch.object(MeteoraSDK, "_make_request")
    def test_get_pool_new_api_format(self, mock_request):
        """Test parsing of the new datapi.meteora.ag /pools response format."""
        mock_request.return_value = {
            "address": "pool_new",
            "token_x": {"address": SOL_MINT, "decimals": 9, "symbol": "SOL"},
            "token_y": {"address": USDC_MINT, "decimals": 6, "symbol": "USDC"},
            "name": "SOL-USDC",
            "pool_config": {"bin_step": 10, "base_fee_pct": "0.25"},
            "active_id": 8388708,
            "current_price": 150.0,
            "liquidity": 1000000,
            "token_x_amount": "5000000000",
            "token_y_amount": "750000000",
        }

        sdk = MeteoraSDK(wallet_address=WALLET)
        pool = sdk.get_pool("pool_new")
        assert pool.address == "pool_new"
        assert pool.mint_x == SOL_MINT
        assert pool.mint_y == USDC_MINT
        ...
        assert pool.reserve_x == "5000000000"
        assert pool.reserve_y == "750000000"
        # vault_x/vault_y should be empty (not numeric reserve amounts)
        assert pool.vault_x == ""
        assert pool.vault_y == ""

    @patch.object(MeteoraSDK, "_make_request")
    def test_find_pool_new_api_data_wrapper(self, mock_request):
        """Test find_pool with new API 'data' wrapper and nested token objects."""
        mock_request.return_value = {
            "data": [
                {
                    "address": "pool_data",
        ...
        pool = sdk.find_pool(SOL_MINT, USDC_MINT)
        assert pool is not None
        assert pool.address == "pool_data"
        assert pool.mint_x == SOL_MINT

    def test_from_api_response_vault_not_polluted_by_reserves(self):
        """Ensure vault_x/vault_y are never set to numeric reserve amounts."""
        data = {
            "address": "pool_test",
            "token_x": {"address": SOL_MINT, "decimals": 9, "symbol": "SOL"},
            "token_y": {"address": USDC_MINT, "decimals": 6, "symbol": "USDC"},
        ...
        # vault fields must NOT contain numeric reserve amounts
        assert pool.vault_x != "999999999"
        assert pool.vault_y != "888888888"
