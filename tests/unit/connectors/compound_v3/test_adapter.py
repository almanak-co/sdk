...
class TestWithdrawOperations:
    ...
    def test_withdraw_collateral_all(self, adapter):
        ...
        Post-refactor: ``withdraw_all`` requires either an on-chain balance
        query (``CompoundV3Config.gateway_client``) or a fallback amount.
        The unit-test ``adapter`` fixture provides neither, so the call is
        expected to fail with a clear "no fallback amount provided" error.
        """
        result = adapter.withdraw_collateral(
            asset="WETH",
            amount=Decimal("0"),
        ...
        assert result.success is False
        assert "no fallback amount provided" in result.error

    def test_withdraw_collateral_all_uses_fallback_amount_when_query_unavailable(self, adapter):
        """Test withdraw_all falls back to provided amount when on-chain query is unavailable."""
        with patch.object(adapter, "_query_collateral_balance", return_value=None):
            result = adapter.withdraw_collateral(
                asset="WETH",
                amount=Decimal("0.25"),
                withdraw_all=True,
        ...
        assert result.tx_data is not None
        assert "Withdraw all" in result.description
        # Calldata must encode the fallback amount (0.25 WETH = 0.25 * 10^18 wei)
        assert hex(int(Decimal("0.25") * Decimal(10**18)))[2:] in result.tx_data["data"]
...
class TestApproval:
    ...
    def test_build_approve_unknown_token(self, adapter):
        ...
        assert result.success is False
        # Post-TokenResolver migration: error comes from the resolver, which
        # phrases the failure as ``Cannot resolve token '<symbol>' on <chain>``.
        assert "Cannot resolve token 'UNKNOWN'" in result.error
