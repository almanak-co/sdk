...
class TestHelperMethods:
    ...
    def test_decode_uint256(self, parser):
        ...
        value = HexDecoder.decode_uint256(data)
    ...
    def test_hex_to_decimal(self, parser):
        """Test int(<hex>, 16) — the prior ``_hex_to_decimal`` shim was removed.

        The receipt parser now calls ``int(value, 16)`` inline; this test
        documents the equivalent behavior so a future regression is visible.
        """
        value = Decimal(int("ff", 16))
    ...
    def test_hex_to_decimal_empty(self):
        ...
        # Empty hex defaults to zero (matches the legacy _hex_to_decimal contract)
        assert Decimal(HexDecoder.decode_uint256("")) == Decimal("0")
