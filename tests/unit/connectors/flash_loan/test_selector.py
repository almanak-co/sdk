...
class TestSelectProvider:
    ...
    def test_select_provider_fee_priority_prefers_zero_fee_provider(self, selector_ethereum: FlashLoanSelector) -> None:
        """Test that fee priority prefers a zero-fee provider.

        Both Balancer and Morpho charge zero fees on Ethereum. The selector's
        secondary tiebreakers (gas, reliability) favor Morpho today, so the
        contract this test enforces is "fee == 0", not the specific provider.
        """
        # USDC is supported on Aave, Balancer, and Morpho.
    ...
    def test_select_provider_reliability_priority_scores_correctly(self, selector_ethereum: FlashLoanSelector) -> None:
        ...
        assert result.is_success
        # Aave (0.98), Morpho (0.97), and Balancer (0.95) all support USDC.
        # Zero-fee providers (Balancer, Morpho) can still win on combined score.
        ...
        assert "morpho" in provider_names
    ...
    def test_select_provider_default_priority(self, selector_ethereum: FlashLoanSelector) -> None:
        ...
        Default is FEE; both Balancer and Morpho charge zero fees on
        Ethereum, so any zero-fee provider satisfies the contract.
        """
        result = selector_ethereum.select_provider(
            token="USDC",
            amount=Decimal("1000"),
...
class TestPriorityWeights:
    ...
    def test_gas_priority(self, selector_ethereum: FlashLoanSelector) -> None:
        ...
        Per ``PROVIDER_GAS_ESTIMATES``: aave 300k, balancer 250k, morpho 200k.
        Morpho is now the most gas-efficient on Ethereum.
        """
        result = selector_ethereum.select_provider(
            token="USDC",
            amount=Decimal("1000"),
...
class TestEdgeCases:
    ...
    def test_invalid_priority_uses_default(self, selector_ethereum: FlashLoanSelector) -> None:
        """Test invalid priority falls back to default (FEE)."""
        result = selector_ethereum.select_provider(
            token="USDC",
            amount=Decimal("1000"),
        ...
        assert result.is_success
        # Default FEE → any zero-fee provider (Balancer or Morpho).
        assert result.provider in ("balancer", "morpho")
        assert result.fee_bps == 0
