...
class TestGMXv2AdapterInit:
    ...
    def test_adapter_creation(self) -> None:
        ...
        Post-TokenResolver migration: per-adapter ``self.tokens`` map was
...
class TestGMXv2AdapterPositions:
    ...
    def test_open_position_unknown_token(self, adapter: GMXv2Adapter) -> None:
        ...
        Post-TokenResolver: the resolver phrases the failure as
        ``Cannot resolve token '<symbol>' on <chain>`` rather than the
        legacy ``Unknown collateral token``.
        """
        result = adapter.open_position(
            market="ETH/USD",
        ...
        assert result.success is False
        assert "Cannot resolve token 'UNKNOWN'" in (result.error or "")
    ...
    def test_get_position(self, adapter: GMXv2Adapter) -> None:
        ...
        ``_get_position_key`` builds the key by string-concatenating the
        market and collateral addresses; ``_resolve_token`` returns the
        lowercase form. The test uses the same lowercase form to seed
        the position so the lookup matches.
        """
...
class TestGMXv2AdapterHelpers:
    ...
    def test_resolve_token_by_symbol(self, adapter: GMXv2Adapter) -> None:
        ...
        ``TokenResolver.resolve`` returns the canonical lowercase address;
        ``GMX_V2_TOKENS`` stores the EIP-55 checksum form. Compare them in
        a case-insensitive way.
        """
        token_address = adapter._resolve_token("USDC")

        assert token_address is not None
        assert token_address.lower() == GMX_V2_TOKENS["arbitrum"]["USDC"].lower()
    ...
    def test_resolve_token_by_address(self, adapter: GMXv2Adapter) -> None:
        ...
        # Address is returned canonicalised by the resolver.
        assert token_address is not None
        assert token_address.lower() == address.lower()

    def test_resolve_token_unknown(self, adapter: GMXv2Adapter) -> None:
        """Test token resolution for unknown token.

        Post-TokenResolver: unknown symbols raise ``TokenResolutionError``
        rather than returning ``None``. The adapter wraps the resolver call
        and re-raises with the ``[GMXV2Adapter]`` prefix.
        """
        from almanak.framework.data.tokens.exceptions import TokenResolutionError

        with pytest.raises(TokenResolutionError, match=r"\[GMXV2Adapter\].*Cannot resolve token"):
            adapter._resolve_token("UNKNOWN")

    def test_get_token_decimals(self, adapter: GMXv2Adapter) -> None:
        """Test getting token decimals.

        Post-TokenResolver: unknown tokens raise ``TokenResolutionError``
