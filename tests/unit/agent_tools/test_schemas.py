...
class TestSchemaValidation:
    ...
    # -- WithdrawLendingRequest (VIB-2992) --
    def test_withdraw_amount_all_accepted(self):
        from almanak.framework.agent_tools.schemas import WithdrawLendingRequest

        req = WithdrawLendingRequest(token="USDC", amount="all")
        assert req.amount == "all"

    def test_withdraw_positive_amount(self):
        from almanak.framework.agent_tools.schemas import WithdrawLendingRequest

        req = WithdrawLendingRequest(token="USDC", amount="100")
    ...
    def test_withdraw_zero_rejected(self):
        ...
        with pytest.raises(ValidationError, match="positive"):
            WithdrawLendingRequest(token="USDC", amount="0")

    # -- market_id validation for isolated-market protocols (PR #1535 audit) --
    def test_withdraw_morpho_blue_requires_market_id(self):
        from almanak.framework.agent_tools.schemas import WithdrawLendingRequest

        with pytest.raises(ValidationError, match="market_id is required"):
            WithdrawLendingRequest(token="USDC", amount="100", protocol="morpho_blue")

    def test_withdraw_morpho_blue_with_market_id_ok(self):
        from almanak.framework.agent_tools.schemas import WithdrawLendingRequest

        req = WithdrawLendingRequest(token="USDC", amount="100", protocol="morpho_blue", market_id="0xabc")
        assert req.market_id == "0xabc"

    def test_supply_morpho_blue_requires_market_id(self):
        from almanak.framework.agent_tools.schemas import SupplyLendingRequest

        with pytest.raises(ValidationError, match="market_id is required"):
            SupplyLendingRequest(token="USDC", amount="100", protocol="morpho_blue")

    def test_borrow_morpho_blue_requires_market_id(self):
        from almanak.framework.agent_tools.schemas import BorrowLendingRequest
    ...
    def test_repay_morpho_blue_requires_market_id(self):
        from almanak.framework.agent_tools.schemas import RepayLendingRequest

        with pytest.raises(ValidationError, match="market_id is required"):
            RepayLendingRequest(token="USDC", amount="all", protocol="morpho_blue")

    def test_aave_v3_market_id_not_required(self):
        """Unified-pool protocols don't need market_id."""
        from almanak.framework.agent_tools.schemas import SupplyLendingRequest, WithdrawLendingRequest

        # None of these should raise
        SupplyLendingRequest(token="USDC", amount="100", protocol="aave_v3")
        WithdrawLendingRequest(token="USDC", amount="all", protocol="aave_v3")

    def test_morpho_blue_alias_normalization(self):
        """`morpho-blue` and `morpho blue` should map to `morpho_blue` in PROTOCOL_CAPABILITIES."""
        from almanak.framework.agent_tools.schemas import WithdrawLendingRequest

        # Hyphen alias: must still enforce market_id
        with pytest.raises(ValidationError, match="market_id is required"):
            WithdrawLendingRequest(token="USDC", amount="all", protocol="morpho-blue")

        # Whitespace alias
        with pytest.raises(ValidationError, match="market_id is required"):
            WithdrawLendingRequest(token="USDC", amount="all", protocol="morpho blue")

        # With market_id, both aliases validate
        WithdrawLendingRequest(token="USDC", amount="all", protocol="morpho-blue", market_id="0xabc")
        WithdrawLendingRequest(token="USDC", amount="all", protocol="Morpho_Blue", market_id="0xabc")
