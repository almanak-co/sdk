...
class TestV4LPOpenIntegration:
    ...
    def test_lp_open_event_carries_position_hash(self):
        """VIB-4636 — V4 LP_OPEN's ``position_hash`` lot-matching anchor MUST
        reach the persisted accounting_events payload.

        Pre-fix, ``handle_lp`` ignored ``lp_open_data.position_hash`` and
        the field forwarded as ``None`` even though the V4 receipt parser
        computed the canonical ``keccak(positionManager, tickLower,
        tickUpper, salt)``. The handler must now thread it into
        :class:`LPAccountingEvent` so V4 lot matching at close time can
        resolve.
        """
        anchor = "0x" + "ab" * 32  # 32-byte position key
        lp_open = LPOpenData(
            position_id=1,
            tick_lower=-60000,
            tick_upper=60000,
        ...
        event = handle_lp(outbox, ledger)
        assert event is not None
        assert event.position_hash == anchor

        payload = json.loads(event.to_payload_json())
        assert payload["position_hash"] == anchor
        assert isinstance(payload["position_hash"], str)
        assert len(payload["position_hash"]) == 66

    def test_v3_lp_open_event_position_hash_stays_none(self):
        """V3 LP_OPEN leaves ``position_hash`` ``None`` (the field exists on
        the payload but is only populated for V4). Asserts the field is
        emitted with a stable ``None`` value rather than being absent — keeps
        downstream JSON consumers safe to ``payload["position_hash"]``."""
        lp_open = LPOpenData(
            position_id=42,
            tick_lower=-60000,
            tick_upper=60000,
...
class TestV4RealignTokenPair:
    """VIB-4636 (sibling) — ``_v4_realign_token_pair`` re-pairs (token0, token1)
    by canonical V4 PoolKey order, with a fail-open contract for every shape
    that cannot be resolved. Exercises both the typed-object and dict-fallback
    reads (``deserialize_extracted_data`` can return either)."""
