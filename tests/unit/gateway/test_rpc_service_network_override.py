...
class TestGetRpcUrlNetworkOverride:
    ...
    def _make_service(self, default_network: Network = Network.ANVIL) -> RpcServiceServicer:
        """Create an RpcService through the typed settings owner boundary."""
        return RpcServiceServicer(GatewaySettings(network=default_network))
    ...
    def test_default_uses_settings_network(self, mock_get_rpc_url):
        ...
        svc = self._make_service(default_network=Network.ANVIL)

        svc._get_rpc_url("arbitrum")

        mock_get_rpc_url.assert_called_once_with("arbitrum", network=Network.ANVIL)
    ...
    def test_override_takes_precedence(self, mock_get_rpc_url):
        ...
        svc._get_rpc_url("arbitrum", network_override=Network.MAINNET)

        mock_get_rpc_url.assert_called_once_with("arbitrum", network=Network.MAINNET)
    ...
    @pytest.mark.asyncio
    async def test_proto_default_maps_to_none_override(self):
        """The protobuf empty-string default becomes no typed override."""
        svc = self._make_service(default_network=Network.ANVIL)
        context = MagicMock()
        ...
        with patch.object(svc, "_get_rpc_url", return_value=None) as mock_get_rpc_url:
        ...
        mock_get_rpc_url.assert_called_once_with("arbitrum", network_override=None)
    ...
    def test_unsupported_chain_returns_none(self, mock_get_rpc_url):
        ...
        result = svc._get_rpc_url("unsupported_chain", network_override=Network.MAINNET)
...
class TestAnvilElevationGuard:
    ...
    def _make_service(self, default_network: Network) -> RpcServiceServicer:
        """Create a service whose owner network is already canonical."""
        return RpcServiceServicer(GatewaySettings(network=default_network))

    @pytest.mark.asyncio
    async def test_call_rejects_anvil_override_on_non_anvil_gateway(self):
        svc = self._make_service(Network.MAINNET)
        context = MagicMock()
        request = gateway_pb2.RpcRequest(
            chain="arbitrum",
    ...
    async def test_call_anvil_override_on_anvil_gateway_passes_guard(self):
        svc = self._make_service(Network.ANVIL)
        context = MagicMock()
        request = gateway_pb2.RpcRequest(
            chain="arbitrum",
            method="evm_increaseTime",
    ...
    async def test_call_mainnet_override_on_anvil_gateway_still_allowed(self):
        ...
        assert response.success is False
        context.set_code.assert_called_once_with(grpc.StatusCode.FAILED_PRECONDITION)
        mock_url.assert_called_once_with("arbitrum", network_override=Network.MAINNET)
    ...
    async def test_call_mixed_case_anvil_override_is_normalized(self):
        ...
        denied_svc = self._make_service(Network.MAINNET)
        ...
        allowed_svc = self._make_service(Network.ANVIL)
        ...
        mock_url.assert_called_once_with("arbitrum", network_override=Network.ANVIL)
    ...
    def test_batch_mainnet_override_on_anvil_gateway_still_allowed(self):
        ...
        assert override is Network.MAINNET
    ...
    def test_batch_anvil_override_on_anvil_gateway_passes_guard(self):
        ...
        assert override is Network.ANVIL
        assert response is None
        context.set_code.assert_not_called()
