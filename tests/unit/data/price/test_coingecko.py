...
class TestRateLimitState:
    ...
    def test_backoff_max_cap(self) -> None:
        """Backoff saturates at the configured max_backoff_seconds.

        The numeric anchor is tied to the production constant; if production
        changes the cap, this test should be updated alongside so a silent
        regression to 0 (no backoff) cannot slip through.
        """
        state = RateLimitState()
        ...
        assert state.backoff_seconds == state.max_backoff_seconds
        assert state.max_backoff_seconds == 10.0  # tied to coingecko.py:260
