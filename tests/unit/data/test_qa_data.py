...
Usage:
    Run all QA tests:
        uv run pytest tests/unit/data/test_qa_data.py -v

    Run specific test:
        uv run pytest tests/unit/data/test_qa_data.py::test_qa_cex_spot_prices -v

    Generate report (with --qa-report flag):
        uv run pytest tests/unit/data/test_qa_data.py -v --qa-report

    Skip plot generation for faster runs:
        uv run pytest tests/unit/data/test_qa_data.py -v -m "not generate_plots"
"""
...
# These exercise the QA framework against live CoinGecko/DEX endpoints — keep them
# out of `make test-unit` (which filters with `-m "not integration"`).
