"""Unit tests for TraderJoe V2 (Liquidity Book) Volume Provider.

This module tests the TraderJoeV2VolumeProvider class in
providers/dex/traderjoe_v2_volume.py, covering:
...
class TestTraderJoeV2VolumeProviderInitialization:
    """Tests for TraderJoeV2VolumeProvider initialization."""
    ...
    def test_init_with_custom_fallback(self):
        ...
        provider = TraderJoeV2VolumeProvider(fallback_volume=Decimal("1000"))
    ...
    def test_init_with_custom_rate_limit(self):
        ...
        provider = TraderJoeV2VolumeProvider(requests_per_minute=50)
    ...
    def test_init_with_provided_client(self):
        ...
        provider = TraderJoeV2VolumeProvider(client=mock_client)
...
class TestSupportedChains:
    ...
    def test_supported_chains_include_avalanche(self):
        """Test that Avalanche is supported (primary chain for TraderJoe)."""
        assert Chain.AVALANCHE in SUPPORTED_CHAINS

    def test_arbitrum_not_yet_supported(self):
        """Test that Arbitrum is not yet supported (pending decentralized network deployment)."""
        # Arbitrum support is on hosted service but not decentralized network
    ...
    def test_all_supported_chains_have_subgraph_ids(self):
        ...
        for chain in SUPPORTED_CHAINS:
            ...
            assert TRADERJOE_V2_SUBGRAPH_IDS[chain]  # Non-empty

    def test_subgraph_ids_are_valid_format(self):
        """Test subgraph IDs are non-empty strings."""
        for chain, subgraph_id in TRADERJOE_V2_SUBGRAPH_IDS.items():
...
class TestGetVolume:
    ...
    async def test_get_volume_success(self):
                ...
                "lbPairDayDatas": [
                    {
                        ...
                        "volumeTokenX": "500000.00",
                        "volumeTokenY": "1000000.50",
                        "feesUSD": "4500.15",
                        "totalValueLockedUSD": "25000000.00",
                        "txCount": "150",
                    }
                ]
            }
        ...
        assert call_args.kwargs["subgraph_id"] == TRADERJOE_V2_SUBGRAPH_IDS[Chain.AVALANCHE]
    ...
    async def test_get_volume_no_data_returns_fallback(self):
        ...
        mock_client = MagicMock(spec=SubgraphClient)
        mock_client.query = AsyncMock(return_value={"lbPairDayDatas": []})
    ...
    async def test_get_volume_unsupported_chain_raises(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            await provider.get_volume(
                pool_address="0x123",
                chain=Chain.ETHEREUM,  # Not supported for TraderJoe V2
                start_date=date(2024, 1, 15),
                end_date=date(2024, 1, 15),
            )
    ...
    async def test_get_volume_arbitrum_unsupported(self):
        """Test that Arbitrum (pending deployment) raises ValueError."""
        provider = TraderJoeV2VolumeProvider()
    ...
    async def test_get_volume_normalizes_address(self):
        ...
        call_args = mock_client.query.call_args
        assert call_args.kwargs["variables"]["lbPairAddress"] == "0xabc123def"
...
class TestLiquidityBookSpecificFields:
    """Tests for Liquidity Book specific fields in responses."""

    @pytest.mark.asyncio
    async def test_handles_liquidity_book_response_format(self):
        """Test that provider handles full Liquidity Book response format."""
        mock_client = MagicMock(spec=SubgraphClient)
        mock_client.query = AsyncMock(
            return_value={
                "lbPairDayDatas": [
                    {
                        "id": "0x7eC3717f70894F6d9BA0F8ff67a0115e4c919Cc2-19739",
                        "date": 1705276800,
                        "volumeUSD": "2500000.00",
                        "volumeTokenX": "1000.5",  # e.g., AVAX
                        "volumeTokenY": "1500000.00",  # e.g., USDC
                        "feesUSD": "7500.00",
                        "totalValueLockedUSD": "50000000.00",
                        "txCount": "500",
                    }
                ]
            }
        ...
        assert len(volumes) == 1
        assert volumes[0].value == Decimal("2500000.00")
        assert volumes[0].source_info.confidence == DataConfidence.HIGH
        assert volumes[0].source_info.source == DATA_SOURCE
...
class TestAllSupportedChains:
    ...
    async def test_can_query_all_supported_chains(self, chain: Chain):
        ...
        call_args = mock_client.query.call_args
        assert call_args.kwargs["subgraph_id"] == TRADERJOE_V2_SUBGRAPH_IDS[chain]
