"""Unit tests for the gateway-backed Aerodrome Volume Provider.

**VIB-4870 / W7**: ``AerodromeVolumeProvider`` is now a thin gRPC client
...
class TestInitialization:
    ...
    def test_init_legacy_kwargs_accepted(self):
        provider = AerodromeVolumeProvider(client=MagicMock(), fallback_volume=Decimal("1000"), requests_per_minute=50)
...
class TestSupportedChains:
    ...
    def test_all_supported_chains_have_subgraph_ids(self):
        for chain in SUPPORTED_CHAINS:
            ...
            assert AERODROME_SUBGRAPH_IDS[chain]


class TestGetVolume:
    @pytest.mark.asyncio
    async def test_get_volume_success(self):
        response = make_response([make_point(1705276800, "9999.5")])
