"""Unit tests for SushiSwap V3 Volume Provider.

This module tests the SushiSwapV3VolumeProvider class in providers/dex/sushiswap_v3_volume.py,
covering:
...
class TestSushiSwapV3VolumeProviderInitialization:
    """Tests for SushiSwapV3VolumeProvider initialization."""
    ...
    def test_init_with_custom_fallback(self):
        ...
        provider = SushiSwapV3VolumeProvider(fallback_volume=Decimal("1000"))
    ...
    def test_init_with_custom_rate_limit(self):
        ...
        provider = SushiSwapV3VolumeProvider(requests_per_minute=50)
    ...
    def test_init_with_provided_client(self):
        ...
        provider = SushiSwapV3VolumeProvider(client=mock_client)
...
class TestSupportedChains:
    ...
    def test_supported_chains_include_ethereum(self):
        """Test that Ethereum is supported."""
        # US-008 requires: Ethereum, Arbitrum chains
    ...
    def test_all_supported_chains_have_subgraph_ids(self):
        ...
        for chain in SUPPORTED_CHAINS:
            ...
            assert SUSHISWAP_V3_SUBGRAPH_IDS[chain]  # Non-empty

    def test_subgraph_ids_are_valid_format(self):
        """Test subgraph IDs are non-empty strings."""
        for chain, subgraph_id in SUSHISWAP_V3_SUBGRAPH_IDS.items():
...
class TestGetVolume:
    ...
    async def test_get_volume_success(self):
        ...
        assert call_args.kwargs["subgraph_id"] == SUSHISWAP_V3_SUBGRAPH_IDS[Chain.ETHEREUM]
    ...
    async def test_get_volume_unsupported_chain_raises(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            await provider.get_volume(
                pool_address="0x123",
                chain=Chain.POLYGON,  # Not supported for SushiSwap V3
                start_date=date(2024, 1, 15),
                end_date=date(2024, 1, 15),
            )
...
class TestAllSupportedChains:
    ...
    async def test_can_query_all_supported_chains(self, chain: Chain):
        ...
        call_args = mock_client.query.call_args
        assert call_args.kwargs["subgraph_id"] == SUSHISWAP_V3_SUBGRAPH_IDS[chain]
