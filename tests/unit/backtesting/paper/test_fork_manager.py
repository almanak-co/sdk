...
@pytest.mark.skip(
    reason=(
        "Tests target attributes/kwargs (rpc_timeout_seconds, "
        "health_timeout_seconds, timeout_seconds=) that no longer exist on "
        "the current RollingForkManager. Surfaced by #2066; the test class "
        "needs rewriting against the live API. Tracked: #2068."
    )
)
...
@pytest.mark.skip(
    reason=(
        "RollingForkManager.health_check() was removed in a fork_manager "
        "refactor. Surfaced by #2066; either restore the method or rewrite "
        "the class against the current readiness API. Tracked: #2068."
    )
)
...
@pytest.mark.skip(
    reason=(
        "RollingForkManager.restart() was removed; callers now use "
        "stop()/start() directly. Surfaced by #2066; rewrite or delete. "
        "Tracked: #2068."
    )
)
class TestRestart:
...
@pytest.mark.skip(
    reason=(
        "RollingForkManager._tail_anvil_log() and `_anvil_log_path` were "
        "removed; log capture is handled differently now. Surfaced by "
        "#2066; rewrite or delete. Tracked: #2068."
    )
)
