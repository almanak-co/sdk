...
class TestKnownBalanceSlots:
    ...
    def test_known_slots_has_usdc_for_major_chains(self) -> None:
        """Major USDC chains have a known slot.

        Not every chain in KNOWN_BALANCE_SLOTS has native USDC (e.g. xlayer
        ships with USDT only); only assert for the chains that should.
        """
...
class TestRpcCallRawTimeout:
    ...
    def test_default_timeout_values(self, rpc_url: str, monkeypatch: pytest.MonkeyPatch) -> None:
        """Defaults pull from the documented env-overridable defaults."""
    ...
    def test_custom_timeout_values(self, rpc_url: str) -> None:
        """Both timeout fields accept overrides via the dataclass kwargs."""
        manager = RollingForkManager(
            rpc_url=rpc_url,
            chain="arbitrum",
        ...
        assert manager.health_timeout_seconds == 3.0

    def test_validates_non_positive_rpc_timeout(self, rpc_url: str) -> None:
        """Construction must reject zero/negative rpc_timeout_seconds."""
        with pytest.raises(ValueError, match="rpc_timeout_seconds must be positive"):
            RollingForkManager(rpc_url=rpc_url, chain="arbitrum", rpc_timeout_seconds=0)

    def test_validates_non_positive_health_timeout(self, rpc_url: str) -> None:
        """Construction must reject zero/negative health_timeout_seconds."""
        with pytest.raises(ValueError, match="health_timeout_seconds must be positive"):
            RollingForkManager(rpc_url=rpc_url, chain="arbitrum", health_timeout_seconds=0)

    @pytest.mark.asyncio
    async def test_rpc_call_raw_timeout_returns_false(self, fork_manager: RollingForkManager) -> None:
        """A TimeoutError from the aiohttp session surfaces as (False, None)."""
        mock_session_ctx = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_rpc_call_raw_applies_manager_timeout_to_post(self, rpc_url: str) -> None:
        """The aiohttp POST is called with a timeout matching the manager's rpc_timeout_seconds."""
        import aiohttp

        manager = RollingForkManager(rpc_url=rpc_url, chain="arbitrum", rpc_timeout_seconds=4.5)
        ...
        with patch("aiohttp.ClientSession", CapturingSession):
            success, result = await manager._rpc_call_raw("eth_blockNumber", [])
