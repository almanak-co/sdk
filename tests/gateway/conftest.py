...
def _isolate_gateway_env(tmp_path, monkeypatch):
    ...
    2. Deployment-mode env vars — ``almanak.config.env`` calls
       ...
       ``ALMANAK_GATEWAY_DATABASE_URL`` / hosted-mode identity into the test
       process. With ``DATABASE_URL`` set but hosted mode unset, the gateway startup invariant aborts in
