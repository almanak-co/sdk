...
async def test_teardown_service_request_lifecycle(servicer: TeardownServiceServicer) -> None:
    ctx = _Context()
    request = TeardownRequest(strategy_id="agent-1", mode=TeardownMode.HARD)
    ...
    active = await servicer.GetActiveTeardownRequest(gateway_pb2.GetActiveTeardownRequestRequest(strategy_id="agent-1"), ctx)
    started = await servicer.MarkTeardownStarted(
        gateway_pb2.MarkTeardownStartedRequest(strategy_id="agent-1", total_positions=3),
        ctx,
    )
    completed = await servicer.MarkTeardownCompleted(
        gateway_pb2.MarkTeardownCompletedRequest(strategy_id="agent-1", result_json='{"intents": 3}'),
        ctx,
    )
...
async def test_teardown_service_state_and_approval_lifecycle(servicer: TeardownServiceServicer) -> None:
    ...
    loaded = await servicer.LoadTeardownState(gateway_pb2.LoadTeardownStateRequest(strategy_id="agent-1"), ctx)
...
async def test_teardown_service_returns_invalid_argument_for_malformed_payloads(
    ...
    progress = await servicer.UpdateTeardownProgress(
        gateway_pb2.UpdateTeardownProgressRequest(strategy_id="agent-1", current_phase="bogus"),
        progress_ctx,
    )

    completed_ctx = _Context()
    completed = await servicer.MarkTeardownCompleted(
        gateway_pb2.MarkTeardownCompletedRequest(strategy_id="agent-1", result_json="{not-json"),
        completed_ctx,
    )
