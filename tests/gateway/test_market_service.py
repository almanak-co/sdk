...
class TestOHLCVProviderCaching:
    """Tests for _get_ohlcv_provider() lazy init and caching."""

    def test_returns_same_instance_on_repeated_calls(self, market_service):
        """_get_ohlcv_provider caches the provider — only one instance created."""
        with patch("almanak.gateway.services.market_service._BinanceDataAdapter"):
            provider_1 = market_service._get_ohlcv_provider()
            provider_2 = market_service._get_ohlcv_provider()

        assert provider_1 is provider_2

    def test_creates_routing_provider(self, market_service):
        """_get_ohlcv_provider returns a RoutingOHLCVProvider."""
        from almanak.framework.data.ohlcv.routing_provider import RoutingOHLCVProvider

        with patch("almanak.gateway.services.market_service._BinanceDataAdapter"):
            provider = market_service._get_ohlcv_provider()

        assert isinstance(provider, RoutingOHLCVProvider)

    def test_concurrent_calls_return_same_instance(self, market_service):
        """Concurrent _get_ohlcv_provider() calls return the same instance (lock test)."""
        import concurrent.futures

        with patch("almanak.gateway.services.market_service._BinanceDataAdapter"):
            with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
                futures = [pool.submit(market_service._get_ohlcv_provider) for _ in range(8)]
                providers = [f.result() for f in concurrent.futures.as_completed(futures)]

        # All threads must get the same instance
        assert all(p is providers[0] for p in providers)


class TestBinanceDataAdapter:
    """Tests for _BinanceDataAdapter sync-to-async bridging."""

    def test_fetch_returns_data_envelope(self):
        """_BinanceDataAdapter.fetch() returns a DataEnvelope with candle data."""
        from almanak.framework.data.models import DataEnvelope
        from almanak.gateway.services.market_service import _BinanceDataAdapter

        mock_provider = MagicMock()
        mock_candles = [{"open": 100, "close": 101, "high": 102, "low": 99, "volume": 1000}]
        mock_provider.get_ohlcv = AsyncMock(return_value=mock_candles)

        adapter = _BinanceDataAdapter(mock_provider)
        result = adapter.fetch(token="ETH", quote="USD", timeframe="1h", limit=10)

        assert isinstance(result, DataEnvelope)
        assert result.value == mock_candles
        assert result.meta.source == "binance"
        assert result.meta.cache_hit is False
        mock_provider.get_ohlcv.assert_called_once_with(
            token="ETH", quote="USD", timeframe="1h", limit=10
        )

    def test_name_property(self):
        """Adapter reports its name as 'binance'."""
        from almanak.gateway.services.market_service import _BinanceDataAdapter

        adapter = _BinanceDataAdapter(MagicMock())
        assert adapter.name == "binance"

    def test_health_delegates_to_provider(self):
        """health() exposes underlying provider metrics when available."""
        from almanak.gateway.services.market_service import _BinanceDataAdapter

        mock_provider = MagicMock()
        mock_provider.get_health_metrics.return_value = {"requests": 42, "errors": 1}

        adapter = _BinanceDataAdapter(mock_provider)
        assert adapter.health() == {"requests": 42, "errors": 1}

    def test_health_returns_empty_when_no_metrics(self):
        """health() returns {} if provider has no get_health_metrics method."""
        from almanak.gateway.services.market_service import _BinanceDataAdapter

        mock_provider = MagicMock(spec=[])  # no get_health_metrics attr
        adapter = _BinanceDataAdapter(mock_provider)
