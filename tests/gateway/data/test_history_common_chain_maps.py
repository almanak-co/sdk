...
``test_external_ids_inversion::test_coingecko_onchain_collapse_is_union_with_mantle``
...
class TestCoinGeckoOnchainNetworkMap:
    def test_is_readonly_derived_view(self) -> None:
        assert isinstance(_CHAIN_TO_CG_ONCHAIN_NETWORK, MappingProxyType)

    def test_key_set_is_union_including_mantle(self) -> None:
        # VIB-4851 B1.3: the derive is the union with the price-layer CoinGecko
        # Onchain map, so it gains ``mantle`` on top of the original 9. The
        # intentional widening. ``robinhood`` was added later, so the set is 11.
        expected = {
            "ethereum",
            "arbitrum",
            "base",
        ...
        assert set(_CHAIN_TO_CG_ONCHAIN_NETWORK) == expected
        assert _CHAIN_TO_CG_ONCHAIN_NETWORK["mantle"] == "mantle"  # the gained key

    def test_values(self) -> None:
        assert _CHAIN_TO_CG_ONCHAIN_NETWORK["ethereum"] == "eth"
        assert _CHAIN_TO_CG_ONCHAIN_NETWORK["polygon"] == "polygon_pos"
        assert _CHAIN_TO_CG_ONCHAIN_NETWORK["avalanche"] == "avax"
        assert _CHAIN_TO_CG_ONCHAIN_NETWORK["solana"] == "solana"
    ...
    def test_unsupported_chain_misses(self) -> None:
        ...
        assert _CHAIN_TO_CG_ONCHAIN_NETWORK.get("fantom") is None
