...
class TestAerodromeReceiptParser:
    ...
    def test_extract_lp_close_data_from_burn_event(self):
        """extract_lp_close_data returns correct amounts when Burn event is present."""
        parser = AerodromeReceiptParser(
            chain="base",
        ...
        amount0 = 500_000_000_000_000_000  # 0.5 WETH
        amount1 = 1_000_000_000  # 1000 USDC
        receiver = USER_ADDRESS

        receipt = {
            "transactionHash": "0xburn_test",
            "blockNumber": 99999,
            "status": 1,
            "logs": [create_burn_log(USER_ADDRESS, receiver, amount0, amount1)],
            "gasUsed": 200000,
        ...
        assert lp_close.amount0_collected == amount0
        assert lp_close.amount1_collected == amount1

    def test_extract_lp_close_data_transfer_fallback(self):
        """extract_lp_close_data uses Transfer fallback when no Burn event (VIB-299).

        Simulates a removeLiquidity receipt without a Burn event but with
        Transfer events for the returned tokens. This is the primary fix
        for VIB-299 where Burn events were not being detected.
        """
        parser = AerodromeReceiptParser(
            chain="base",
            token0_address=WETH_ADDRESS,
        ...
        # Simulate removeLiquidity Transfer events (no Burn event)
        transfer_topic = "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"
        zero_addr = "0x0000000000000000000000000000000000000000"

        weth_amount = 500_000_000_000_000_000  # 0.5 WETH returned
        usdc_amount = 1_000_000_000  # 1000 USDC returned
        lp_amount = 1_000_000_000_000_000_000  # LP tokens burned

        logs = [
            # LP token burn (user -> zero address) - should be excluded
            {
                "address": POOL_ADDRESS,
                "topics": [
                    transfer_topic,
                    f"0x000000000000000000000000{USER_ADDRESS[2:].lower()}",
                    f"0x000000000000000000000000{zero_addr[2:]}",
                ],
                "data": f"0x{lp_amount:064x}",
                "logIndex": 0,
            },
            # WETH transfer from pool to user
            {
                ...
                "data": f"0x{weth_amount:064x}",
                "logIndex": 1,
            },
            # USDC transfer from pool to user
            {
                ...
                "data": f"0x{usdc_amount:064x}",
                "logIndex": 2,
            },
        ]

        receipt = {
            "transactionHash": "0xfallback_test",
            "blockNumber": 99998,
            "status": 1,
            "logs": logs,
            "gasUsed": 200000,
        ...
        lp_close = parser.extract_lp_close_data(receipt)
        assert lp_close is not None
        # With known token addresses, amounts are matched by address (Path A)
        assert lp_close.amount0_collected == weth_amount
        assert lp_close.amount1_collected == usdc_amount

    def test_extract_lp_close_data_transfer_fallback_no_token_addrs(self):
        """Transfer fallback works even without known token addresses (Path B)."""
        parser = AerodromeReceiptParser(chain="base")

        transfer_topic = "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"
        zero_addr = "0x0000000000000000000000000000000000000000"

        weth_amount = 500_000_000_000_000_000
        usdc_amount = 1_000_000_000

        logs = [
            # LP burn (to zero) - excluded from fallback
            {
                "address": POOL_ADDRESS,
                "topics": [
                    transfer_topic,
                ...
                "data": f"0x{1_000_000_000_000_000_000:064x}",
                "logIndex": 0,
            },
            # WETH return
            {
        ...
        receipt = {
            "transactionHash": "0xfallback_no_addrs",
            "blockNumber": 99997,
            "status": 1,
            "logs": logs,
            "gasUsed": 200000,
        ...
        lp_close = parser.extract_lp_close_data(receipt)
        assert lp_close is not None
        # Path B groups by token_address, amounts sorted descending
        assert lp_close.amount0_collected == weth_amount
        assert lp_close.amount1_collected == usdc_amount
