"""Base Zodiac negative-authorisation anchor.

Per-chain "negative anchor" companion to the regular intent tests under
``tests/intents/base/``. The default-on Zodiac model wired up in PR #2026
...
every other chain, including base — an over-permissive manifest
regression on, say, aave_v3 SUPPLY would slip past CI because no test
ever revokes a load-bearing aave_v3 target.

This anchor closes the gap on base by parametrizing the canonical
``run_negative_authorisation_case`` helper across one pair per intent-type
family on this chain. Aerodrome LP is intentionally excluded — its
Solidly fungible-LP semantics need a bespoke intent test (issue #2028).
...
# manifest generator covers on this chain. Sushiswap_v3 LP is used here
# instead of uniswap_v3 simply to vary the LP-side connector under test
# vs. the SWAP case; both use the same Uniswap V3 NonfungiblePositionManager
# ABI so either is a valid load-bearing-target source. Aerodrome LP is
# excluded — issue #2028.
