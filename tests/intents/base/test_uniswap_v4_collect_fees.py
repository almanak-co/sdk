"""4-layer intent tests for Uniswap V4 LP_COLLECT_FEES on Base Anvil fork.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for
collecting fees from V4 LP positions via PositionManager on Base:
...
# can actually accrue fees. Using ``WETH/USDC/3000`` here would open
# a position on a separate (rarely-used / uninitialised) pool key and
# the counter-swap would never touch it.
...
# =============================================================================
# CollectFeesIntent Tests -- Uniswap V4 on Base
# =============================================================================
...
class TestUniswapV4CollectFeesIntent:
    """Test Uniswap V4 LP_COLLECT_FEES using CollectFeesIntent on Base.
    ...
    async def test_collect_fees_weth_usdc(
        ...
        print(f"\n{'=' * 80}")
        print("Test: LP_COLLECT_FEES ETH/USDC via Uniswap V4 on Base")
        print(f"{'=' * 80}")
        ...
        for i, tx_result in enumerate(execution_result.transaction_results):
            ...
            if gas_price is None:
                # Fall back to the tx's gasPrice if effectiveGasPrice is absent
                tx_hash = receipt_dict.get("transactionHash") or receipt_dict.get("transaction_hash")
                if tx_hash:
