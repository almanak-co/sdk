"""Production-grade BridgeIntent tests for the Stargate bridge on Base.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for a
USDC bridge from Base to Arbitrum via Stargate V2:
...
To run:
    uv run pytest tests/intents/base/test_stargate_bridge.py -v -s
"""
...
class TestStargateBridgeIntent:
    ...
    async def test_bridge_usdc_base_to_arbitrum_using_intent(  # noqa: layers
        ...
        """Bridge USDC Base -> Arbitrum via Stargate V2 using BridgeIntent.
        ...
        Flow:
        1. Create BridgeIntent for USDC base -> arbitrum (preferred Stargate)
        ...
        stargate_pool_addr = STARGATE_ROUTER_ADDRESSES[BASE_CHAIN_ID]["USDC"]
        ...
        print(f"\n{'='*80}")
        print("Test: USDC Base -> Arbitrum bridge via Stargate BridgeIntent")
        print(f"{'='*80}")
        ...
        # Deposit destination must be the Stargate USDC pool on Base
