"""Production-grade BridgeIntent tests for the Across bridge on Base.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for a
USDC bridge from Base to Arbitrum via Across Protocol V3:

1. Create BridgeIntent for USDC base -> arbitrum
2. Compile to ActionBundle via IntentCompiler (approve + depositV3 on SpokePool)
3. Execute on a Base Anvil fork via ExecutionOrchestrator
...
To run:
    uv run pytest tests/intents/base/test_across_bridge.py -v -s
"""
...
# `V3FundsDeposited(address,...)` event). The Base SpokePool at
# 0x09aea4b2242abC8bb4BB78D537A67a245A7bEC64 emits this topic on depositV3()
...
class TestAcrossBridgeIntent:
    ...
    These tests verify the full BridgeIntent flow on a Base Anvil fork:
    - BridgeIntent creation with proper parameters
    - IntentCompiler generates approve + Across depositV3 transactions
    - Transactions execute successfully against the real Base SpokePool
    ...
    @pytest.mark.asyncio
    async def test_bridge_usdc_base_to_arbitrum_using_intent(  # noqa: layers
        self,
        web3: Web3,
        anvil_rpc_url: str,
        funded_wallet: str,
        ...
        """Bridge USDC Base -> Arbitrum via Across using BridgeIntent.
        ...
        3. Execute via ExecutionOrchestrator on Base Anvil fork
        ...
        assert in_decimals == 6, f"Base USDC is 6 decimals, got {in_decimals}"
        ...
        spoke_pool_addr = ACROSS_SPOKE_POOL_ADDRESSES[BASE_CHAIN_ID]
        ...
        # docstring. Without this, the Base SpokePool reverts with
        ...
        print(f"\n{'='*80}")
        print("Test: USDC Base -> Arbitrum bridge via Across BridgeIntent")
        print(f"{'='*80}")
        ...
        # Deposit destination must be the Across Base SpokePool
