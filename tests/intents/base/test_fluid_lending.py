"""Production-grade lending intent tests for Fluid fTokens (ERC-4626) on Base.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for the
Fluid fUSDC vault on Base (UAT card VIB-5030, D1.S1 + D2.M2):
...
To run:
    uv run pytest tests/intents/base/test_fluid_lending.py -v -s
"""
...
def _assert_targets_ftoken(compilation_result: Any) -> None:
    """The lending transaction targets the chain's Fluid fUSDC vault."""
...
class TestFluidSupplyIntent:
    """Fluid fToken supply/withdraw on the Base fUSDC vault — all 5 layers."""
