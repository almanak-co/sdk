"""4-layer intent tests for Uniswap V4 LP_CLOSE on Base Anvil fork.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for
closing V4 LP positions via PositionManager on Base:
...
To run:
    uv run pytest tests/intents/base/test_uniswap_v4_lp_close.py -v -s
"""
...
async def _open_v4_position(
    ...
    Self-sufficient setup that mirrors the arbitrum / ethereum LP_CLOSE goldens
    so VIB-4356 can land without depending on VIB-4355's parallel LP_OPEN file.
...
# =============================================================================
# LPCloseIntent Tests -- Uniswap V4 on Base
# =============================================================================
...
class TestUniswapV4LPCloseIntent:
    """Test Uniswap V4 LP_CLOSE using LPCloseIntent on Base.
    ...
    async def test_lp_close_weth_usdc(
        ...
        """Test full LP_OPEN -> LP_CLOSE lifecycle for WETH/USDC via V4 on Base.
        ...
        print(f"\n{'='*80}")
        print("Test: LP_CLOSE WETH/USDC via Uniswap V4 on Base")
        print(f"{'='*80}")
    ...
    async def test_lp_close_without_liquidity_fails_compilation(
        ...
        WETH/USDC around ``compiler.compile(...)`` and asserting the Safe
        ...
        # Use a token id well above any minted position on Base V4 at fork
        # time so the on-chain `get_position_liquidity` query returns 0 and the
        # compiler must fall back to the protocol_params-required error path.
        # (The arbitrum/ethereum goldens use "99999" — that id is already
        # minted on Base, so we need a deliberately out-of-range value here.)
        close_intent = LPCloseIntent(
