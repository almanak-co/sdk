"""Production-grade LP Intent tests for Uniswap V3 on Base.
...
To run:
    uv run pytest tests/intents/base/test_uniswap_v3_lp.py -v -s
"""
...
class TestUniswapV3CollectFeesIntent:
    """Test Uniswap V3 LP_COLLECT_FEES using CollectFeesIntent on Base.
    ...
    async def test_collect_fees_weth_usdc(
        ...
        * At least one wallet balance strictly increases (fees transferred back).
        ...
        print(f"\n{'=' * 80}")
        print("Test: LP_COLLECT_FEES WETH/USDC via Uniswap V3 on Base")
        print(f"{'=' * 80}")
