"""Production-grade lending intent tests for Aave V3 on Base.
...
To run:
    uv run pytest tests/intents/base/test_aave_v3_lending.py -v -s
"""
...
class TestAaveV3BorrowIntent:
    ...
    async def test_borrow_usdc_with_wsteth_collateral_using_intent(
        ...
        the Base WETH reserve (no new supplies allowed); wstETH still carries
        an active LTV (~75%) and is unfrozen, so it's the canonical
