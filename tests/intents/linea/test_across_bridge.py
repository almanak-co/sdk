"""Production-grade BridgeIntent tests for the Across bridge on Linea.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for a
USDC bridge from Linea to Arbitrum via Across Protocol V3:

1. Create BridgeIntent for USDC linea -> arbitrum
2. Compile to ActionBundle via IntentCompiler (approve + depositV3 on SpokePool)
3. Execute on a Linea Anvil fork via ExecutionOrchestrator
...
Closes the ``across · BRIDGE · linea`` intent-coverage gap that landed when
Linea joined the Across connector (VIB-4856). Mirrors the five sibling
``test_across_bridge.py`` tests (base / optimism / arbitrum / ethereum /
polygon). See VIB-4298 (intent-coverage epic) / VIB-4871.
...
- Layer 3 (receipt parsing): exercised through AcrossReceiptParser. Its
  parse_receipt() returns a minimal {status, tx_hash} view (bridges are
  source-leg-only routers, so there is no swap/position payload to decode),
  and extract_bridge_data() -- the method ResultEnricher actually calls for
  BRIDGE intents -- returns the typed BridgeData with the parsed amount,
  token, and route. We assert parse_receipt reports on-chain success and
  that the extracted BridgeData amount is positive and exactly equal to the
  bridged amount. We additionally verify the SpokePool-emitted FundsDeposited
  event topic and the ERC-20 Transfer (wallet -> SpokePool) directly from the
  raw receipt logs. Together these satisfy Layer 3.
...
To run:
    uv run pytest tests/intents/linea/test_across_bridge.py -v -s
"""
...
def _sync_anvil_time_to_wall_clock(web3: Web3) -> int:
    ...
    if wall_clock_ts > current_block_ts:
        ...
        # make_request() returns JSON-RPC error payloads instead of raising; check
        # both responses so an Anvil RPC rejection fails the test loudly here
        # instead of silently masquerading as an Across quote/deposit failure
        # several seconds downstream.
...
# `V3FundsDeposited(address,...)` event). The Linea SpokePool at
# 0x7E63A5f1a8F0B4d0934B2f2327DAED3F6bb2ee75 emits this topic on depositV3()
...
class TestAcrossBridgeIntent:
    ...
    These tests verify the full BridgeIntent flow on a Linea Anvil fork:
    - BridgeIntent creation with proper parameters
    - IntentCompiler generates approve + Across depositV3 transactions
    - Transactions execute successfully against the real Linea SpokePool
    ...
    @pytest.mark.asyncio
    async def test_bridge_usdc_linea_to_arbitrum_using_intent(  # noqa: layers
        self,
        web3: Web3,
        anvil_rpc_url: str,
        funded_wallet: str,
        ...
        """Bridge USDC Linea -> Arbitrum via Across using BridgeIntent.
        ...
        3. Execute via ExecutionOrchestrator on Linea Anvil fork
        ...
        assert in_decimals == 6, f"Linea USDC is 6 decimals, got {in_decimals}"
        ...
        spoke_pool_addr = ACROSS_SPOKE_POOL_ADDRESSES[LINEA_CHAIN_ID]
        ...
        # docstring. Without this, the Linea SpokePool reverts with
        ...
        print(f"\n{'='*80}")
        print("Test: USDC Linea -> Arbitrum bridge via Across BridgeIntent")
        print(f"{'='*80}")
        ...
        # Deposit destination must be the Across Linea SpokePool
        ...
        assert execution_result.success is True, f"Execution failed: {execution_result.error}"
        ...
        #
        # Two-step parser contract: AcrossReceiptParser.parse_receipt() returns a
        # minimal {status, tx_hash} view (bridges are source-leg-only routers, so
        # there is no swap/position payload to decode here). The typed bridge
        # amounts live in extract_bridge_data(), which is the method the
        # ResultEnricher actually calls for BRIDGE intents (SUPPORTED_EXTRACTIONS
        # == {"bridge_data"}). We invoke parse_receipt() first to satisfy the
        # 4-layer parse contract, assert it reports on-chain success + the deposit
        # tx hash, then extract the typed BridgeData and assert the parsed amounts
        # are positive AND exactly equal to the bridged amount.
        ...
        assert parse_result["status"] == 1, (
            f"AcrossReceiptParser.parse_receipt must report on-chain success, got {parse_result}"
        )
        assert parse_result["tx_hash"].lower() == deposit_result.tx_hash.lower(), (
            f"parse_receipt tx_hash must match the deposit tx. "
            f"Expected: {deposit_result.tx_hash}, got: {parse_result['tx_hash']}"
        )
        ...
        # Positivity guard (Layer 3): the parsed bridge amount must be > 0 so a
        # parser that silently emits a zero-amount BridgeData fails loudly.
        assert bridge_data.amount_sent_raw > 0, "Parsed BridgeData.amount_sent_raw must be positive"
        assert bridge_data.amount_sent > 0, "Parsed BridgeData.amount_sent must be positive"
