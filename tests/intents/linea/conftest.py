"""Fixtures for Linea intent tests.

Uses gateway's Anvil fixtures to avoid duplicate fork instances.

Default-on Zodiac (Phase G): every intent test runs through Safe + Roles +
``execTransactionWithRole`` by default. The ``funded_wallet`` and
...
def anvil_instance(anvil_linea: AnvilFixture) -> AnvilFixture:
    """Expose the chain-specific AnvilFixture for shared recovery logic."""
    return anvil_linea


@pytest.fixture(scope="module")
def anvil_rpc_url(anvil_linea: AnvilFixture) -> str:
    """Get the Anvil RPC URL for Linea chain."""
    return f"http://127.0.0.1:{anvil_linea.port}"


@pytest.fixture(scope="module")
def web3(anvil_rpc_url: str) -> Web3:
    """Connect to gateway's Anvil fork for Linea."""
...
def _eoa_funded_wallet(web3: Web3, anvil_rpc_url: str, anvil_instance: AnvilFixture) -> str:
    ...
    module-scoped seeding work. For the default (Zodiac) path, ``funded_wallet``
...
def funded_wallet(
    ...
    By default (no ``@pytest.mark.no_zodiac`` marker): returns the per-test
    Safe address from ``zodiac_safe``. The Safe has already been seeded with
    the same CHAIN_CONFIGS ERC-20 balances the EOA path normally receives, so
    tests that read ``funded_wallet`` purely as a token holder keep working.

    With ``@pytest.mark.no_zodiac(reason="...")``: returns ``TEST_WALLET``
    (the EOA), preserving the original module-scoped EOA behaviour.
