"""Fixtures for Mantle chain intent tests.
...
CHAIN_NAME = "mantle"
REQUIRED_CHAIN_ID = 5000


def _seed_wallet_state(web3: Web3, rpc_url: str) -> str:
    """Seed test wallet balances for Mantle on the current fork instance."""
    config = CHAIN_CONFIGS[CHAIN_NAME]

    # Fund with 1000 native tokens (MNT)
    fund_native_token(TEST_WALLET, 1000 * 10**18, rpc_url)
    ...
    for token_symbol, token_address in config.get("tokens", {}).items():
        ...
        if balance_slot is not None:
            try:
                ...
                # Use wrapping for wrapped native tokens
                if token_symbol == "WMNT":
                    wrap_amount = 100 * (10**decimals)
...
def anvil_instance(anvil_mantle: AnvilFixture) -> AnvilFixture:
    """Expose the chain-specific AnvilFixture for shared recovery logic."""
    return anvil_mantle


@pytest.fixture(scope="module")
def anvil_rpc_url(anvil_mantle: AnvilFixture) -> str:
    """Get the Anvil RPC URL for Mantle chain."""
    return f"http://127.0.0.1:{anvil_mantle.port}"


@pytest.fixture(scope="module")
def web3(anvil_rpc_url: str) -> Web3:
    """Connect to gateway's Anvil fork for Mantle chain."""
...
def orchestrator(
    ...
    Without the marker: returns the standard ``ExecutionOrchestrator``.
    """
    if zodiac_safe is not None:
