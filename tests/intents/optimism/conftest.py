"""Fixtures for Optimism intent tests.
...
CHAIN_NAME = "optimism"
REQUIRED_CHAIN_ID = 10


def _seed_wallet_state(web3: Web3, rpc_url: str) -> str:
    """Seed test wallet balances for Optimism on the current fork instance."""
    config = CHAIN_CONFIGS[CHAIN_NAME]

    # Fund with 100 native tokens (ETH on Optimism)
...
def anvil_instance(anvil_optimism: AnvilFixture) -> AnvilFixture:
    """Expose the chain-specific AnvilFixture for shared recovery logic."""
    return anvil_optimism


@pytest.fixture(scope="module")
def anvil_rpc_url(anvil_optimism: AnvilFixture) -> str:
    """Get the Anvil RPC URL for Optimism chain."""
    return f"http://127.0.0.1:{anvil_optimism.port}"


@pytest.fixture(scope="module")
def web3(anvil_rpc_url: str) -> Web3:
    """Connect to gateway's Anvil fork for Optimism."""
