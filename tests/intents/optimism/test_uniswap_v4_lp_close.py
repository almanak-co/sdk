"""4-layer intent tests for Uniswap V4 LP_CLOSE on Optimism Anvil fork.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for
closing V4 LP positions via PositionManager on Optimism:
...
VIB-4360 / VIB-4343: registry edit (adding "optimism" to uniswap_v4 declared
...
To run:
    uv run pytest tests/intents/optimism/test_uniswap_v4_lp_close.py -v -s
"""
...
# WETH/USDC pool with 0.05% fee tier. On Optimism V4 the WETH/USDC/3000
# pool key is uninitialized (LP_OPEN reverts with PoolNotInitialized,
# selector 0x486aa307), but the WETH/USDC/500 pool key IS initialized
# at fork time. We stick with WETH (an ERC-20) rather than native ETH
# because Optimism's deployed V4 PositionManager does not surface
# native-ETH returns from ``TAKE_PAIR`` for the wallet recipient on
# this fork -- the principal ETH stays in the PoolManager and the
# bilateral LP_CLOSE assertion would always fail.
...
async def _open_v4_position(
    ...
    Self-sufficient setup that mirrors the arbitrum / base / ethereum LP_CLOSE
    goldens so VIB-4360 can land without depending on VIB-4359's parallel
    LP_OPEN file. Uses the WETH/USDC/500 pool key (see ``LP_POOL`` comment).
...
# =============================================================================
# LPCloseIntent Tests -- Uniswap V4 on Optimism
# =============================================================================
...
class TestUniswapV4LPCloseIntent:
    """Test Uniswap V4 LP_CLOSE using LPCloseIntent on Optimism.
    ...
    async def test_lp_close_weth_usdc(
        ...
        """Test full LP_OPEN -> LP_CLOSE lifecycle for WETH/USDC via V4 on Optimism.
        ...
        print(f"\n{'=' * 80}")
        print("Test: LP_CLOSE WETH/USDC via Uniswap V4 on Optimism")
        print(f"{'=' * 80}")
        ...
        # Layer 4: Balance Deltas — Safe gains BOTH tokens (principal + any fees)
    ...
    async def test_lp_close_without_liquidity_fails_compilation(
        ...
        # Use a token id well above any minted position on Optimism V4 at fork
        ...
        # ``tests/intents/base/test_uniswap_v4_lp_close.py``.)
        close_intent = LPCloseIntent(
