"""Nightly on-chain permission authorisation coverage — optimism.
...
Note: as of Phase F landing, no declared cases target ``chain="optimism"`` —
``discover_cases`` returns an empty list and pytest collects zero
parametrized tests. The runner lands now so adding a case for an OP-native
connector (Velodrome, Sonne, etc.) is a one-file change in
``permission_cases/`` with no runner edit.
"""
