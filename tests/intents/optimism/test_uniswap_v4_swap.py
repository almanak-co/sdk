"""Production-grade SwapIntent tests for Uniswap V4 on Optimism.
...
To run:
    uv run pytest tests/intents/optimism/test_uniswap_v4_swap.py -v -s
"""
...
EXPECTED_POOL_KEY = PoolKey(
    currency0=CHAIN_CONFIGS[CHAIN_NAME]["tokens"]["WETH"],
...
# =============================================================================
# SwapIntent Tests — Uniswap V4 on Optimism
# =============================================================================
...
class TestUniswapV4SwapIntent:
    """Test Uniswap V4 swaps using SwapIntent on Optimism.
    ...
    async def test_swap_usdc_to_weth_using_intent(
        ...
        """Test USDC -> WETH swap using SwapIntent via Uniswap V4 on Optimism.
        ...
        print(f"\n{'='*80}")
        print("Test: USDC -> WETH Swap via Uniswap V4 on Optimism")
        print(f"{'='*80}")
        ...
        assert weth_received == parsed_swap.amount_out
    ...
    async def test_swap_weth_to_usdc_using_intent(
        ...
        """Test WETH -> USDC swap using SwapIntent via Uniswap V4 on Optimism (reverse direction).
        ...
        print(f"\n{'='*80}")
        print("Test: WETH -> USDC Swap via Uniswap V4 on Optimism")
        print(f"{'='*80}")
        ...
        for tx_result in execution_result.transaction_results:
            if tx_result.receipt:
                ...
                if parse_result.swap_result:
                    ...
                    assert parse_result.swap_result.amount_out_decimal > 0, "Parsed amount_out must be > 0"
                    assert parse_result.swap_result.effective_price > 0, "Parsed effective_price must be > 0"
                    print(f"  Swap: in={parse_result.swap_result.amount_in_decimal}, "
        ...
        assert weth_spent == parsed_swap.amount_in
    ...
    async def test_swap_intent_with_insufficient_balance_fails(
        ...
        print(f"\n{'='*80}")
        print("Test: Uniswap V4 SwapIntent with Insufficient Balance on Optimism")
        print(f"{'='*80}")
