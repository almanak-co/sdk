"""Production-grade BridgeIntent tests for the Stargate bridge on Optimism.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for a
USDC bridge from Optimism to Arbitrum via Stargate V2:
...
To run:
    uv run pytest tests/intents/optimism/test_stargate_bridge.py -v -s
"""
...
class TestStargateBridgeIntent:
    ...
    async def test_bridge_usdc_optimism_to_arbitrum_using_intent(  # noqa: layers
        ...
        """Bridge USDC Optimism -> Arbitrum via Stargate V2 using BridgeIntent.
        ...
        Flow:
        1. Create BridgeIntent for USDC optimism -> arbitrum (preferred Stargate)
        ...
        # Stargate V2 USDC pool on Optimism.
        stargate_pool_addr = STARGATE_ROUTER_ADDRESSES[OPTIMISM_CHAIN_ID]["USDC"]
        ...
        print(f"\n{'='*80}")
        print("Test: USDC Optimism -> Arbitrum bridge via Stargate BridgeIntent")
        print(f"{'='*80}")
        ...
        # Deposit destination must be the Stargate USDC pool on Optimism
