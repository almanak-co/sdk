...
CHAIN_NAME = "optimism"
POSITION_MANAGER = SUSHISWAP_V3[CHAIN_NAME]["position_manager"]
MAX_UINT128 = 2**128 - 1
...
class TestSushiSwapV3CollectFeesIntent:
    """SushiSwap V3 LP_COLLECT_FEES on Optimism (4-layer Intent flow).
    ...
    async def test_collect_fees_weth_usdc(
        ...
        * At least one wallet balance strictly increases (fees transferred back).
        ...
        # 5. Parse receipts — Layer 3 strict: a Collect event must be parsed
        # before its amounts can seed Layer 5. Accepting the no-event case as
        # zero fees would feed synthetic zeros into the accounting comparison
        # and turn a parser miss into a false pass.
        parser = SushiSwapV3ReceiptParser(chain=CHAIN_NAME)
