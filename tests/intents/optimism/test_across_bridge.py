"""Production-grade BridgeIntent tests for the Across bridge on Optimism.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for a
USDC bridge from Optimism to Arbitrum via Across Protocol V3:

1. Create BridgeIntent for USDC optimism -> arbitrum
2. Compile to ActionBundle via IntentCompiler (approve + depositV3 on SpokePool)
3. Execute on an Optimism Anvil fork via ExecutionOrchestrator
...
To run:
    uv run pytest tests/intents/optimism/test_across_bridge.py -v -s
"""
...
# `V3FundsDeposited(address,...)` event). The Optimism SpokePool at
# 0x6f26Bf09B1C792e3228e5467807a900A503c0281 emits this topic on depositV3()
...
class TestAcrossBridgeIntent:
    ...
    These tests verify the full BridgeIntent flow on an Optimism Anvil fork:
    - BridgeIntent creation with proper parameters
    - IntentCompiler generates approve + Across depositV3 transactions
    - Transactions execute successfully against the real Optimism SpokePool
    ...
    @pytest.mark.asyncio
    async def test_bridge_usdc_optimism_to_arbitrum_using_intent(  # noqa: layers
        self,
        web3: Web3,
        anvil_rpc_url: str,
        funded_wallet: str,
        ...
        """Bridge USDC Optimism -> Arbitrum via Across using BridgeIntent.
        ...
        3. Execute via ExecutionOrchestrator on Optimism Anvil fork
        ...
        assert in_decimals == 6, f"Optimism USDC is 6 decimals, got {in_decimals}"
        ...
        spoke_pool_addr = ACROSS_SPOKE_POOL_ADDRESSES[OPTIMISM_CHAIN_ID]
        ...
        # docstring. Without this, the Optimism SpokePool reverts with
        ...
        print(f"\n{'='*80}")
        print("Test: USDC Optimism -> Arbitrum bridge via Across BridgeIntent")
        print(f"{'='*80}")
        ...
        # Deposit destination must be the Across Optimism SpokePool
