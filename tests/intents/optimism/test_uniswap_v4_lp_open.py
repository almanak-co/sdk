...
class TestUniswapV4LPOpenIntent:
    ...
    async def test_lp_open_weth_usdc(
        ...
        # PoolKey order (``currency0 < currency1``), which differs from the
        # user-intent order on Optimism (USDC address < WETH address). Match
        # by token symbol so the test is order-agnostic and still catches
        # a zero / mis-scaled / wrong-token row.
        amounts_by_symbol = {
