...
CHAIN_NAME = "xlayer"
REQUIRED_CHAIN_ID = 196


def _seed_wallet_state(web3: Web3, rpc_url: str) -> str:
    """Seed test wallet balances for X-Layer on the current fork instance."""
    config = CHAIN_CONFIGS[CHAIN_NAME]

    # Fund with 100 native OKB
    fund_native_token(TEST_WALLET, 100 * 10**18, rpc_url)

    # Fund with common tokens via storage slot manipulation.
    # On X-Layer, all tokens (including WETH) are bridged ERC-20s, not wrapped native,
    # so we use storage slot funding for everything (no _wrap_native_token).
...
def anvil_instance(anvil_xlayer: AnvilFixture) -> AnvilFixture:
    """Expose the chain-specific AnvilFixture for shared recovery logic."""
    return anvil_xlayer


@pytest.fixture(scope="module")
def anvil_rpc_url(anvil_xlayer: AnvilFixture) -> str:
    """Get the Anvil RPC URL for X-Layer chain."""
    return f"http://127.0.0.1:{anvil_xlayer.port}"


@pytest.fixture(scope="module")
def web3(anvil_rpc_url: str) -> Web3:
    """Connect to gateway's Anvil fork for X-Layer."""
