"""Production-grade Morpho Blue lending intent tests for Arbitrum.

Mirrors the coverage shape of the Ethereum/Base Morpho Blue tests:
...
Note: Arbitrum Morpho Blue is deployed at 0x6c247b1F... (not the universal
...
class TestMorphoBlueBorrowIntent:
    ...
    async def test_borrow_usdc_with_wsteth_collateral_using_intent(
        ...
        # LTV headroom: 0.2 wstETH (~$700) collateral with 100 USDC borrow = ~14% LTV,
        # well below the 30% cap in .claude/rules/intent-tests.md §10. Prevents
        # flakiness if wstETH price swings during CI (session-scoped CoinGecko prices).
        ...
        # Step 1 — supply the wstETH collateral (two-intent form, #2827: the
...
async def _setup_borrow(
    ...
    """Helper: supply wstETH collateral, then standalone-borrow USDC (two-intent form, #2827).
