"""Production-grade BridgeIntent tests for the Stargate bridge on Arbitrum.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for a
USDC bridge from Arbitrum to Optimism via Stargate V2:
...
- Layer 1 (compilation): verified on-chain-independent (runs locally).
  ...
  asserted to match Optimism (30111) -- this catches a compiler bug
...
To run:
    uv run pytest tests/intents/arbitrum/test_stargate_bridge.py -v -s
"""
...
DEST_CHAIN = "optimism"
# LayerZero endpoint ID for Optimism (used as dstEid on the Stargate send()).
OPTIMISM_LZ_EID = 30111
...
class TestStargateBridgeIntent:
    ...
    async def test_bridge_usdc_arbitrum_to_optimism_using_intent(  # noqa: layers
        ...
        """Bridge USDC Arbitrum -> Optimism via Stargate V2 using BridgeIntent.
        ...
        Flow:
        1. Create BridgeIntent for USDC arbitrum -> optimism (preferred Stargate)
        ...
        stargate_pool_addr = STARGATE_ROUTER_ADDRESSES[ARBITRUM_CHAIN_ID]["USDC"]
        ...
        print(f"\n{'='*80}")
        print("Test: USDC Arbitrum -> Optimism bridge via Stargate BridgeIntent")
        print(f"{'='*80}")
        ...
        # Deposit destination must be the Stargate USDC pool on Arbitrum
        ...
        # "optimism" to the wrong LZ id. Stargate V2 send() signature:
        ...
        assert encoded_dst_eid == OPTIMISM_LZ_EID, (
            f"Stargate send() calldata must encode dstEid={OPTIMISM_LZ_EID} (Optimism LZ id), "
