...
class TestTraderJoeV2LPCloseIntent:
    ...
    async def test_lp_close_position_with_liquidity(
        ...
        # 1. Open position. Capture the OPEN result + intent now, but DEFER
        # its Layer-5 enrichment + persistence to the end of the test so the
        # LP_CLOSE compile/execute/parse/balance hard asserts below run first
        # and an enrichment/persistence regression cannot mask them (gemini
        # PR #2366). All Layer-5 persistence (OPEN then CLOSE) happens at
        # step 8 as live assertions (VIB-4634 fixed — pool_address is now the
        ...
        # so an enricher/persistence regression cannot mask core close logic.
        # Enrichment also happens HERE, not at execute time (CodeRabbit
        # PR #2366). Persist the prior OPEN first (linkage + cost basis),
        # then CLOSE.
