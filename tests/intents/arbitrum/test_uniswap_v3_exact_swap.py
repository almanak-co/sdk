"""Atomic, sealable Uniswap V3 SWAP proofs on Arbitrum."""
...
class TestUniswapV3ExactSwapProof:
    ...
    async def test_swap_exact_safe(
        ...
        await run_uniswap_v3_swap_exact_proof(
            ...
            # The compiler derives its venue-verification factory from the gateway
            # client (framework/intents/compiler.py). Without one, #3749's exact V3
            # pool check has no provider and refuses with "verification unavailable".
            # The LP lanes already pass this adapter; the swap lanes did not.
