"""Atomic, sealable Uniswap V4 SWAP proofs on Arbitrum.
...
assets themselves, not in the native-keyed pool, so these cells declare
``v4_swap.v1``. Fee 500 is pinned because the connector's unpinned default tier
holds no liquidity on this pair and the compiler refuses it at 24.88% price
impact; the pin travels into the route, so the pool traded is the pool asserted.
"""
