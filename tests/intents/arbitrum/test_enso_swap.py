"""Production-grade SwapIntent tests for Enso DEX aggregator on Arbitrum.
...
To run:
    uv run pytest tests/intents/arbitrum/test_enso_swap.py -v -s
"""
...
class TestEnsoSwapIntent:
    """Enso aggregator swaps using SwapIntent on Arbitrum."""
    ...
    async def test_swap_usdc_to_weth_via_enso(
        ...
        """BUY: USDC -> WETH swap via Enso on Arbitrum."""
        ...
        assert usdc_before >= expected_usdc_raw, (
            f"Insufficient USDC balance: have {format_token_amount(usdc_before, in_decimals)}, "
            f"need {swap_amount}"
        )
