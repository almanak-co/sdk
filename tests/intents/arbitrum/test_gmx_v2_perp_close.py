...
class TestGmxV2PerpCloseIntent:
    ...
    async def test_recent_fork_close_eth_long_returns_collateral(
        ...
        # Arbitrum-Anvil's privileged GMX keeper transaction clears the
        # synthetic Foundry wallet's native balance even though the keeper is
        # the transaction sender. Restore the exact measured post-submission
        # balance so the close leg retains its test-only gas reserve.
