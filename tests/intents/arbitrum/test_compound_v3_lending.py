"""Production-grade lending intent tests for Compound V3 (Comet) on Arbitrum.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for the
USDC Comet on Arbitrum (``0x9c4ec768c28520B50860ea7a15bd7213a9fF58bf``):
...
- No ``getUserAccountData``. The Comet exposes per-position state via
  ...
  directly; helpers are file-local until a second Comet test exists.
...
To run:
    uv run pytest tests/intents/arbitrum/test_compound_v3_lending.py -v -s
"""
...
CHAIN_NAME = "arbitrum"
MARKET_ID = "usdc"  # Comet alias key in COMPOUND_V3_COMET_ADDRESSES["arbitrum"]
...
# =============================================================================
# Helper Functions (file-local — not yet shared because this is the first
# Compound V3 test file. Promote to a shared module when a second one lands.)
# =============================================================================
...
def comet_address() -> str:
    """Return the Arbitrum native-USDC Comet address (checksummed by lookup)."""
...
class TestCompoundV3SupplyIntent:
    ...
    async def test_supply_usdc_base_using_intent(
        ...
        print(f"\n{'=' * 80}")
        print(f"Test: Supply {supply_amount} USDC (base) to Compound V3 USDC Comet")
        print(f"{'=' * 80}")
    ...
    async def test_supply_weth_collateral_using_intent(
        ...
        print(f"\n{'=' * 80}")
        print(f"Test: Supply {collateral_amount} WETH as collateral on Compound V3 USDC Comet")
        print(f"{'=' * 80}")
    ...
    async def test_withdraw_usdc_base_using_intent(
        ...
        print(f"\n{'=' * 80}")
        print(f"Test: Withdraw {withdraw_amount} USDC from Compound V3 USDC Comet")
        print(f"{'=' * 80}")
...
class TestCompoundV3BorrowIntent:
    ...
    async def test_borrow_usdc_with_weth_collateral_using_intent(
        ...
        borrow amount exactly. Comet's WETH borrow collateral factor is ~82.5%
        — this test stays well clear.
        """
        tokens = CHAIN_CONFIGS[CHAIN_NAME]["tokens"]
        weth = tokens["WETH"]
        usdc = tokens["USDC"]
        ...
        print(f"\n{'=' * 80}")
        print(f"Test: Borrow {borrow_amount} USDC with {collateral_amount} WETH collateral (Compound V3)")
        print(f"{'=' * 80}")
    ...
    async def test_repay_usdc_using_intent(
        ...
        print(f"\n{'=' * 80}")
        print(f"Test: Repay {repay_amount} USDC on Compound V3 USDC Comet")
        print(f"{'=' * 80}")
