"""Avalanche Zodiac negative-authorisation anchor.

Per-chain "negative anchor" companion to the regular intent tests under
``tests/intents/avalanche/``. The default-on Zodiac model wired up in
...
every other chain, including avalanche.

This anchor closes the gap on avalanche by parametrizing the canonical
``run_negative_authorisation_case`` helper across one pair per intent-type
family on this chain. SushiSwap V3 is intentionally excluded — it was
removed from the Avalanche routing maps in VIB-2069 (zero usable
liquidity).
...
_CASES: list[PermissionTestCase] = [
    ...
    PermissionTestCase(
        ...
        config={
            "token0": "USDC",
            "token1": "WAVAX",
            "pool": "USDC/WAVAX/3000",
            "amount0": "100",
            "amount1": "2",
            # USDC-per-WAVAX range — WAVAX is typically $20-40, pick a
            # wide window so the mint is two-sided rather than entirely
            # out-of-range.
            "range_lower": "10",
            "range_upper": "100",
        },
