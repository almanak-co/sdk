...
# =============================================================================
# CollectFeesIntent Tests (LP_COLLECT_FEES) — VIB-4348
# =============================================================================
...
class TestUniswapV3CollectFeesIntent:
    """Test Uniswap V3 LP_COLLECT_FEES using CollectFeesIntent on Avalanche.
    ...
    @pytest.mark.xfail(
        strict=True,
        reason="VIB-5968: same-pool fee-accrual fixture not yet wired — swap routes to different fee tier than LP position (as of 2026-05-13; re-pointed to VIB-5968 2026-07-24)",
    )
    async def test_collect_fees_wavax_usdc(
        ...
        """Collect fees from an in-range WAVAX/USDC position after a same-pool swap.
        ...
        * Wallet balance deltas exactly equal parser-extracted Collect amounts
          (token0/token1 ordered by address: WAVAX < USDC on Avalanche, so
          amount0=WAVAX, amount1=USDC).
        """
        tokens = CHAIN_CONFIGS[CHAIN_NAME]["tokens"]
        usdc_addr = tokens["USDC"]
        wavax_addr = tokens["WAVAX"]
        ...
        print(f"\n{'=' * 80}")
        print("Test: LP_COLLECT_FEES WAVAX/USDC via Uniswap V3 on Avalanche")
        print(f"{'=' * 80}")
        ...
        # On Avalanche WAVAX (0xB31f...) < USDC (0xB97E...), so token0=WAVAX,
        # token1=USDC.
        usdc_after = get_token_balance(web3, usdc_addr, funded_wallet)
        wavax_after = get_token_balance(web3, wavax_addr, funded_wallet)
        ...
        print(f"WAVAX received: {format_token_amount(wavax_received, wavax_decimals)}")

        if int(wavax_addr, 16) < int(usdc_addr, 16):
            parsed_wavax_collected, parsed_usdc_collected = (
                parsed_amount0_collected,
                parsed_amount1_collected,
            )
        else:
            parsed_usdc_collected, parsed_wavax_collected = (
                parsed_amount0_collected,
                parsed_amount1_collected,
            )

        assert wavax_received == parsed_wavax_collected, (
            f"WAVAX wallet delta must exactly equal parsed Collect amount. "
            f"wallet={wavax_received}, parsed={parsed_wavax_collected}"
        )
