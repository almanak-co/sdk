"""Production-grade lending intent tests for Aave V3 on Avalanche.
...
To run:
    uv run pytest tests/intents/avalanche/test_aave_v3_lending.py -v -s
"""
...
class TestAaveV3BorrowIntent:
    ...
    async def test_borrow_usdc_with_wavax_collateral_using_intent(
        ...
        """Test borrowing USDC against WAVAX collateral via the two-intent form.
        ...
        Flow:
        1. Setup: supply WAVAX collateral via SupplyIntent (use_as_collateral
        ...
        print(f"\n{'='*80}")
        print(f"Test: Borrow {borrow_amount} USDC with {collateral_amount} WAVAX collateral (two-intent form)")
        print(f"{'='*80}")
        ...
        # Step 1 — supply the WAVAX collateral (the two-intent form is the
        ...
        # Layer 3 (supply leg): the Supply event must land on the WAVAX reserve.
            ...
            if tx_result.receipt:
                ...
                if supply_parse.success and supply_parse.supplies:
                    collateral_supply = next(
                        (s for s in supply_parse.supplies if s.reserve.lower() == wavax.lower()),
                        None,
                    )
                    assert collateral_supply is not None, (
                        f"Expected a Supply event for WAVAX collateral. Got reserves: "
                        f"{[s.reserve for s in supply_parse.supplies]}"
                    )
                    assert collateral_supply.amount > 0, "WAVAX collateral supply amount must be > 0"
                    supply_event_seen = True
        assert supply_event_seen, "Expected at least one Supply event for the WAVAX collateral leg"
        ...
        print("\n--- Results ---")
        print(f"WAVAX spent (collateral): {format_token_amount(wavax_spent, wavax_decimals)}")
        print(f"USDC received (borrowed): {format_token_amount(usdc_received, usdc_decimals)}")

        # Verify WAVAX was spent as collateral
