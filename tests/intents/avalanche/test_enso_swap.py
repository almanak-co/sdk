"""Production-grade SwapIntent tests for Enso DEX aggregator on Avalanche.
...
To run:
    uv run pytest tests/intents/avalanche/test_enso_swap.py -v -s
"""
...
class TestEnsoSwapIntent:
    """Enso aggregator swaps using SwapIntent on Avalanche."""

    @pytest.mark.intent(IntentType.SWAP)
    @pytest.mark.asyncio
    @pytest.mark.xfail(
        strict=False,
        reason="VIB-5969: Enso aggregator off-chain route non-determinism on Anvil avalanche fork — quote can route through a sub-pool whose fork-block state differs from mainnet, causing execution revert. Needs 10/10 run validation per intent-tests rule #12 (as of 2026-05-13; re-pointed to VIB-5969 2026-07-24)",
    )
    async def test_swap_usdc_to_wavax_via_enso(
        self,
        web3: Web3,
        anvil_rpc_url: str,
        funded_wallet: str,
        ...
        """BUY: USDC -> WAVAX swap via Enso on Avalanche."""
