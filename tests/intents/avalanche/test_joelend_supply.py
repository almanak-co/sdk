"""Production-grade supply/withdraw intent tests for Joe Lend (Banker Joe) on Avalanche.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for Joe Lend supply operations:
...
To run:
    uv run pytest tests/intents/avalanche/test_joelend_supply.py -v -s
"""
...
pytestmark = pytest.mark.no_zodiac(reason="joelend connector not in manifest matrix")
...
@pytest.mark.supply
@pytest.mark.lending
class TestJoeLendSupplyIntent:
    """Test Joe Lend supply/withdraw operations using SupplyIntent and WithdrawIntent.

    These tests verify the full Intent flow:
    - SupplyIntent creation with token symbols and amounts
    - IntentCompiler generates correct Joe Lend transactions (Compound V2 mint)
    - Transactions execute successfully on-chain
    - JoeLendReceiptParser correctly interprets Mint/Redeem events
    ...
    async def test_supply_usdc_using_intent(
        ...
        Flow:
        1. Create SupplyIntent for USDC on Joe Lend
        2. Compile to ActionBundle using IntentCompiler
        ...
        print(f"\n{'='*80}")
        print(f"Test: Supply {supply_amount} USDC.e to Joe Lend using SupplyIntent")
        print(f"{'='*80}")
        ...
        for i, tx_result in enumerate(execution_result.transaction_results):
            ...
            if tx_result.receipt:
                ...
                if parse_result.success and parse_result.supply_amount > 0:
                    ...
                    print(f"  jTokens minted: {parse_result.j_tokens_minted}")
                    assert parse_result.supply_amount > 0, "Supply amount must be positive"
                    assert parse_result.j_tokens_minted > 0, "jTokens minted must be positive"
    ...
    async def test_withdraw_usdc_using_intent(
        ...
        print(f"\n{'='*80}")
        print(f"Test: Withdraw {withdraw_amount} USDC.e from Joe Lend using WithdrawIntent")
        print(f"{'='*80}")
        ...
        for tx_result in execution_result.transaction_results:
            if tx_result.receipt:
                ...
                if parse_result.success and parse_result.withdraw_amount > 0:
                    ...
                    print(f"  jTokens redeemed: {parse_result.j_tokens_redeemed}")
                    assert parse_result.withdraw_amount > 0, "Withdraw amount must be positive"
                    assert parse_result.j_tokens_redeemed > 0, "jTokens redeemed must be positive"
    ...
    async def test_supply_intent_with_insufficient_balance_fails(
        ...
        print(f"\n{'='*80}")
        print("Test: SupplyIntent with Insufficient Balance (Joe Lend)")
        print(f"{'='*80}")
