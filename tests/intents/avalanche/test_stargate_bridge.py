"""Production-grade BridgeIntent tests for the Stargate bridge on Avalanche.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for a
USDC bridge from Avalanche to Arbitrum via Stargate V2:

1. Create BridgeIntent for USDC avalanche -> arbitrum
2. Compile to ActionBundle via IntentCompiler (approve + send() on Stargate pool)
3. Execute on an Avalanche Anvil fork via ExecutionOrchestrator
...
- Layer 4 (balance deltas): source-chain USDC is asserted to decrease by
  ...
  (ETH) drain is NOT asserted -- see the in-function NOTE for why.

To run:
    uv run pytest tests/intents/avalanche/test_stargate_bridge.py -v -s
"""
...
class TestStargateBridgeIntent:
    ...
    These tests verify the full BridgeIntent flow on an Avalanche Anvil fork:
    ...
    @pytest.mark.asyncio
    async def test_bridge_usdc_avalanche_to_arbitrum_using_intent(  # noqa: layers
        self,
        web3: Web3,
        anvil_rpc_url: str,
        funded_wallet: str,
        ...
        """Bridge USDC Avalanche -> Arbitrum via Stargate V2 using BridgeIntent.
        ...
        Flow:
        1. Create BridgeIntent for USDC avalanche -> arbitrum (preferred Stargate)
        2. Compile via IntentCompiler (approve + Stargate pool send())
        3. Execute via ExecutionOrchestrator on Avalanche Anvil fork
        ...
        assert in_decimals == 6, f"Avalanche USDC is 6 decimals, got {in_decimals}"

        # Stargate V2 USDC pool on Avalanche.
        stargate_pool_addr = STARGATE_ROUTER_ADDRESSES[AVALANCHE_CHAIN_ID]["USDC"]
        ...
        print(f"\n{'='*80}")
        print("Test: USDC Avalanche -> Arbitrum bridge via Stargate BridgeIntent")
        print(f"{'='*80}")
        ...
        # Deposit destination must be the Stargate USDC pool on Avalanche
        ...
        # the pool's `send()` drains substantially more native ETH than the
