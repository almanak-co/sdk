"""Atomic, sealable Aave V3 lending proofs on Avalanche.

Collateral is WAVAX; the supplied/borrowed asset is native USDC. Avalanche WETH
carries no balance slot in the intent-test token table, so the fork cannot seed
it -- the wrapped native token is the only fundable collateral reserve.
"""
