...
class TestSparkBorrowIntent:
    ...
    async def test_borrow_usdc_with_weth_collateral_using_intent(
        ...
        # Supply 1 WETH as collateral, borrow 500 USDC (~17% LTV at $3000 ETH —
        # well under the 30% LTV cap from the Phase C plan).
            ...
            if tx_result.receipt:
                ...
                if parse_result.success:
                    ...
                    if parse_result.borrows:
                        ...
                        assert usdc_borrow.interest_rate_mode == 2, (
                            "Spark adapter only supports variable rate (mode=2)"
                        )
    ...
    async def test_repay_usdc_using_intent(
        ...
        # First borrow USDC (1 WETH collateral, 500 USDC borrow — same sizing as the borrow test)
        compiler = IntentCompiler(
            chain=CHAIN_NAME,
            wallet_address=funded_wallet,
            price_oracle=price_oracle,
