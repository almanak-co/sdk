"""Production-grade SwapIntent tests for SushiSwap V3 on Ethereum.

VIB-4307 / VIB-4298 Phase 2: backfill ``(sushiswap_v3, SWAP, ethereum)``
...
To run:
    uv run pytest tests/intents/ethereum/test_sushiswap_v3_swap.py -v -s
"""
...
# WETH/USDC 0.3% fee tier — same pool the Ethereum LP test uses, so
# liquidity at the fork block is already proven by ``test_sushiswap_v3_lp.py``.
SWAP_FEE_TIER = 3000
...
class TestSushiSwapV3SwapIntent:
    """Test SushiSwap V3 swaps using SwapIntent on Ethereum."""
    ...
    async def test_swap_usdc_to_weth_using_intent(
        ...
        """USDC -> WETH swap via SwapIntent on SushiSwap V3 (Ethereum).
        ...
        print(f"\n{'=' * 80}")
        print("Test: USDC -> WETH Swap via SwapIntent (SushiSwap V3 / Ethereum)")
        print(f"{'=' * 80}")
