"""Production-grade BridgeIntent tests for the Stargate bridge on Ethereum.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for a
USDC bridge from Ethereum to Arbitrum via Stargate V2:
...
To run:
    uv run pytest tests/intents/ethereum/test_stargate_bridge.py -v -s
"""
...
class TestStargateBridgeIntent:
    ...
    async def test_bridge_usdc_ethereum_to_arbitrum_using_intent(  # noqa: layers
        ...
        """Bridge USDC Ethereum -> Arbitrum via Stargate V2 using BridgeIntent.
        ...
        Flow:
        1. Create BridgeIntent for USDC ethereum -> arbitrum (preferred Stargate)
        ...
        stargate_pool_addr = STARGATE_ROUTER_ADDRESSES[ETHEREUM_CHAIN_ID]["USDC"]
        ...
        print(f"\n{'='*80}")
        print("Test: USDC Ethereum -> Arbitrum bridge via Stargate BridgeIntent")
        print(f"{'='*80}")
        ...
        # Deposit destination must be the Stargate USDC pool on Ethereum
