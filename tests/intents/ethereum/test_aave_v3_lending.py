"""Production-grade lending intent tests for Aave V3 on Ethereum.
...
To run:
    uv run pytest tests/intents/ethereum/test_aave_v3_lending.py -v -s
"""
...
class TestAaveV3SupplyIntent:
    ...
    async def test_supply_usdc_using_intent(
        ...
        # Parse receipts (Layer 3) - assert decoded Supply amount > 0
    ...
    async def test_withdraw_usdc_using_intent(
        ...
        await orchestrator.execute(supply_result.action_bundle, execution_context)
...
class TestAaveV3BorrowIntent:
    ...
    async def test_borrow_usdc_with_wsteth_collateral_using_intent(
        ...
        Uses wstETH as the collateral asset because Aave governance has set
        Ethereum WETH's ``ltv=0`` (no new collateral usage); wstETH still
        carries an active LTV (~78.5%) and is unfrozen, so it's the canonical
        ...
        # Step 1 — supply the wstETH collateral (the two-intent form is the
            ...
            if tx_result.receipt:
                ...
                if supply_parse.success and supply_parse.supplies:
                    collateral_supply = next(
                        (s for s in supply_parse.supplies if s.reserve.lower() == wsteth.lower()),
                        None,
                    )
        ...
        # Parse receipts (Layer 3) - assert decoded Borrow amount > 0
        parser = AaveV3ReceiptParser(chain=CHAIN_NAME)
    ...
    async def test_repay_usdc_using_intent(
        ...
        1. Setup: Borrow USDC with wstETH collateral first
           (wstETH chosen for collateral because Aave set Ethereum WETH ltv=0
            and froze WETH on Arbitrum/Base — see #1696.)
