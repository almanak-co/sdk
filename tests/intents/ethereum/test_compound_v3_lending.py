"""Production-grade lending intent tests for Compound V3 (Comet) on Ethereum.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for the
USDC Comet on Ethereum (``0xc3d688B66703497DAA19211EEdff47f25384cdc3``):
...
To run:
    uv run pytest tests/intents/ethereum/test_compound_v3_lending.py -v -s
"""
...
CHAIN_NAME = "ethereum"
MARKET_ID = "usdc"  # Comet alias key in COMPOUND_V3_COMET_ADDRESSES["ethereum"]
...
def comet_address() -> str:
    """Return the Ethereum native-USDC Comet address (checksummed by lookup)."""
...
class TestCompoundV3BorrowIntent:
    ...
    async def test_borrow_usdc_with_weth_collateral_using_intent(
        ...
        Ethereum is ~82.5% — this test stays well clear.
        """
        tokens = CHAIN_CONFIGS[CHAIN_NAME]["tokens"]
        weth = tokens["WETH"]
        usdc = tokens["USDC"]
