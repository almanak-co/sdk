"""Production-grade SwapIntent tests for LiFi on BNB.
...
To run:
    uv run pytest tests/intents/bnb/test_lifi_swap.py -v -s
"""
...
@pytest.mark.swap
class TestLiFiSwap:
    """LiFi same-chain swaps using SwapIntent on BNB/BSC."""

    @pytest.mark.intent(IntentType.SWAP)
    @pytest.mark.asyncio
    @pytest.mark.xfail(
        strict=False,
        reason="VIB-5969: LiFi aggregator route flake on Anvil bnb fork — sub-route revert / rate-limit retry stall, needs 10/10 run validation per intent-tests rule #12 (as of 2026-05-13; re-pointed to VIB-5969 2026-07-24)",
    )
    async def test_swap_usdc_to_wbnb_using_intent(
        self,
        web3: Web3,
        funded_wallet: str,
        ...
        """USDC -> WBNB swap via LiFi SwapIntent on BNB."""
