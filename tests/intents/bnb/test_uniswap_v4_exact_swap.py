"""Atomic, sealable Uniswap V4 SWAP proofs on BNB Smart Chain.
...
The connector settles USDC -> USDT here in the pool keyed on the two ERC-20
assets themselves rather than in the native-keyed pool, so these cells declare
``v4_swap.v1``. Fee 100 is pinned because it is the deepest initialised
USDC/USDT pool of that shape on bsc (liquidity 4.45e24, the deepest V4 pool on the chain); the pin travels into
