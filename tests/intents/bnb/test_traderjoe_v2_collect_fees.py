"""4-layer intent test for TraderJoe V2 LP_COLLECT_FEES on BNB Chain Anvil fork.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for
collecting fees from a Liquidity Book position via ``LBPair.collectFees``:

1. Open an LP position in the WBNB/USDT binStep=15 LBPair (setup).
...
To run::
    ...
    uv run pytest tests/intents/bnb/test_traderjoe_v2_collect_fees.py -v -s
"""
...
# WBNB/USDT LBPair with binStep=15 — the only WBNB/USDT pair on BSC whose
# active bin implies a market-aligned price. Pinned in
...
# Other bin steps either return fully empty reserves (bs=25) or imply prices
# an order of magnitude off market (bs=50/100) — see VIB-4377.
POOL = "WBNB/USDT/15"
LP_AMOUNT_WBNB = Decimal("0.1")  # token X (~$55 at ~$546 WBNB)
LP_AMOUNT_USDT = Decimal("100")  # token Y (USDT — 18-decimal Binance-Peg)

# Wide range to ensure both tokens are deposited (TJv2 LP_OPEN bin selection
# is range-driven). Range expressed in USDT-per-WBNB; the active bin tracks
# ~$546 at the 2026-05-14 fork block, so [200, 2000] comfortably brackets it.
...
class TestTraderJoeV2CollectFeesIntent:
    ...
    - LP_OPEN setup mints a position into the WBNB/USDT binStep=15 LBPair.
    ...
    @pytest.mark.asyncio
    async def test_collect_fees_wbnb_usdt(
        self,
        web3: Web3,
        funded_wallet: str,
        orchestrator: ExecutionOrchestrator,
        ...
        """LP_COLLECT_FEES on a freshly-opened WBNB/USDT LB position.
        ...
        4. **Balance deltas** (success path): WBNB / USDT balances are
        ...
        fail_if_traderjoe_pool_missing(web3, CHAIN_NAME, wbnb_addr, usdt_addr, BIN_STEP)
        usdt_decimals = get_token_decimals(web3, usdt_addr)
        wbnb_decimals = get_token_decimals(web3, wbnb_addr)

        print(f"\n{'=' * 80}")
        print("Test: LP_COLLECT_FEES WBNB/USDT via CollectFeesIntent (TraderJoe V2)")
        print(f"{'=' * 80}")
        ...
        print(f"WBNB before collect: {format_token_amount(wbnb_before, wbnb_decimals)}")
        ...
        if not execution_result.success:
            ...
            # the snapshot — neither WBNB nor USDT may move. Catches a
            # silent partial-effect regression in either the LBPair or the
            # orchestrator's revert handling.
            wbnb_after_failed = get_token_balance(web3, wbnb_addr, funded_wallet)
            usdt_after_failed = get_token_balance(web3, usdt_addr, funded_wallet)
            assert wbnb_after_failed == wbnb_before, (
                f"On failed LP_COLLECT_FEES, WBNB balance must be unchanged "
                f"(before={wbnb_before}, after={wbnb_after_failed})."
            )
        ...
        print(f"WBNB delta: {format_token_amount(wbnb_delta, wbnb_decimals)}")
        print(f"USDT delta: {format_token_amount(usdt_delta, usdt_decimals)}")
        assert wbnb_delta >= 0, (
            f"WBNB balance must not decrease during fee collection, got {wbnb_delta}"
        )
        assert usdt_delta >= 0, (
            f"USDT balance must not decrease during fee collection, got {usdt_delta}"
        )
        assert wbnb_delta == parsed_fees_x, (
            f"WBNB wallet delta ({wbnb_delta}) must equal parser fees_x "
            f"({parsed_fees_x}) — receipt parser and on-chain state disagree"
        )
        assert usdt_delta == parsed_fees_y, (
            f"USDT wallet delta ({usdt_delta}) must equal parser fees_y "
        ...
        # (token X = WBNB → fees0, token Y = USDT → fees1).
        assert Decimal(collect_payload["amount0"]) == Decimal("0")
        assert Decimal(collect_payload["amount1"]) == Decimal("0")
        assert Decimal(collect_payload["fees0_collected"]) == _to_human(parsed_fees_x, wbnb_decimals)
        assert Decimal(collect_payload["fees1_collected"]) == _to_human(parsed_fees_y, usdt_decimals)
