"""Nightly on-chain permission authorisation coverage — BSC (BNB Chain).
...
Naming note: the directory is ``bnb/`` to match the existing BSC intent-test
fixtures (``anvil_rpc_url``, ``funded_wallet``, etc. live in
``tests/intents/bnb/conftest.py``), but the canonical chain key the case
files and framework use is ``"bsc"`` — hence ``CHAIN_NAME = "bsc"`` here
and ``@pytest.mark.bsc`` on the tests. See the pancakeswap_v3 case file
