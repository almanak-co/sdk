...
class TestUniswapV4LPCloseIntent:
    ...
    async def test_lp_close_bnb_usdt(
        ...
        # VIB-5117 / VIB-5131: capture the native BNB close PRINCIPAL from the
        ...
        # VIB-5117 / VIB-5131: the native BNB leg (currency0 = 0x0) is returned via
        # TAKE_PAIR with no ERC-20 Transfer, so the parser leaves amount0_collected
        # None. The pre-burn principal capture above must have stamped the REAL
        # principal so the close cost basis is two-legged and realized PnL is exact
        # — never a one-legged understatement (VIB-5131 fails that closed) and never
        # a degenerate-tick measured-zero (which would read as the whole native
        # principal being lost).
        assert close_payload["amount0"] is not None, (
            "native BNB close leg must be measured from the pre-burn principal "
        ...
        assert close_native >= open_native * Decimal("0.9"), (
            "native BNB close principal must be ~the deposited amount, not a "
