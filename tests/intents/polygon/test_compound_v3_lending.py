"""Production-grade lending intent tests for Compound V3 (Comet) on Polygon.
...
Comet.baseToken() returns the bridged address, and Compound V3 reverts at
compile time if the borrow_token does not address-match — so the intent must
use ``token="USDC.e"`` and ``market_id="usdc_e"``. The conftest seeds USDC.e
into the funded wallet alongside the other chain tokens (see
``CHAIN_CONFIGS["polygon"]`` in ``tests/intents/conftest.py``).
...
To run:
    uv run pytest tests/intents/polygon/test_compound_v3_lending.py -v -s
"""
...
CHAIN_NAME = "polygon"
# Polygon Compound V3 Comet's base asset is bridged USDC (USDC.e), not native
# USDC. The market id is "usdc_e" in COMPOUND_V3_COMET_ADDRESSES["polygon"];
# the token symbol passed to intents is "USDC.e" so the compiler resolves to
# the USDC.e address (0x2791...) and matches Comet.baseToken().
MARKET_ID = "usdc_e"
BASE_TOKEN_SYMBOL = "USDC.e"
...
def comet_address() -> str:
    """Return the Polygon USDC.e Comet address (checksummed by lookup)."""
...
class TestCompoundV3SupplyIntent:
    ...
    async def test_supply_weth_collateral_using_intent(
        ...
        parser = CompoundV3ReceiptParser(base_decimals=get_token_decimals(web3, tokens[BASE_TOKEN_SYMBOL]))
...
class TestCompoundV3BorrowIntent:
    ...
    async def test_borrow_usdc_with_weth_collateral_using_intent(
        ...
        Compound V3 has no origination fee, so USDC.e received must equal
        the borrow amount exactly. Comet's WETH borrow collateral factor on
        Polygon is ~80% — this test stays well clear.
        """
        tokens = CHAIN_CONFIGS[CHAIN_NAME]["tokens"]
        weth = tokens["WETH"]
        usdc = tokens[BASE_TOKEN_SYMBOL]
        ...
        intent = BorrowIntent(
            ...
            borrow_token=BASE_TOKEN_SYMBOL,
