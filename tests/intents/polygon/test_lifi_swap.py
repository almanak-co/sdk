"""Production-grade SwapIntent tests for LiFi on Polygon.
...
To run:
    uv run pytest tests/intents/polygon/test_lifi_swap.py -v -s
"""
...
@pytest.mark.swap
class TestLiFiSwap:
    """LiFi same-chain swaps using SwapIntent on Polygon."""

    @pytest.mark.intent(IntentType.SWAP)
    @pytest.mark.asyncio
    @pytest.mark.xfail(
        strict=False,
        reason="VIB-5969: LiFi aggregator route flake on Anvil polygon fork — sub-route revert / rate-limit retry stall, needs 10/10 run validation per intent-tests rule #12 (as of 2026-05-13; re-pointed to VIB-5969 2026-07-24)",
    )
    async def test_swap_usdc_to_weth_using_intent(
        ...
        """USDC -> WETH swap via LiFi SwapIntent on Polygon."""
