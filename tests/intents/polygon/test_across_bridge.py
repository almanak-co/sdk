"""Production-grade BridgeIntent tests for the Across bridge on Polygon.

Tests the full Intent -> Compile -> Execute -> Parse -> Verify flow for a
USDC bridge from Polygon to Arbitrum via Across Protocol V3:
...
To run:
    uv run pytest tests/intents/polygon/test_across_bridge.py -v -s
"""
...
# `V3FundsDeposited(address,...)` event). The Polygon SpokePool at
# 0x9295ee1d8C5b022Be115A2AD3c30C72E34e7F096 emits this topic on depositV3()
...
class TestAcrossBridgeIntent:
    ...
    - Transactions execute successfully against the real Polygon SpokePool
    ...
    async def test_bridge_usdc_polygon_to_arbitrum_using_intent(  # noqa: layers
        ...
        """Bridge USDC Polygon -> Arbitrum via Across using BridgeIntent.
        ...
        spoke_pool_addr = ACROSS_SPOKE_POOL_ADDRESSES[POLYGON_CHAIN_ID]
        ...
        # docstring. Without this, the Polygon SpokePool reverts with
        ...
        print(f"\n{'='*80}")
        print("Test: USDC Polygon -> Arbitrum bridge via Across BridgeIntent")
        print(f"{'='*80}")
        ...
        # Deposit destination must be the Across Polygon SpokePool
