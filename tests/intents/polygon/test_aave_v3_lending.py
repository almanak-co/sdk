"""Production-grade lending intent tests for Aave V3 on Polygon.
...
To run:
    uv run pytest tests/intents/polygon/test_aave_v3_lending.py -v -s
"""
...
class TestAaveV3BorrowIntent:
    ...
    async def test_borrow_usdc_with_weth_collateral_using_intent(
        ...
        # Step 1 — supply the WETH collateral (the two-intent form is the
            ...
            if tx_result.receipt:
                ...
                if supply_parse.success and supply_parse.supplies:
                    collateral_supply = next(
                        (s for s in supply_parse.supplies if s.reserve.lower() == weth.lower()),
                        None,
                    )
