"""Production-grade LP Intent tests for SushiSwap V3 on Polygon.
...
CHAIN_NAME = "polygon"
POSITION_MANAGER = get_address(SUSHISWAP_V3, "polygon", "position_manager")
MAX_UINT128 = 2**128 - 1

# Pool: WETH/USDC 0.05% fee tier on SushiSwap V3 Polygon
...
class TestSushiSwapV3LPOpenIntent:
    ...
    async def test_lp_open_weth_usdc(
        ...
        # Validate pool existence before running the test
        fail_if_v3_pool_missing(web3, CHAIN_NAME, "sushiswap_v3", usdc_addr, weth_addr, 500)
...
class TestSushiSwapV3CollectFeesIntent:
    """SushiSwap V3 LP_COLLECT_FEES on Polygon (4-layer Intent flow).
